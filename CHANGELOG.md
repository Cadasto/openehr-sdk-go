...
- **AQL semantic and portability lint (REQ-161, PROBE-097).** `lint` gains REQ-160-backed containment checks and spec-gap portability advisories; `lint.Options` grows a relation field, a compile-time break for positional struct literals. Every entry point that runs the checks can be given a deployment's own relation, including the validation seam via `validation.ValidateAQLWithTypeRelation` — `ValidateAQL(q, c)` is that call with a nil relation.
- **AQL containment admissibility relation (REQ-160, PROBE-097).** `aql/contain` answers whether a containment route connects two AQL class expressions, derived from the pinned BMM plus a cited overlay table and extensible per deployment. The relation is over RM type names and is spelled `contain.TypeRelation`.
