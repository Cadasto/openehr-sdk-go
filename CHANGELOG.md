...
- **REST clients** — System; EHR read/write (composition, ehrstatus, directory, contribution, item-tags, REQ-059); Definition ADL 1.4 templates + stored AQL CRUD (REQ-057); AQL Query (`openehr/client/query/`, `openehr/aql/`, REQ-055); Admin housekeeping (`openehr/client/admin/`, REQ-099).
...
- **Conformance probes** (`testkit/probes/`) — versioned-write (010–013), definition, query, serialize (030–031, 033–034), discovery (040–041), admin (PROBE-070), OPT path resolution (PROBE-022), primitive constraint validate (PROBE-024).
- **Worked examples** (`cmd/examples/{canonical_json,canxml_roundtrip,ehr_create,opt-parse}`) — canjson, JSON↔XML round-trip, end-to-end EHR creation against httptest, local OPT path resolution.
