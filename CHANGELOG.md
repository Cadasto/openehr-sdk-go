...
- **AQL containment evidence under CI (REQ-160, PROBE-100).** The vendored EHRbase FROM-combination corpus now runs as a ratchet: every shape a conformant engine accepts must parse and draw no Error-severity containment code, with no behaviour change.
