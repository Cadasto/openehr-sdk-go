...
- **openEHR terminology vocabulary (REQ-034).** The openEHR Terminology (TERM Release-3.0.0) is vendored and generated into the stdlib-only `openehr/terminology` accessor; lifecycle-state and change-type validity is now group membership — `contribution.Builder.Build` refuses a batch `audit.change_type` outside the group or without its pinned rubric on every path, `WithAudit` included (REQ-130) — and the `ctx/` `math_function` default is `640|actual` ([plan](docs/plans/archive/2026-09-07-terminology-vocabulary.md)).
