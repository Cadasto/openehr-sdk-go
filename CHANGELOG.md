...
- **Null 2xx bodies classify as no representation (REQ-151, REQ-094, REQ-144).** `transport.IsNoRepresentationBody` is the single empty-body predicate across every 2xx decode arm, so a JSON `null` body no longer decodes as an all-zero resource ([plan](docs/plans/archive/2026-09-04-error-axis-leftovers.md)).
