...
- **Reserved stored-query name refused client-side (REQ-057, PROBE-099/100).** Store operations reject the reserved name `aql` before the wire and stored-path diagnostics name the invoked operation — alongside widened probe corpora and new docs-transcript and prose-count guards.
