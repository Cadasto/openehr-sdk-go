...
- **Template upload interoperates with XML-only deployments (REQ-082, PROBE-067).** `definition.UploadTemplate` now sends `Accept: application/json, application/xml` and takes the template id from the `Location` header when the `201` body is empty or an XML `OperationalTemplate`, so a deployment that serves the ADL 1.4 template surface only as XML (EHRbase) no longer answers `406 Not Acceptable`; a JSON `TemplateIdentifier` body decodes as before.
