...
- **Generated LOCATABLE identity surface (ADR 0013; REQ-031, REQ-040).** `bmmgen` emits `Get*/Set*` identity accessors widening the sealed `rm.Locatable` plus `rm.MutableLocatable`, and the reverse registry `rm.RMTypeName` / `rm.IsTypedNil`, retiring the hand-maintained identity switches.
