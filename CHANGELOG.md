...
- OPT wire decode extension (`openehr/template/`) — captures `<term_definitions code="...">` and `<term_bindings terminology="...">` blocks on C_ARCHETYPE_ROOT nodes. New accessors `ArchetypeRoot.Terms() / Term(code) / TermBindings()`; new types `ArchetypeTerm`, `TermBinding`, `CodedTermRef`. Per-archetype-root scope preserved (at-codes are not globally unique across sibling roots).
- Compiled-template foundation (`internal/templatecompile/`, REQ-100 follow-up Phase 4) — `Compile(*template.OperationalTemplate, ...Options) (*Compiled, error)` produces a walker-friendly tree with: stable AQL paths on every node (`CompiledNode.AQLPath`); O(1) lookup via `Compiled.NodeAt(path)` / `AllByRMType(rm)` / `AllByNodeID(at)`; implicit RM-mandatory attributes injected from `rminfo.Default` (e.g. `COMPOSITION.composer/language/territory`) with `CompiledAttribute.Implicit() / Required() / RMTypeName()`; per-archetype-root term lookup via parent-walk (`CompiledNode.Term(code)`); flat `TermBindings()` list. Internal package — public surface (`template.Compile` / `template.Compiled`) deferred until REQ-101 (composition builder) and REQ-102 (validation) confirm the stable shape.
