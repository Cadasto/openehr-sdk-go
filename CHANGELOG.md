...
- **AQL write paths refuse previously emitted invalid AQL (REQ-119).** *Breaking:* validating `Build` / `FormatWhere` / `Emit` return `ErrInvalidQuery` where they once wrote structure-changing or unparseable text, and `Emit` now re-parses and structurally verifies its own output at the cost of one extra parse — see `traceability.yaml`.
