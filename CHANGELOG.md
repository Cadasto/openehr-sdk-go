...
Tenth `v0.x` minor — the **RM behavioural-functions cycle**: hand-written realisations of the pure/derived RM functions — `Parse*` identifier parsing/derivation, `VERSION.is_branch`, temporal `DV_*` read/inspect/compare/convert, and locatable-path read access via the new `openehr/rm/rmpath` — plus real-world OPT synthesis & validation coverage (SDK-GAP-12). Additive, with one minor behaviour change to `ehr.VersionUID` partial-segment accessors (well-formed server-issued uids unaffected); safe to upgrade from `v0.9.0`.
...
- **Real-world OPT synthesis & validation coverage (SDK-GAP-12; REQ-102/107/110).** The template-driven instance synthesiser (`openehr/instance`, `composition.NewSkeleton`) and the lockstep validation walker now handle three shapes that production CKM-derived OPTs exercise but the in-repo fixtures had not: `ELEMENT` / `CLUSTER.name` (`DV_TEXT`), BMM generic interval types (`DV_INTERVAL<DV_QUANTITY>` plus the `DV_COUNT` / `DV_DATE_TIME` / `DV_DATE` / `DV_TIME` / `DV_PROPORTION` / `DV_ORDERED` siblings), and optional same-`node_id` content roots (Minimal policy emits the first colliding sibling only, so validator node binding stays unambiguous). New `internal/templateinstance/rmwrite` + `openehr/validation/rmread` support for `DV_COUNT` / `DV_QUANTITY` / `DV_MULTIMEDIA` / `DV_INTERVAL<T>`; `INTEGER` / `REAL` AOM 1.4 primitive short-names now validate through DV-wrapper scalar channels; shared `rm.AsInt64` / `rm.AsReal` coercion and a single `rminfo.IsNonStorableAttr` predicate keep generator and validator in lockstep; `DV_MULTIMEDIA.media_type` defaults to the `IANA_media-types` code set. Vendored real-world OPT corpus (`Referral Request.v1`, `Demonstration.v1`, `social`); PROBE-027 extended. Additive.
