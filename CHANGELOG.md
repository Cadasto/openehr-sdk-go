...
- **Probe runner and in-memory sandbox (REQ-082, partial).** `testkit/probe` runs the conformance catalogue in Sandbox or Live mode with fail-closed mode selection and one shared result type; `sandbox.Backend` is an in-memory openEHR REST backend (EHR create/get/head plus scripted routes) implementing `http.RoundTripper` with no listener and no credentials. The per-package `Result` types under `testkit/probes/*` are now aliases of `probe.Result`, so their `Status` field is `probe.Status` rather than `string` ([plan](docs/plans/2026-08-18-probe-runnability.md)).
