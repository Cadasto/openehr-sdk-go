...
- **ITS-REST conformance remediation (REQ-059/093/095/099).** Corrected wire deviations in the landed REST clients against the vendored OpenAPI contract — dotted-attribute `openehr-audit-details`/`openehr-version` headers, System `OPTIONS /`, Admin `/admin/ehr/all`, Definition `/example` — plus smaller payload/status/coverage fixes and vendored EHRbase specs (`resources/ehrbase/`, Apache-2.0). See [plan](docs/plans/2026-06-19-its-rest-conformance-remediation.md).
