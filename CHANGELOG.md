...
- **Live conformance snapshot for the stored-AQL path (REQ-082).** `TestLiveStoredQuerySnapshot` witnesses stored-query registration and execution against a real CDR — the `{name, version}` recovered from the store's `Location` header (PROBE-079) and a stored execution returning a typed `ResultSet` scoped to the run's own EHR (PROBE-066) — opt-in and skipped in CI like the core and composition snapshots.
