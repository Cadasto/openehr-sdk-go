...
- **Execution-oriented parsed AQL AST (SDK-GAP-17; REQ-113).** New `parse.ParseQuery` + `Document.Query()` return a structured, generated-type-free `parse.Query` AST (SELECT items / FROM containment tree / WHERE expression tree / ORDER BY / LIMIT / OFFSET); shared `aql.WhereExpr` + `Value` vocabulary unified across read and write sides (concretes exported as `Comparison`, `Junction`, `NotExpr`, `ExistsExpr`, `MatchesExpr`, `LikeExpr`, `ParamValue`/`StringValue`/`IntValue`/`RealValue`/`BoolValue`). `Document.Tree()` exposes the validated ANTLR tree as a Tier-1 interim. `(*Query).Emit` + `aql.FormatWhere`/`FormatValue` close the round-trip loop (16-case idempotence property).
