...
JSON tags **MUST** preserve the original BMM property name (snake_case), with `omitempty` for non-mandatory single properties. Example:

```go
type DVQuantity struct {
    DVAmount                            // embedded ancestor
...
|---|---|---|
| `Boolean` | `bool` | |
| `Integer` | `int32` | Pinned to 32-bit per the BMM spec |
| `Integer64` | `int64` | |
| `Real` | `float64` | The BMM `Real` is single-precision conceptually but openEHR JSON uses unbounded-precision numbers; `float64` is the safe choice |
| `Double` | `float64` | Same as `Real` |
| `Character` | `rune` | |
| `String` | `string` | |
| `Octet` | `byte` | |
...
- The generator emits a `typereg/registry_gen.go` that registers every concrete RM `_type` with its constructor (REQ-040).
