...
*compression algorithms*, *integrity check algorithms*). The Reference Model's own
invariants reference them: `AUDIT_DETAILS.Change_type_valid`, `EVENT_CONTEXT.Setting_valid`,
...
> and rubrics. Read it to *check* the pin, not to copy codes out of it. SDK code takes
> code sets, rubrics, and membership verdicts from the generated `openehr/terminology`
...
|---|---|
| `openehr/terminology` | The generated, stdlib-only accessor. It exposes every group and code set of this pin as typed, closed, source-ordered values with code → rubric, rubric → code, and membership lookups, plus the pinned release version (§ REQ-034). |
...
that the manifest does not list. **Never hand-edit the pin.** Its value lies in being byte-identical to
