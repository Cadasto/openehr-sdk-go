...
// SDK-AQL-005: comments are skipped (see COMMENT below), so the COMMENT_CHANNEL
// declaration is dropped as now-unused.
...
COMMENT: (
    SYM_DOUBLE_DASH ' ' ~[\r\n]* ('\r'? '\n' | EOF)
    | SYM_DOUBLE_DASH ('\r'? '\n' | EOF)
    ) -> skip; // SDK-AQL-005: skip, not COMMENT_CHANNEL
...
// functions
// SDK-AQL-001: the string `CONTAINS` function is unreachable in the official
// grammar — the `CONTAINS` containment operator (declared earlier) shadows it.
// Spell the string function `CONTAINS_STR` instead; the pattern is inlined here
// (a separate token would either shadow CONTAINS_STR or be unreachable), so
// `CONTAINS_STR(a,b)` lexes as STRING_FUNCTION_ID while `CONTAINS` stays the
// containment operator. Spec spelling `CONTAINS(a,b)` is intentionally rejected.
STRING_FUNCTION_ID: LENGTH | (C O N T A I N S '_' S T R) | POSITION | SUBSTRING | CONCAT_WS | CONCAT ;
...
fragment ESCAPE_SEQ: '\\' ['"?abfnrtv\\*] ;   // SDK-AQL-004: add \* ( \" already present in baseline)
