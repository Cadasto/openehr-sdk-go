...
`make aqlgen` consumes) has a row here. IDs (`SDK-AQL-NNN`) are stable and are
never reused. There are two classes:
...
`.aql` files must parse and `.reject` files must not. The `parse` package tests assert both.
...
- **Symptom:** `STRING_FUNCTION_ID` lists `CONTAINS`, but the `CONTAINS`
  containment-operator token is declared earlier and shadows it (on equal length,
  the first rule wins). The string function `CONTAINS(a,b)` is therefore
  unreachable: it lexes as the containment operator and mis-parses.
...
- **Symptom:** official AQL has no bare `SELECT *`; projection columns must be
  explicit. The SDK admits it as a convenience that some CDRs honour.
...
- **Symptom:** `limitClause` accepts only `INTEGER`, so a parameterised page
  size or offset (`LIMIT $count OFFSET $start`) cannot be expressed. Everywhere
  else a value appears, `$param` is allowed.
