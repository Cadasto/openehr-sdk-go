...
**CHANGELOG.md** — update only on request or when cutting a release. **One single-sentence bullet (~35 words max) per artefact class**: artefact + scope + key REQ/PROBE, never API inventories or per-REQ breakdowns (those live in `traceability.yaml`, commits, and PR bodies); the optional per-release summary line is one sentence too. Release notes are generated verbatim from the block, so a long entry is a defect — err short. Pre-1.0: `### Added` only — fix-ups and dropped experiments fold into the relevant bullet, not a `### Changed`/`### Fixed` section.
