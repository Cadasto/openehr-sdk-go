...
**Covers:** REQ-144 (new — Definition metadata decoding: timestamp tolerance + empty-list shape), ADR 0019 (new — the timestamp-tolerance decision)
...
| Step | Status |
|---|---|
| Spec/registry updated (ADR 0019, wire.md § REQ-144, REQ.md row + band flip) | ☐ |
...
- [ ] Write **ADR 0019 — Definition metadata timestamp tolerance** (`docs/adr/0019-definition-timestamp-tolerance.md`, metadata bullets per the ADR 0017 form: Status / Strand: — (direct decision) / Introduces REQ-144 / Related ADR 0004, ADR 0015, REQ-095). Decision: decode of the two Definition metadata timestamps accepts a **closed** layout set (below); encode stays stdlib RFC 3339. The ADR records the two deliberate departures: (a) for `saved` the tolerance exceeds the vendored pin, which lists `saved` as `required` with `format: date-time` — a keyed compatibility exception to REQ-095's OpenAPI-authoritative rule, granted on deployment evidence (the same evidence bar ADR 0004 sets: a real catalog failure, not pre-emption); (b) absent / `null` / empty values yield the zero `time.Time` rather than an error, prioritising catalog readability over pin strictness. Consequences: the layout set is one-way (removal is a break); zone-less values acquire UTC on decode and re-marshal with a `Z` suffix the wire never carried (the round-trip asymmetry, stated openly).
- [ ] Update `docs/adr/README.md` with the 0019 row.
- [ ] Write **`wire.md` § REQ-144** as `### REQ-144 — Definition metadata decoding` under `## REST leaf operations`, directly after the REQ-143 block (~line 364). The bullets below are the `sdd-specify` seed — the § authored in Phase 0 is the only normative home, and its RFC-2119 wording is written there, not copied from here:
...
- [wire.md § REQ-095](../specifications/wire.md#req-095) — the OpenAPI-authoritative rule ADR 0019 keys its exception against.
