...
- Canonical normative prose for REQ-151 is written in Phase 0 (`transport.md`), the REQ-052 clause likewise; both land ahead of the code in this plan's own commits.
...
- The two indexes `make spec-check` cannot see: a `docs/roadmap.md` row for REQ-151 (model: the REQ-150 row) and a touch on the transport row (line ~110); the REQ.md band prose records "151 taken 2026-08-30 …, leaving 152–159 free."
...
| Step | Status |
|---|---|
| Spec/registry updated (transport.md § REQ-151, wire.md REQ-052 clause, REQ.md row + band prose, ADR 0018, PROBE-101 catalog entry) | ☐ |
| Indexes spec-check misses (roadmap.md rows, REQ.md band prose) | ☐ |
| Code (`transport/errors.go`, `transport/client.go`, six leaf files, `canjson/marshal.go`) | ☐ |
...
- [ ] Write **`transport.md` § REQ-151** as `## REQ-151 — Typed 2xx decode failure`, inserted after the REQ-150 block (~line 239), before `## Coverage`. Contract points the § makes normative (RFC-2119 wording belongs there, not here):
...
- [ ] Amend **§ REQ-094** (line ~127): one sentence noting the *decode-failure* arm of `ehr.Create` is now typed by REQ-151 while the empty-body keyed exception stands unchanged.
- [ ] Amend **`wire.md` § REQ-052**: a new clause under Canonical JSON — a `canjson.Marshal` / `MarshalIndent` failure wraps the new encode-only sentinel `canjson.ErrInvalidValue`, `errors.Is`-distinguishable from the decode-side `canjson.ErrInvalidShape` and from `transport.ErrInvalidShape` (three distinct sentinel values). Note neutrally that `canxml` reuses one sentinel across both directions (`canxml/marshal.go`); aligning it is out of scope.
- [ ] Add the **PROBE-101 catalog entry** to `conformance.md` (before implementation — ordering is normative): Title "2xx decode failure carries the raw body"; Preconditions: a sandbox server returning 200 with a syntactically valid JSON body that does not match the requested type; Wire assertion: the leaf error satisfies `errors.As` for `*transport.DecodeError` with `Body` equal to the served bytes; the same leaf against 404 yields `*transport.WireError` / `ErrNotFound` and not the new type; `Error()` output contains no fragment of the body; a list leaf served a JSON object instead of an array yields the same type with the body recoverable. Effect: read-only. Modes: Sandbox. Status: Draft (flipped in Phase 4).
- [ ] REQ.md: registry row for REQ-151 (after the REQ-150 row, line ~112) with `Impl.` `planned`, flipped to `landed` at close-out; band-prose sentence in line ~148 ("151 … taken 2026-08-30 by REQ-151, homed in transport.md, leaving 152–159 free").
- [ ] `traceability.yaml`: new REQ-151 block (`canonical: docs/specifications/transport.md#req-151--typed-2xx-decode-failure`, `status: draft`, `implementation: planned` until code lands in this same PR, `packages: [transport, openehr/client/definition, openehr/client/system, openehr/client/demographic, openehr/client/ehr/composition]`, `adrs: [docs/adr/0018-raw-bytes-on-decode-error.md]`, `plans:` this file). Do **not** list `probes: [PROBE-101]` while the catalog entry is Draft and the REQ is landed/partial — either flip both in the same commit (Phase 4) or keep `implementation: planned` until then; spec-check fails a landed REQ citing a Draft probe. Append this plan + a one-line `notes:` amendment record to the REQ-052 and REQ-094 blocks.
...
- [ ] Failing tests first in `transport/client_test.go` (external `_test` package, stdlib `testing` only):
  - `TestDecodeTypedErrorCarriesBody` — httptest 200 with body `{"unexpected":"shape"}` decoded as an RM type; assert `errors.As` yields `*transport.DecodeError`, `Body` equals the served bytes, `Unwrap` reaches the decoder error, metadata on the triple is non-nil.
  - `TestDecodeErrorStringIsValueFree` — `Error()` contains the method and route but no substring of the body. Removing the value-free guard must fail this named test.
...
- [ ] Run the four tests; then the package suite.
...
**Definition of done:** every in-scope read-path 2xx decode failure surfaces as `*transport.DecodeError`; the excluded arms are covered by a test asserting they are **not** the new type (`contribution.Commit` decode failure stays `*ehr.NoRepresentationError`).

### Phase 4 — PROBE-101

**Tasks:**

- [ ] Implement `testkit/probes/transport/probe_101_typed_decode_error.go` (the transport probe dir exists — PROBE-091 is the model) plus its case in `probes_test.go`, driving an httptest-style sandbox server through the four catalog assertions (typed error + body equality; 404 arm; value-free `Error()`; list-leaf object body).
- [ ] Flip the `conformance.md` entry to `Status: Implemented`, add the Coverage-matrix row, and in the same commit set `traceability.yaml` REQ-151 to `implementation: landed` with `probes: [PROBE-101]` and the test files listed; flip the REQ.md `Impl.` cell to `landed`.
- [ ] `make probe-status` (informational — inline probes print MISSING by filename heuristic; this one matches the `probe_NNN_*.go` pattern so it should list), then `make spec-check`.
...
- [ ] Failing test in `openehr/serialize/canjson`: `canjson.Marshal(make(chan int))` satisfies `errors.Is(err, canjson.ErrInvalidValue)` and does **not** satisfy `errors.Is` for `canjson.ErrInvalidShape` nor `transport.ErrInvalidShape`; the underlying `*json.UnsupportedTypeError` stays reachable via `errors.As`.
...
- [ ] `go test ./openehr/serialize/... -count=1`. The leaf packages that call `canjson.Marshal` and wrap with their own `fmt.Errorf` chains are unaffected — the sentinel travels inside their wraps.
...
- [ ] Roadmap rows (REQ-151 row modeled on REQ-150's; transport summary row); confirm the REQ.md band prose from Phase 0 survived review edits.
...
- [transport.md § REQ-094](../specifications/transport.md#req-094--prefer-response-shape-negotiation) — cross-reference amendment only; the `ehr.Create` empty-body keyed exception stands.
