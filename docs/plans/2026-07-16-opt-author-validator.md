...
- PROBE-085 green.
- `traceability.yaml` (REQ-114 `planned → landed`, then `make spec-gen`), `roadmap.md`, `docs/examples.md` updated.
...
| Step | Status |
|---|---|
| REQ-114 § + map entry (`clinical-modeling.md`, `traceability.yaml`) | |
...
2. Add the `traceability.yaml` entry (`implementation: planned`; spec section **Status:** `Draft`) and run `make spec-gen`.
