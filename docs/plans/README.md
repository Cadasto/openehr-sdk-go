...
|---|---|---|---|
| [2026-09-24-rm-floor-archetype-roots.md](2026-09-24-rm-floor-archetype-roots.md) | The template-less floor reports `is_archetype_root` for a COMPOSITION / EHR_STATUS / EHR_ACCESS / PARTY without `archetype_details`, and `required` / `rm_version_valid` for an incomplete `ARCHETYPED` on any LOCATABLE | REQ-112 (no new id; `partial` until landed) | Spec-first; reported by the consuming CDR project |
...
| Plan | Scope | Covers | Probe |
|---|---|---|---|
| [2026-08-18-probe-runnability.md](2026-08-18-probe-runnability.md) | Give the existing probe catalog (75 entries) the execution modes REQ-082 mandates: one shared result type + a runner (phase 1), a real `sandbox/` in-memory backend (phase 2), then Cassette recording/replay (phase 3 — recorder, replayer, the `cmd/probe-record` capture harness and the `ehr-create` / `ehr-lifecycle` recordings landed; coverage beyond those two remains) and Live mode (phase 4 — wired, pre-release; runs need a reachable CDR) | REQ-082 (**`partial`**) | PROBE-078 / 079 / 065 / 062 / 060 promoted out of `Deferred`/`Draft` to Implemented, and PROBE-102 / 103 / 104 added for the STRAND-09 REST surfaces that had no probe; PROBE-077 still Deferred |

Phase 0 (the REQ-082 normative prose — mode selection, the probe result contract, per-mode rules, cross-mode precedence) landed 2026-08-18 alongside [STRAND-11](../specifications/research-strands.md#strand-11--probe-recording-format-har-or-a-purpose-built-yaml), which opened the recording-format fork; the strand is now **Resolved** — [ADR 0020](../adr/0020-cassette-recording-har.md) picked HAR 1.2 against a real capture. Of the eight probes and the [STRAND-09](../specifications/research-strands.md#strand-09--its-rest-conformance-follow-ups) item this plan gated, PROBE-065 / 078 / 079 and STRAND-09 item 1 (PROBE-062 / 102 / 103 / 104) are Implemented as of 2026-09-09; PROBE-077 is still Deferred and STRAND-09 item 2 waits on an ADR. REQ-082 is one of the three [`v1.0.0` gate](../releases.md#v100-gate) conditions.
...
|---|---|---|
| [2026-06-23-simplified-formats.md](2026-06-23-simplified-formats.md) | Umbrella — FLAT/STRUCTURED codecs (WebTemplate export Phase 2 **landed**; Phase 3 codecs **landed**); carries the shared simplified-template model and what is left of the REQ-053 edge deferrals. The datatype-coverage residue went to the [coverage ratchet](archive/2026-08-03-flat-coverage-ratchet.md) (landed) and the three residuals it named to the [closure plan](archive/2026-08-05-flat-rm-attributes.md) (landed) — the umbrella's § Residual scope is the ledger of the four items still open (reused-sibling FLAT, the deferred underscore families, the ITS `ctx/` sketches, the WebTemplate-projection gaps); `.schema` media types on input closed 2026-09-05 via `simplified.ParseMediaType` | REQ-053; PROBE-076 |
...
Phase 3 — FLAT / STRUCTURED composition codecs (`openehr/serialize/simplified`, REQ-053) — **landed and was archived** ([archive/2026-07-14-flat-structured-codecs.md](archive/2026-07-14-flat-structured-codecs.md)): bidirectional codecs, `ctx/`, `|raw`/`|other`, full datatype set, PROBE-076, and `WithTemplate` name + RM-mandatory completion so a decoded composition validates against the OPT. Residual edge deferrals are tracked by the umbrella above + the package [`deviations.md`](../../openehr/serialize/simplified/deviations.md) — the EVENT_CONTEXT optionals left that list on 2026-08-05 by riding REQ-140's underscore grammar at their real paths ([ADR 0016](../adr/0016-event-context-optionals-underscore-spelling.md)) rather than gaining `ctx/` short forms; `.schema` media types on input — once the list's oldest survivor — closed 2026-09-05 via `simplified.ParseMediaType` ([spec-and-interop leftovers plan](archive/2026-09-04-spec-interop-leftovers.md)); the upstream byte-conformance probe landed as PROBE-086, and its coverage ratchet is archived at [2026-08-03-flat-coverage-ratchet.md](archive/2026-08-03-flat-coverage-ratchet.md).
