...
The AQL class-predicate plan **landed 2026-08-08 and was archived** ([archive/2026-08-08-aql-class-predicate-splice-and-source-text.md](archive/2026-08-08-aql-class-predicate-splice-and-source-text.md)): REQ-119's last verbatim-splice position is closed, so a class predicate round-trips as identity, a spliced one is refused on bracket escape, and the `VERSION` bracket answers to its own sub-grammar. Getting there needed the coupled extraction fix — every re-emitted predicate and path reads its source text from the character stream, since `GetText()` drops the whitespace an `AND` / `OR` / `MATCHES` predicate needs — which **changes emitted AQL**: predicate and path text is verbatim, comments and caller spacing included. PROBE-090 extended, its catalogue six suites → ten. The first scanner's confrontation corpus could not fail (0 of 2954 cases parsed as a *different* query); it was rebuilt to rail on that property instead of the accept/refuse split, and immediately found a defect three reviews had not. A full `nodePredicate` validator stays deferred, and path positions stay out of scope by REQ-055 rule 3 on a corrected ground.
