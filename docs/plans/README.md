...
Five draft plans from a consumer→SDK gap survey plus the review-round follow-ups that were living only in session notes. All amend shipped REQs in place (normative deltas ride in the implementing PR) **except** the RM-function stubs, which propose new ids. **None is approved for implementation yet** — they exist so the work is ready. Two carry a named cross-plan seam: the `canjson.ErrInvalidShape` decision in the decode-error-surface plan settles which sentinel the fidelity plan's `Real`-precision task reuses.
