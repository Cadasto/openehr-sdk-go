...
Five plans from a consumer→SDK gap survey plus the review-round follow-ups that were living only in session notes. All amend shipped REQs in place (normative deltas ride in the implementing PR) **except** the RM-function stubs, which propose new ids. The `ehr.Create` empty-2xx typing plan has since **landed and was archived** (below); the remaining four are **not approved for implementation yet** — they exist so the work is ready. Two of those carry a named cross-plan seam: the `canjson.ErrInvalidShape` decision in the decode-error-surface plan settles which sentinel the fidelity plan's `Real`-precision task reuses.
