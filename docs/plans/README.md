...
| Plan | Scope | Covers | Probe |
|---|---|---|---|
| [2026-08-18-probe-runnability.md](2026-08-18-probe-runnability.md) | Give the existing probe catalog (72 entries) the execution modes REQ-082 mandates: one shared result type + a runner (phase 1), a real `sandbox/` in-memory backend (phase 2), then Cassette recording/replay (phase 3 — recorder, replayer, the `cmd/probe-record` capture harness and the `ehr-create` / `ehr-lifecycle` recordings landed; coverage beyond those two remains) and Live mode (phase 4 — wired, pre-release; runs need a reachable CDR) | REQ-082 (**`partial`**) | none new — PROBE-078 / 079 promoted out of `Status: Deferred` and PROBE-065 out of `Status: Draft`, all now Implemented; PROBE-077 still Deferred |
