...
| Plan | Scope | Covers | Probe |
|---|---|---|---|
| [2026-08-18-probe-runnability.md](2026-08-18-probe-runnability.md) | Give the existing probe catalog (72 entries) the execution modes REQ-082 mandates: one shared result type + a runner (phase 1), a real `sandbox/` in-memory backend (phase 2), then Cassette recording/replay (phase 3 — recorder, replayer and the first corpus file landed; coverage beyond the vendored `POST /ehr` remains) and Live mode (phase 4 — wired, pre-release; runs need a reachable CDR) | REQ-082 (**`partial`**) | none new — promotes PROBE-077 / 078 / 079 out of `Status: Deferred` and unblocks PROBE-065 (`Status: Draft`) |
