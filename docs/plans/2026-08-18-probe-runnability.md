...
**Probes:** no new `PROBE-NNN` — this plan gives the existing catalog its missing execution modes. It promotes **PROBE-065**, **PROBE-077**, **PROBE-078**, **PROBE-079** out of `Deferred`, and is the gate [STRAND-09](../specifications/research-strands.md#strand-09--its-rest-conformance-follow-ups) item 1 names for four `testkit/probes/rest/*` probes.
...
Four probes are deferred with near-identical wording — *"landed and unit-covered; the dedicated wire-level probe deferred to a follow-up cycle"* — and STRAND-09 defers four more explicitly *"until the sandbox/cassette probe runner (REQ-082) is the gate."* Eight probes and a strand are queued behind one unwritten package. REQ-082 is also one of the three `v1.0.0` gates in [releases.md](../releases.md#v100-gate).
...
**Definition of done:** every probe reports through the shared type; `make probe-status` reflects real per-mode state rather than file existence; `make ci` green.
...
**Tasks:** endpoint and credential configuration; the mutating-probe opt-in gate; per-run resource scoping; promote PROBE-065 / 077 / 078 / 079 and the STRAND-09 REST probes out of `Deferred`.
