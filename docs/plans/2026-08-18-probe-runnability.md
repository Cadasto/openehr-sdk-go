...
| Phase 4 — Live mode (local CDRs) | partial — runner Live path + `TestLiveCreateEHR` + `TestLiveCoreSnapshot` (core EHR create/get/exists/status green against **EHRbase 2.35.1**; the System capabilities probe *skips* — EHRbase does not implement the spec's `OPTIONS /` conformance operation, an accepted deployment deviation) + `TestLiveCompositionSnapshot` (template upload, composition save and read-back green — the SDK's canonical encoding accepted by a real CDR): `OPENEHR_LIVE_*` names the target, `OPENEHR_LIVE_ALLOW_MUTATING` is the separate write opt-in REQ-082 requires; both unset in CI, so the tests skip without dialing |
