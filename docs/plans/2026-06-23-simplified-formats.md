...
- **`.schema`-suffixed media types on input** — the oldest item on the list, and a deliberate non-goal so far: the codec emits and accepts the two canonical *Simplified Formats* strings only (§ Decisions above), so EHRbase's `.schema` variants are refused rather than tolerated. Liberal-on-input acceptance needs a consumer asking for it.
