...
**Probes:** PROBE-100 (proposed — *Upstream AQL admissibility corpus ratchet*, in-repo). PROBE-021's Cassette/Live ratification stays pending a reference deployment — unchanged by this plan, recorded here because the audit filed it beside this finding
...
- Code and tests land with `// REQ-160` / `// PROBE-100` citations;
  [`traceability.yaml`](../specifications/traceability.yaml) REQ-160 test list grows.
...
| Step | Status |
|---|---|
| Phase 0 — PROBE-100 entry + Vendored fixtures § + REQ-160 evidence sentence | |
| Phase 1 — ingest script + vendored corpus + provenance pin + exclusion list | |
| Phase 2 — CSV oracle + ratchet test | |
| Phase 3 — CI wiring + close-out | |
| Index `spec-check` misses (roadmap note) | |
