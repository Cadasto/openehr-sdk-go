...
3. **Phase 4 — `assign[T]` covers 7 of the 10 sites.** The 10 coercion blocks verified at the exact cited lines, but `write.go:372/776/818` (Instruction.activities, ItemList.items, ItemTable.rows) are multiple-cardinality attrs that `append` rather than scalar-assign; they need `coerceValueOrPtr[T]` used directly, or `assign` redesigned to take a `set func(T)` closure (as `writeDVTemporalValueSingle` already does) to cover assign + append uniformly. Stays reflection-free (REQ-024); per-attr error specificity is preserved via the threaded `attr`/`rmName`.
4. **Wording / counts (cosmetic).** `objectIDValue` (`audit.go:132-179`) is a 12-`case` switch (6 subtypes × value + pointer), not "six-arm"; the new `rm.ObjectIDValue` returns `(string, bool)` vs `rm.UIDValue`'s bare `string` — a deliberate signature asymmetry worth a doc note. `nodeIDOf` / `nameValueOf` in `openehr/rm/rmpath/walk.go` are 36 arms each, not 34.

These are accuracy fixes to apply per-phase at implementation time; none change the plan's structure, sequencing, or the reality of the smells it targets.
