...
1. **Fail-loud, never silent** (REQ-053): an unrecognised `_`-segment is `ErrUnknownPath`; a recognised family with an unrecognised suffix is `ErrUnsupportedDatatype` naming the offending FLAT key; encode never drops a populated in-scope value — it emits, rides `|raw` where legal, or returns a typed error. The single out-of-scope surface is the ADR 0015 composer projection (`external_ref` / `identifiers` dropped for `ctx/composer_name`), carved out in wire.md § REQ-140 and registered in `deviations.md`. No new error sentinels without updating `simplified.go`'s taxonomy godoc.
...
**Families landed here:** `_uid` (any LOCATABLE the Web Template models, incl. root — narrowed 2026-08-06, PR #89 review), `_link:N` (`|meaning`, `|type`, `|target` ↔ `rm.Link{Meaning DvText, Type DvText, Target DvEhrUri}`), `_work_flow_id` + `_guideline_id` (ENTRY, OBJECT_REF), `context/_end_time`, `context/_location`.
