...
- RESOLVED in the final fix wave. The generated decode helper (`openehr/rm/typereg/streaming.go`) now memoises the caller-hook join per (aggregate, caller) pointer, so a deep decode under a caller's `WithUnmarshalers` joins once rather than once per nesting level, and it treats `WithUnmarshalers(nil)` as no caller. A white-box counter (`callerJoinCount`) pins that the join does not grow with decode depth.
