...
**Defers:** a general per-node decode-time presence map for `mappings` at arbitrary depth (Phase 2 delivers the boundary case + the documented model limitation, not a whole-tree presence API); any change to `omitempty` on `mappings` or `data`; terminology validation of `TERM_MAPPING.target`; any caller's own skip-on-decode-failure policy.
...
> **Review rider (2026-09-03):** the test above is quoted as first landed. Review found its header claim false — `[]` decodes to a *non-nil* empty slice, `null` and absent to nil — so it now lives as `TestDVTextMappingsDecodePresenceAndEncodeCollapse`, asserting decoded nilness per case as well as the re-encode collapse.

- [x] **Step 3: Record the limitation in the spec** — `docs/specifications/wire.md` § REQ-052

Add a note: decode preserves the `[]` / `null` / absent distinction (`[]` decodes to a non-nil empty slice; `null` and absent both decode to nil), but re-encode does not — `omitempty` re-drops the key for all three, so the collapse is on the wire form, not on decode. A caller needing the wire-level distinction reads JSON-key presence at the decode boundary, the shape REQ-112 uses for `EHR_STATUS.subject` (`ValidateRMEHRStatusBytes`). `omitempty` is intentionally unchanged: emitting `[]` would be RM-invalid (`Mappings_valid`), emitting `null` a spelling nothing needs.
...
- [x] **Step 3: Record the deviation** — one line in `docs/specifications/wire.md` § REQ-052 (or canjson `doc.go`): `DV_MULTIMEDIA.data` / `integrity_check` (`[]byte`) encode as base64 (correct canonical spelling) but carry the same re-encode collapse as `mappings` — decode preserves `[]`/`null`/absent, only `omitempty` on re-encode drops it; not fixed — `size` carries the unencoded byte count, so present-but-empty inline data is not a distinction the SDK needs to preserve.
