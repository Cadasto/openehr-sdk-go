...
- [x] **Step 1:** Grep the SDK for `canjson.ErrInvalidShape` consumers. No consumer + no near-term need → lean toward (B). A consumer classifying shape errors → (A). *(SDK grep: only canjson's own tests and doc prose — no behavioural consumer. Decided (A) on the queued fidelity-plan need.)*
