...
| Template-less RM validation floor (REQ-112) | **Landed** | `openehr/validation/`, `openehr/validation/rmread/` | SDK-GAP-15 — `ValidateRM` + typed sugars `ValidateRMFolder` / `EHRStatus` / `EHRAccess` / `Demographic` walk any RM root with `rminfo` as sole driver (no template required); RM-mandatory absences + per-type invariant catalogue (CODE_PHRASE terminology+code, DV_QUANTITY units, DV_INTERVAL `lower ≤ upper`, OBJECT_REF type/namespace). PROBE-077 deferred. [archive](plans/archive/2026-06-29-sdk-gap-15-rm-floor-validation.md) |
