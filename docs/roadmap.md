...
| Canonical JSON | **Landed** | `openehr/serialize/canjson/` REQ-052 | PROBE-030/031/038; REQ-052/040 narrow polymorphic decode (`<Parent>Like` interfaces) [archived plan](plans/archive/2026-05-26-rm-polymorphic-decode-coverage.md); REQ-040/052/102/107 polymorphic `_type` encode/decode round-trip stability — value-in-interface `_type` on encode (`internal/jsonpoly`) + `DV_INTERVAL<T>` bounds-backed validation, corpus round-trips byte-stable ([archived plan](plans/archive/2026-06-23-polymorphic-encode-decode.md)) |
| Canonical XML | **Landed** | `openehr/serialize/canxml/` REQ-056 | PROBE-033/034; traceability indexed |
| FLAT / STRUCTURED | **Landed** | `openehr/serialize/simplified/` REQ-053, REQ-140 | Bidirectional FLAT + STRUCTURED codecs + OPT-free FLAT↔STRUCTURED interconversion, driven by the REQ-106 Web Template (`Marshal`/`Unmarshal{Flat,Structured}`). Full DV_* datatype set (first-class + `\|raw` for decorated/exotic), `ctx/` context, `\|other` open value-sets, strict fail-loud decode. `Unmarshal*(…, WithTemplate(compiled))` repopulates `LOCATABLE.name` + completes the RM-mandatory attributes FLAT omits → decoded composition **validates against the OPT** (PROBE-076 asserts idempotence + conformance over the EHRbase `Test_dv_*` corpus). Standalone `CODE_PHRASE` leaves (ENTRY `language`/`encoding`), the optional `DV_ORDERED`/`QUANTIFIED`/`AMOUNT` suffixes (`|magnitude_status`, `|normal_status`, `|accuracy`, `|precision`, …), and the reference's real-path metadata spellings on decode ([ADR 0015](adr/0015-flat-metadata-spelling.md)) landed 2026-08-03 via the PROBE-086 coverage ratchet ([plan](plans/archive/2026-08-03-flat-coverage-ratchet.md)). The REQ-053 residual deferrals closed 2026-08-05 ([plan](plans/archive/2026-08-05-flat-rm-attributes.md)): the DV_TEXT substitution carve-out and `ctx/setting` emission, then the `DV_MULTIMEDIA` / `DV_PARSABLE` / `DV_INTERVAL<T>` and ENTRY-`subject` leaf closures — all four riding **REQ-140**, the full underscore-prefixed RM attribute grammar (`_uid`, `_link:N`, `_feeder_audit`, the party families, the value decorations, the DataValue members), recursive and RM-type-keyed, both directions and byte-exact (**PROBE-089**), which took PROBE-086 coverage to **80.4%** of the upstream corpus (1466 of its 1824 keys, from 19.7% at REQ-140's own baseline; 19.5% measured across both PRs of the plan). Deferred (deviations.md), all refused loudly: `.schema` media types on input, ACTIVITY `action_archetype_id` (`STRING`), reused-sibling FLAT (the REQ-116 residual), and the named underscore refusals — `_instruction_details`, `_wf_definition`, FEEDER_AUDIT_DETAILS `other_details` and PARTICIPATION `time`. The [ADR 0015](adr/0015-flat-metadata-spelling.md) composer boundary is one of two exceptions to "refused loudly", both encode-side: decode refuses it, and on encode *exactly one* of its shapes is projected rather than refused — a **named** PARTY_IDENTIFIED keeps the name and drops its `external_ref` / `identifiers` — a registered projection loss, not a refusal. The second is the `_uid` / `_link:N` / `_feeder_audit` of a **folded structural wrapper** (`ITEM_TREE`, `HISTORY`, a lifted `EVENT`, …), which FLAT has no key for at all: dropped on encode, and closable only by a reference-side spelling, not a codec change. Upstream byte-conformance landed as **PROBE-086** (REQ-080). [plan](plans/archive/2026-07-14-flat-structured-codecs.md) |
| WebTemplate JSON export | **Landed** | `openehr/template/webtemplate/` REQ-106 | `Build`/`Marshal(*templatecompile.Compiled)` → EHRbase `openEHR_SDK` v2.3 WebTemplate JSON; ELEMENT-collapse + structural-node drop + inContext leaves + reference-locked lower-snake ids; core-datatype inputs. Structural + input-suffix/type parity against **three** vendored references (PROBE-075): `constrain_test` 104/104, and since REQ-116 the two reuse oracles `Corona_Anamnese` 230/230 and `GECCO_Diagnose` 34/34; deviations in package `deviations.md`. Archetype-reuse-under-slot **landed** via REQ-116 (name-derived `id`). [ADR 0014](adr/0014-webtemplate-reference-implementation-lock.md), [plan](plans/archive/2026-05-22-webtemplate-export.md) |
| Template-level node naming | **Landed** | `openehr/template/`, `internal/templatecompile/`, `openehr/template/webtemplate/` REQ-116 | The fixed `C_STRING` an OPT pins on a node's `name` is parsed and exposed (`NodeName()`), carried onto `CompiledNode`, emitted as the reference's `[archetype_id,'Name']` predicate on **both** path builders, and preferred for the WebTemplate `id` — so a template reusing one archetype among siblings is addressable and exportable at last. Paths through nodes pinning no name are byte-identical to pre-change output (snapshot-asserted over all 58 vendored OPTs). Unblocks PROBE-086. [plan](plans/archive/2026-07-29-template-node-naming.md) |
...
| RM behavioural functions (REQ-120–123) | **Landed** | `openehr/rm/`, `openehr/rm/rmpath/` | Identifier parsing/derivation + `Parse*` (REQ-120), `VERSION.is_branch` (REQ-122), temporal `DV_*` read/inspect/compare/convert (REQ-123) as hand-written methods; locatable path read access `ItemAtPath`/`ItemsAtPath`/`PathExists`/`PathUnique` (REQ-121) as package functions over an own reflection-free walker. `bmmgen` manual-implementation skip ([ADR 0002 D7](adr/0002-bmm-codegen-decisions.md)); surface & fallibility [ADR 0011](adr/0011-rm-behavioural-functions-surface.md). Arithmetic / `parent` / `path_of_item` / VERSIONED_OBJECT container ops deferred. [plan](plans/archive/2026-06-19-rm-functions.md) |
...
| Template-less RM validation floor (REQ-112) | **Landed** | `openehr/validation/`, `openehr/validation/rmread/` | `ValidateRM` + typed sugars `ValidateRMFolder` / `EHRStatus` / `EHRAccess` / `Demographic` walk any RM root with `rminfo` as sole driver (no template required); RM-mandatory absences + per-type invariant catalogue (CODE_PHRASE terminology+code, DV_QUANTITY units, DV_INTERVAL `lower ≤ upper`, OBJECT_REF type/namespace). PROBE-077 deferred. [archive](plans/archive/2026-06-29-rm-floor-validation.md). `ValidateRMEHRStatusBytes` detects the value-typed mandatory `subject` (`rm.PartySelf`) via JSON-key presence (PROBE-081). |
...
| Parsed AQL AST + round-trip emitter (REQ-113) | **Landed** | `openehr/aql/parse/`, `openehr/aql/` | Tier-2 `parse.Query` structured AST (read-side mirror of `aql.Builder`); shared `WhereExpr` / `Value` vocabulary across read and write (`Comparison` / `Junction` / `NotExpr` / `ExistsExpr` / `MatchesExpr` / `LikeExpr` / `Param/String/Int/Real/Bool/NullValue`); `(*Query).Emit` + `aql.FormatWhere` close the round-trip loop (PROBE-080 inline, ~100 cases in `roundtrip_test.go`). Out-of-catalogue shapes surface `aql.ErrIncompleteAST` at parse and refuse on `Emit`. Tier-1 `Document.Tree` interim accessor marked `// Deprecated:`. [archive](plans/archive/2026-06-29-aql-execution-ast.md). `ClassExpr.PredicateComparison` (standing predicate) + `aql.Comparison.ParsedPath` expose parsed structure — a WHERE path carries alias+segments, a class standing predicate carries segments with an empty alias (PR #74); `aql.IdentifiedPath`/`PathSegment` moved into `openehr/aql` (PROBE-082). The v1 catalogue gaps this row's `ErrIncompleteAST` guarded are closed by REQ-117 (next row). |
| AQL expression-catalogue completion (REQ-117) | **Landed** | `openehr/aql/`, `openehr/aql/parse/`, `openehr/aql/lint/` | The structured AST covers the whole SDK grammar profile — literal + mixed-star SELECT items, function calls on either side of a comparison with parameter / primitive / nested-call arguments, path-valued operands, structured `MATCHES TERMINOLOGY(…)` / `{uri}`, and containment junctions at the FROM root (PROBE-087); the lone residual `ErrIncompleteAST` is an integer literal the AST cannot represent (`LIMIT`/`OFFSET` beyond Go `int`, or an INTEGER primitive beyond `int64` in a value position). The lint gate stops over-rejecting an `ORDER BY` key that names a SELECT `AS` alias and a bare boolean comparison operand (no new codes). The builder gains the containment algebra — `Class`, `Contains`/`NotContains`, `ContainsAnd`/`ContainsOr` with precedence-driven parentheses — plus opt-in in-text `LimitInline`/`OffsetInline`, mutually exclusive with the envelope channel (PROBE-088; the PROBE-020 golden is byte-identical). **Deferred:** a builder entry point for a FROM-root junction. [plan](plans/archive/2026-08-04-aql-expressivity-completion.md) |
| AQL `SELECT TOP` carrier + literal source text (REQ-118) | **Landed** | `openehr/aql/`, `openehr/aql/parse/`, `openehr/aql/lint/` | The deprecated `SELECT TOP n [FORWARD\|BACKWARD]` clause (openEHR QUERY Release-1.1.0 §4.4.3 — deprecated in favour of `LIMIT` + `ORDER BY`, removal announced) is now carried on both sides: `parse.SelectClause.Top` reads it with its direction and `Builder.Top` / `Builder.TopDirected` write it, so a query the SDK did not author can be read, re-emitted, and built. It leaves **one** residual `ErrIncompleteAST` — a numeric literal the AST cannot represent, which an out-of-range `TOP` count now falls under (PROBE-087). `parse.LiteralExpr.Raw` adds a projected literal's source text as written, which the openEHR result schema needs as an unaliased column name (`1.50`, `"dq"`, `001` all differ from their canonical rendering); emission stays canonical. §4.4.3 forbids `TOP` with `LIMIT`: the parser reports both as written, the lint gate diagnoses it (`aql_deprecated_top` Warning, `aql_top_with_limit` Error — the first two new lint codes since REQ-109), and `Build()` refuses to construct it (PROBE-088). No grammar change; `TOP $n` stays unmodelled. [plan](plans/archive/2026-08-05-aql-top-carrier-literal-source-text.md) |
| Re-parseable canonical AQL emission (REQ-119) | **Landed** | `openehr/aql/`, `openehr/aql/parse/`, `openehr/aql/lint/` | The builder guarantee PROBE-021 pins ("the typed builders cannot emit syntactically invalid AQL") and REQ-117's fixed point were stated over clause SHAPES; nothing bound the VALUE positions inside them, and the round-trip corpus is written in canonical form so it never carried a value needing an escape. Every validating write path (`Builder.Build`, `FormatWhere`, `(*parse.Query).Emit`) now applies the value guards in **every** position it renders, SELECT included: grammar `ESCAPE_SEQ` string escaping (invalid UTF-8 bytes as `OCTAL_ESC`, surrogate pairs combined on decode), a real's mandatory fractional part with infinities/NaN refused, `math.MinInt64` accepted, scientific notation read, a positional `MATCHES {uri}` guard (pct-encoding, one `#`, bracketed IPv6, and the `TERM_CODE` shadowing that spellability alone misses), `TERMINOLOGY` arity, and function names checked against declaration order — so `true`/`false` stay admissible while the aggregates are value-position-only. `aql.ValidateValue` exports the check for write paths outside the package; `aql.EqualValues` gives the vocabulary a panic-free comparison across value AND pointer shapes (`==` panics since v0.18.0, invisibly to the compiler). `Emit` also gained REQ-117's containment refusals. **Wire-visible:** a literal carrying `'` or `\` and a whole-valued real (`2` → `2.0`) emit differently; the string's prior spelling was unparseable and the real's re-parsed as a different value (PROBE-090). Predicate and path text is now re-emitted **verbatim** rather than whitespace-collapsed — the collapsed form was not re-parseable wherever the predicate needed a keyword separator — which is visible on the read side too, so a consumer comparing that text normalises through `aql.StripPredicateTrivia`. Implementation-aligned fix-up of the residuals left open on PR #87; the class-predicate splice (#99) carries [its own plan](plans/archive/2026-08-08-aql-class-predicate-splice-and-source-text.md). |
| OPT → RM instance synthesis | **Landed** | `openehr/instance/` REQ-107 | `Generate(ctx, c, opts)` + closed-root accessors + `internal/templateinstance/rmwrite/`; PROBE-027 on `vital_signs.opt` + `clinical_note.opt` + the REQ-107 real-world corpus (`Referral Request.v1` / `Demonstration.v1` / `social`); `Options.UIDSource` test-determinism seam; canjson-polymorphic `Composition.uid`; REQ-107 seeded synthetic value fill (`ValueFill`/`ValueSource`, [archived plan](plans/archive/2026-06-23-seeded-synthetic-generation.md)). [archive](plans/archive/2026-05-24-template-instance-example-generator.md) + [wire-parser archive](plans/archive/2026-05-26-c-primitive-object-wire-parser.md). **Open:** `medium`/`detail_level` structural level (representative optional subset) ([dossier § Deferred](plans/archive/2026-06-23-seeded-synthetic-generation.md)) |
...
REST delivery detail: [2026-05-15-rest-api-client.md](plans/archive/2026-05-15-rest-api-client.md) (archived; this roadmap reflects the tree). Demographic follow-up: [2026-06-14-demographic-rest-client.md](plans/archive/2026-06-14-demographic-rest-client.md). ITS-REST conformance remediation (audit-details header grammar, System `OPTIONS`, Admin/Definition paths, status mapping, `WithLifecycleState`/`WithGET`/`WithPath`, versioned stored-query PUT): [2026-06-19-its-rest-conformance-remediation.md](plans/archive/2026-06-19-its-rest-conformance-remediation.md) — follow-ups in [STRAND-09](specifications/research-strands.md#strand-09--its-rest-conformance-follow-ups) (REST probes; stored-query `fetch` omission). Larger Tier-3 gaps (dedicated ITEM_TAG endpoints, VERSIONED_* read families, content negotiation) remain deferred to named follow-ups.
...
| Cadasto admin | **Partial** | `cadasto/admin/` | Health probes (`Live`, `Ready`) landed per REQ-083; tenant/env/system-info planned. Distinct from ITS Admin client. |
...
| Versioned-write probes | **Landed** | `testkit/probes/versioned/` | PROBE-010–013; PROBE-071 (REQ-094 representation writes) |
| Validation probes | **Landed** | `testkit/probes/validation/` | PROBE-025/026 (REQ-102) |
| Instance synthesis probe | **Landed** | `testkit/probes/instance/` | PROBE-027 (REQ-104/107) on `vital_signs.opt` + `clinical_note.opt` + real-world corpus (`Referral Request.v1` / `Demonstration.v1` / `social`) |
...
| openEHR conformance ratification | **Partial** | `testkit/conformance/webtemplate/` | REQ-080, REQ-082 — first slice landed 2026-08-01: **PROBE-086** round-trips the pinned upstream EHRbase FLAT corpus (34 bodies this SDK did not write) through the REQ-053 codec, exact on the modelled subset — 1466 of the corpus's 1824 keys (80.4%) after the REQ-140 underscore grammar landed 2026-08-05, up from 356 (19.5%) after the 2026-08-03 ratchet and 192 (10.5%) on landing — and counting the rest (52 refusals, none of them an underscore family the grammar carries). The per-fixture excluded/compared ratchet is pinned in the harness's own tests; the probe adds a zero-coverage floor on top (census in [SKIPPED.md](../testkit/conformance/webtemplate/SKIPPED.md)). Ratification against a live CDR, and the Cassette/Live modes REQ-082 mandates, remain open |
