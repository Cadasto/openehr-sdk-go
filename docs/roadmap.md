...
| WebTemplate JSON export | **Landed** | `openehr/template/webtemplate/` REQ-106 | `Build`/`Marshal(*templatecompile.Compiled)` → EHRbase `openEHR_SDK` v2.3 WebTemplate JSON; ELEMENT-collapse + structural-node drop + inContext leaves + reference-locked lower-snake ids; core-datatype inputs. Structural + input-suffix/type parity 104/104 vs vendored `constrain_test` (PROBE-075); deviations in package `deviations.md`. Archetype-reuse-under-slot deferred. [ADR 0014](adr/0014-webtemplate-reference-implementation-lock.md), [plan](plans/archive/2026-05-22-webtemplate-export.md) |
