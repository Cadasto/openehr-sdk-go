...
| Canonical JSON | **Landed** | `openehr/serialize/canjson/` REQ-052 | REQ-052 is **partial** overall — [§ Floating-point precision](specifications/wire.md#floating-point-precision)'s decode-side precision-loss arm is now closed (`Real.UnmarshalJSON` refuses a literal past 17 significant decimal digits, wrapping `canjson.ErrInvalidShape`, [plan](plans/archive/2026-09-01-rm-canonical-json-fidelity.md)); what keeps the REQ partial is the § field-order clause, deferred to an official openEHR canonical-JSON specification not yet published — the codec itself is landed. PROBE-030/031/038. Narrow polymorphic decode via `<Parent>Like` interfaces; `_type` encode/decode round-trips byte-stable, with `DV_INTERVAL<T>` bounds validation ([plan](plans/archive/2026-06-23-polymorphic-encode-decode.md)) |
