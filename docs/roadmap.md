...
| Parsed AQL AST + round-trip emitter (REQ-113) | **Landed** | `openehr/aql/parse/`, `openehr/aql/` | Tier-2 `parse.Query` structured AST (read-side mirror of `aql.Builder`); shared `WhereExpr` / `Value` vocabulary across read and write (`Comparison` / `Junction` / `NotExpr` / `ExistsExpr` / `MatchesExpr` / `LikeExpr` / `Param/String/Int/Real/Bool/NullValue`); `(*Query).Emit` + `aql.FormatWhere` close the round-trip loop (PROBE-080 inline, ~52 cases in `roundtrip_test.go`). Out-of-catalogue shapes surface `aql.ErrIncompleteAST` at parse and refuse on `Emit`. Tier-1 `Document.Tree` interim accessor marked `// Deprecated:`. [archive](plans/archive/2026-06-29-aql-execution-ast.md). `ClassExpr.PredicateComparison` (standing predicate) + `aql.Comparison.ParsedPath` expose parsed structure — a WHERE path carries alias+segments, a class standing predicate carries segments with an empty alias (PR #74); `aql.IdentifiedPath`/`PathSegment` moved into `openehr/aql` (PROBE-082). |
