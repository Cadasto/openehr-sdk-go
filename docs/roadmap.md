...
| Canonical JSON | **Landed** | `openehr/serialize/canjson/` REQ-052 | REQ-052 is **landed** — [§ Floating-point precision](specifications/wire.md#floating-point-precision)'s decode-side precision-loss arm is now closed for literals past 17 significant decimal digits (`Real.UnmarshalJSON` refuses them, wrapping `canjson.ErrInvalidShape`; a shorter, binary-inexact literal such as `0.1` or an integer past 2^53 is still accepted unreported, by design — [plan](plans/archive/2026-09-01-rm-canonical-json-fidelity.md)); the field-order clause that last kept the REQ partial was reworded on 2026-09-03 into the SDK's own output contract — deterministic encode profile, order-agnostic decode, no order asserted for other implementations — closing the last open clause. PROBE-030/031/038. Narrow polymorphic decode via `<Parent>Like` interfaces; `_type` encode/decode round-trips byte-stable, with `DV_INTERVAL<T>` bounds validation ([plan](plans/archive/2026-06-23-polymorphic-encode-decode.md)) |
...
| RM class-universe absence reasons | **Landed** | `openehr/rm/rminfo/` REQ-049 | The **negative space** behind that universe: why a name is *not* in it — undeclared, excluded package, excluded class, primitive, enumeration — as a closed `AbsenceReason` on an optional `AbsenceReporter`, under a precedence that puts what a name *is* ahead of why it was *skipped*. Compiled-in only, backed by a generated table beside `lookup_gen.go`; PROBE-098 checks it against an independent reduction of the pinned BMM ([spec](specifications/bmm-conformance.md#req-049--rm-class-universe-absence-reasons), [plan](plans/archive/2026-08-26-rminfo-absence-reason.md)) |
...
| Template-less RM validation floor | **Landed** | `openehr/validation/`, `openehr/validation/rmread/` REQ-112 | `ValidateRM` and typed sugars walk any RM root with `rminfo` as sole driver — no template required — checking RM-mandatory absences and a per-type invariant catalogue, now including `TERM_MAPPING.match`'s value set and `DV_TEXT.mappings`'s `Mappings_valid` ([plan](plans/archive/2026-09-01-rm-canonical-json-fidelity.md)). PROBE-077 deferred ([plan](plans/archive/2026-06-29-rm-floor-validation.md)) |
...
| AQL containment admissibility relation | **Landed** | `openehr/aql/contain/` REQ-160 | `Default().CanContain` / `Containable` / `ArchetypeMatches` answer Admissible / Never / ByReference / UnknownClass from the pinned BMM's by-value graph plus a cited overlay table (EHR root, `VERSION` tier, `FOLDER→COMPOSITION` by reference), extensible per deployment via `WithOverlay`; its evidence base runs under CI rather than being cited — the vendored upstream FROM/CONTAINS corpus, where no engine-accepted shape may verdict Never ([ADR 0017](adr/0017-aql-semantic-layer.md); PROBE-097, PROBE-100) |
| AQL semantic and portability lint | **Landed** | `openehr/aql/lint/` REQ-161 | Additive REQ-109 Layer-2 checks on the REQ-160 relation (impossible containment, non-containable target, archetype/class mismatch, unknown class, by-reference hop) plus three spec-gap portability advisories (`VERSION` predicate default, unreferenced `VERSIONED_OBJECT`, conservative fan-out); `lint.Options` gains a relation field for dialect overlays, surfaced at the validation seam as `ValidateAQLWithTypeRelation` ([ADR 0017](adr/0017-aql-semantic-layer.md); PROBE-097) |
| AQL builder containment verification | **Landed** | `openehr/aql/` REQ-162 | `(*Builder).VerifyContainment` walks the builder's own FROM root and containment algebra, opt-in, reporting the REQ-161 containment codes at read/write parity with the linter; the verification never runs inside `Build()` and changes no emitted byte — REQ-163 below gives `Build()` a `SELECT` verification of its own ([ADR 0017](adr/0017-aql-semantic-layer.md); PROBE-097) |
| AQL write-side expressivity parity | **Landed** | `openehr/aql/` REQ-163 | The builder spells the class- and projection-position vocabularies the read side already modelled: a sealed `VERSION` predicate (`LATEST_VERSION` / `ALL_VERSIONS` / a comparison) via `aql.Version`, one standing comparison in class position via `Containment.Predicated`, and a typed projection (`DISTINCT`, `AS`, aggregates, function calls, literals, star). `Build()` now verifies the `SELECT` it emitted, which is what bounds `Col`'s verbatim splicing. **Wire-visible:** a program using none of the new API emits the same bytes (PROBE-020's golden is unchanged), but a `Col` that splits the projection, smuggles a clause-level flag or a `TOP` direction the builder never recorded, or leaves a lexical region open where the clause ends is now refused, as is a typed construction the position's grammar does not admit — semver-minor on REQ-119's footing — and `aql.SelectField` is no longer usefully `==`-comparable, `==` compiling but panicking at run time on a call-shaped field ([spec](specifications/clinical-modeling.md#req-163--aql-write-side-expressivity-parity), [plan](plans/archive/2026-08-26-aql-write-side-parity.md); PROBE-088 goldens, PROBE-097 arm (c)) |
| AQL path-shape and paging lint | **Landed** | `openehr/aql/lint/` REQ-164 | A third additive REQ-109 Layer-2 group, five Warning codes over the query text plus the pinned BMM — no OPT, no CDR: an identified path stepping through an unpredicated multi-valued RM attribute (`aql_path_repeating_unpredicated`, in `SELECT`, `WHERE` and `ORDER BY` alike), a row-bounded query with no `ORDER BY` (`aql_paging_no_order_by`, in-text `LIMIT` and the request envelope both), an unaliased projection item (`aql_select_no_alias`), two projected paths on one alias descending into different repeating scopes (`aql_fanout_path_grain`, the path source of SPECQUERY-9 row multiplicity beside REQ-161's junction source), and a containment step the REQ-160 relation proves inert (`aql_contains_redundant_step`, on the new `TypeRelation.Unavoidable`). The segment walk stops silently wherever the pin cannot type a step. **Visible to CI consumers:** unchanged queries gain new Warnings — none flips `Result.OK()`, and the per-code filter is the opt-out ([spec](specifications/clinical-modeling.md#req-164--aql-path-shape-and-paging-lint), [plan](plans/archive/2026-08-26-aql-path-shape-lint.md); PROBE-099, with PROBE-028 deliberately re-baselined) |
...
| Typed 2xx decode failure | **Landed** | `transport/`, `openehr/client/*` REQ-151 | A 2xx body that will not decode as the requested representation fails with `*transport.DecodeError` — the raw bytes, the method and the route template, wrapping the codec's own error, with a value-free `Error()` and bytes carried by default ([ADR 0018](adr/0018-raw-bytes-on-decode-error.md)). A non-2xx stays `WireError`, an empty representation body stays `ErrInvalidShape`, and the REQ-094 write-result funnel keeps `NoRepresentationError`; PROBE-101 ([plan](plans/archive/2026-08-30-read-path-decode-taxonomy.md)) |
...
| Contribution | **Landed** | `openehr/client/ehr/contribution/` | **Write:** submission body is `Contribution_create`, response the persisted `rm.Contribution`; the commit audit drops server-assigned `time_committed` and keeps a `DV_CODED_TEXT` `change_type` (PROBE-072). **Read (REQ-142):** `Get` issues `GET /ehr/{ehr_id}/contribution/{contribution_uid}`; empty ids fail closed before any request and 404 maps to `ErrNotFound` (PROBE-092). **Authoring (REQ-130):** `Builder` + `Creation`/`Amendment`/`Modification`/`Deletion` assemble the batch — per-operation change-type codes, body-carried `lifecycle_state`, an undeclared batch code refused, and the server-assigned `contribution`/`uid` omitted (PROBE-084) |
| ItemTags | **Partial** | `openehr/client/ehr/itemtags/` | REQ-059 is **partial** overall (PROBE-062 deferred); header codec + composition/ehrstatus/directory GET and composition PUT are landed |
...
| Definition — ADL 1.4 templates | **Landed** | `openehr/client/definition/` | Upload/list/get/delete + example composition. List filters `template_id`/`concept`/`version`/`offset`/`fetch` (REQ-143, PROBE-093); explicit zero paging is sent, negative refused. **Metadata decode (REQ-144):** `created_timestamp` accepts a closed tolerant layout set (ADR 0019) and an empty 2xx body yields a non-nil empty slice |
| Definition — stored AQL | **Landed** | `openehr/client/definition/` | Put/get/list/delete; REQ-057 `PutStoredQuery` recovers `{name, version}` from the `Location` header on a body-less reply. REQ-144 covers `saved` decode and the non-nil empty list on the same terms. PROBE-079 deferred |
