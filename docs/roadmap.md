...
| RM behavioural functions (REQ-120–123) | **Landed** | `openehr/rm/`, `openehr/rm/rmpath/` | Identifier parsing/derivation + `Parse*` (REQ-120), `VERSION.is_branch` (REQ-122), temporal `DV_*` read/inspect/compare/convert (REQ-123) as hand-written methods; locatable path read access `ItemAtPath`/`ItemsAtPath`/`PathExists`/`PathUnique` (REQ-121) as package functions over an own reflection-free walker. `bmmgen` manual-implementation skip ([ADR 0002 D7](adr/0002-bmm-codegen-decisions.md)); surface & fallibility [ADR 0011](adr/0011-rm-behavioural-functions-surface.md). Arithmetic / `parent` / `path_of_item` / VERSIONED_OBJECT container ops deferred. [plan](plans/2026-06-19-rm-functions.md) |
