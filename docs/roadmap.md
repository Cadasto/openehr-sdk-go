...
| Canonical JSON | **Landed** | `openehr/serialize/canjson/` REQ-052 | REQ-052 is **partial** overall — the decode-side precision-loss arm of [§ Floating-point precision](specifications/wire.md#floating-point-precision) is open; the codec itself is landed. PROBE-030/031/038. Narrow polymorphic decode via `<Parent>Like` interfaces; `_type` encode/decode round-trips byte-stable, with `DV_INTERVAL<T>` bounds validation ([plan](plans/archive/2026-06-23-polymorphic-encode-decode.md)) |
