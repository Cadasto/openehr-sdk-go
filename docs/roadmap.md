...
| FLAT / STRUCTURED | **Partial** | `openehr/serialize/simplified/` REQ-053 | Bidirectional FLAT + STRUCTURED codecs + OPT-free FLAT↔STRUCTURED interconversion, driven by the REQ-106 Web Template (`Marshal`/`Unmarshal{Flat,Structured}`). Landed: full DV_* datatype set (first-class + `\|raw` for decorated/exotic), `ctx/` context, `\|other` open value-sets, strict fail-loud decode; PROBE-076 round-trips the EHRbase `Test_dv_*` corpus (24 pass). Deferred (deviations.md): `LOCATABLE.name` on decode, remaining `ctx/` fields, first-class `_`-attr suffixes, `.schema` media types, upstream-conformance probe. [plan](plans/archive/2026-07-14-flat-structured-codecs.md) |
