...
| AQL containment admissibility relation | **Partial** | `openehr/aql/contain/` REQ-160 | `Default().CanContain` / `Containable` / `ArchetypeMatches` answer Admissible / Never / ByReference / UnknownClass from the pinned BMM's by-value graph plus a cited overlay table (EHR root, `VERSION` tier, `FOLDER→COMPOSITION` by reference), extensible per deployment via `WithOverlay`. Phase 1 of [ADR 0017](adr/0017-aql-semantic-layer.md); **Partial** until the REQ-161 semantic lint consumes it and arms PROBE-097 ([plan](plans/2026-08-21-aql-semantic-layer.md)) |
