...
| Canonical JSON | **Landed** | `openehr/serialize/canjson/` REQ-052 | PROBE-030/031/038; SDK-GAP-11 narrow polymorphic decode (`<Parent>Like` interfaces) [archived plan](plans/archive/2026-05-26-rm-polymorphic-decode-coverage.md); SDK-GAP-13 polymorphic `_type` encode/decode round-trip stability — value-in-interface `_type` on encode (`internal/jsonpoly`) + `DV_INTERVAL<T>` bounds-backed validation, corpus round-trips byte-stable ([archived plan](plans/archive/2026-06-23-sdk-gap-13-polymorphic-encode-decode.md)) |
...
| Template-less RM validation floor (REQ-112) | **Landed** | `openehr/validation/`, `openehr/validation/rmread/` | SDK-GAP-15 — `ValidateRM` + typed sugars `ValidateRMFolder` / `EHRStatus` / `EHRAccess` / `Demographic` walk any RM root with `rminfo` as sole driver (no template required); RM-mandatory absences + per-type invariant catalogue (CODE_PHRASE terminology+code, DV_QUANTITY units, DV_INTERVAL `lower ≤ upper`, OBJECT_REF type/namespace). PROBE-077 deferred. [archive](plans/archive/2026-06-29-sdk-gap-15-rm-floor-validation.md). SDK-GAP-18 — `ValidateRMEHRStatusBytes` detects the value-typed mandatory `subject` (`rm.PartySelf`) via JSON-key presence (PROBE-081). |
...
| Parsed AQL AST + round-trip emitter (REQ-113) | **Landed** | `openehr/aql/parse/`, `openehr/aql/` | SDK-GAP-17 — Tier-2 `parse.Query` structured AST (read-side mirror of `aql.Builder`); shared `WhereExpr` / `Value` vocabulary across read and write (`Comparison` / `Junction` / `NotExpr` / `ExistsExpr` / `MatchesExpr` / `LikeExpr` / `Param/String/Int/Real/Bool/NullValue`); `(*Query).Emit` + `aql.FormatWhere` close the round-trip loop (PROBE-080 inline, ~52 cases in `roundtrip_test.go`). Out-of-catalogue shapes surface `aql.ErrIncompleteAST` at parse and refuse on `Emit`. Tier-1 `Document.Tree` interim accessor marked `// Deprecated:`. [archive](plans/archive/2026-06-29-sdk-gap-17-aql-execution-ast.md). SDK-GAP-19 — `ClassExpr.PredicateComparison` (standing predicate) + `aql.Comparison.ParsedPath` (WHERE path alias+segments) expose parsed structure; `aql.IdentifiedPath`/`PathSegment` moved into `openehr/aql` (PROBE-082). |
| OPT → RM instance synthesis | **Landed** | `openehr/instance/` REQ-107 | `Generate(ctx, c, opts)` + closed-root accessors + `internal/templateinstance/rmwrite/`; PROBE-027 on `vital_signs.opt` + `clinical_note.opt` + the SDK-GAP-12 real-world corpus (`Referral Request.v1` / `Demonstration.v1` / `social`); `Options.UIDSource` test-determinism seam; canjson-polymorphic `Composition.uid`; SDK-GAP-14 seeded synthetic value fill (`ValueFill`/`ValueSource`, [archived plan](plans/archive/2026-06-23-sdk-gap-14-seeded-synthetic-generation.md)). [archive](plans/archive/2026-05-24-template-instance-example-generator.md) + [wire-parser archive](plans/archive/2026-05-26-c-primitive-object-wire-parser.md). **Open:** SDK-GAP-14 follow-up — `medium`/`detail_level` structural level (representative optional subset) ([dossier § Deferred](plans/archive/2026-06-23-sdk-gap-14-seeded-synthetic-generation.md)) |
...
| Query (AQL execute) | **Landed** | `openehr/client/query/` | Ad-hoc + stored execute; REQ-055; SDK-GAP-16 — POST execution scopes via the `openehr-ehr-id` request header (verb-aware: GET keeps `ehr_id` query parameter), [archive](plans/archive/2026-06-29-sdk-gap-16-stored-query-rest-conformance.md) |
| Definition — ADL 1.4 templates | **Landed** | `openehr/client/definition/` | Upload/list/get/delete, example composition |
| Definition — stored AQL | **Landed** | `openehr/client/definition/` | Put/get/list/delete; REQ-057; SDK-GAP-16 — `PutStoredQuery` recovers the assigned `{name, version}` from the `Location` response header on a body-less store reply (body-decode + synthesised-metadata fallbacks unchanged), [archive](plans/archive/2026-06-29-sdk-gap-16-stored-query-rest-conformance.md) |
