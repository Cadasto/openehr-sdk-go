...
| FLAT / STRUCTURED | **Landed** | `openehr/serialize/simplified/` REQ-053 | Bidirectional FLAT + STRUCTURED codecs + OPT-free FLAT↔STRUCTURED interconversion, driven by the REQ-106 Web Template (`Marshal`/`Unmarshal{Flat,Structured}`). Full DV_* datatype set (first-class + `\|raw` for decorated/exotic), `ctx/` context, `\|other` open value-sets, strict fail-loud decode. `Unmarshal*(…, WithTemplate(compiled))` repopulates `LOCATABLE.name` + completes the RM-mandatory attributes FLAT omits → decoded composition **validates against the OPT** (PROBE-076 asserts idempotence + conformance over the EHRbase `Test_dv_*` corpus). Standalone `CODE_PHRASE` leaves (ENTRY `language`/`encoding`), the optional `DV_ORDERED`/`QUANTIFIED`/`AMOUNT` suffixes (`|magnitude_status`, `|normal_status`, `|accuracy`, `|precision`, …), and the reference's real-path metadata spellings on decode ([ADR 0015](adr/0015-flat-metadata-spelling.md)) landed 2026-08-03 via the PROBE-086 coverage ratchet ([plan](plans/archive/2026-08-03-flat-coverage-ratchet.md)). The DV_TEXT subtype substitution (a fully-captured `DV_CODED_TEXT` at a `DV_TEXT` leaf) and `ctx/setting` emission landed 2026-08-05 (REQ-053 amended; [plan](plans/2026-08-05-flat-rm-attributes.md) Phases A/B). Deferred (deviations.md): the `ctx/` short forms for workflow ids and the composer `external_ref`, and `.schema` media types; the EVENT_CONTEXT optionals (participations, facility, `end_time`, `location`) ride the REQ-140 underscore grammar instead ([ADR 0016](adr/0016-event-context-optionals-underscore-spelling.md)). Upstream byte-conformance landed as **PROBE-086** (REQ-080). [plan](plans/archive/2026-07-14-flat-structured-codecs.md) |
