...
| `Prefer` negotiation | **Landed** | `transport/`, `openehr/client/ehr/*` REQ-094 | All three write-path modes: `return=representation` bare-body decode (PROBE-061/071), `return=identifier` slot population, and a 2xx representation with an empty or undecodable body → `NoRepresentationError` (wrapping `ErrInvalidShape`). PROBE-065 round-trip deferred |
