...
| Packaging | [`../docs/specifications/packaging.md`](../docs/specifications/packaging.md) |
...
The package taxonomy, listing every package with its scope notes, is **normative in [module-layout.md § Package taxonomy](../docs/specifications/module-layout.md#package-taxonomy)**. This section gives the overall shape and does not copy the tree.

The module resolves into three concentric layers, each usable without the one above it:

1. **Building blocks (no I/O).** `openehr/rm` (+ `typereg`, `rminfo`), `openehr/terminology` (the pinned openEHR vocabulary, stdlib-only, below `openehr/rm`), `openehr/bmm`, `openehr/aom`, `openehr/serialize`, `openehr/template`, `openehr/validation`, `openehr/instance`, `openehr/composition`, and `openehr/aql` (query models, builders, and opt-in containment verification). These are pure data structures and algorithms. They decode a canonical JSON Composition, validate it against an OPT, build one from a template, render an AQL string, or check a containment chain against the pinned RM. They never use the network and never import `transport/` or `auth/`, so a CI validator or a synthetic-data faker can depend on exactly one of them.
2. **The HTTP path.** `auth/` produces a `TokenSource`. `transport/` wraps the caller's injected `*http.Client` and adds auth, retry, OpenTelemetry, and the ITS-REST envelope. `openehr/client/*` are the thin, typed leaf clients (System, EHR + sub-resources, Query, Definition, Admin). They call through `transport/` and decode with the codecs. `smart/discovery` supplies the `ServiceCatalog` that tells the transport where the openEHR REST base URL lives.
...
Imports flow strictly downward, with no upward or cyclic imports. [module-layout.md § Dependency direction](../docs/specifications/module-layout.md#dependency-direction) defines this normatively, with the per-edge invariants. The same graph as a diagram:
...
**Life of a REST call.** A consumer constructs a `ServiceCatalog` (via `smart/discovery` or by hand), passes it plus an `*http.Client` to `transport.New`, and calls a leaf method such as `ehr.Create(ctx, client)`. The leaf builds the request URL from the catalog and hands it to `transport.Do`. `transport.Do` attaches the bearer token from the `TokenSource`, starts an OTel span, applies the retry policy, and sends the request through the injected client. The canonical codecs decode the response envelope into typed `openehr/rm` structs, which the leaf returns with version metadata. Dependencies only ever point downward. Nothing in a building block imports a client, and nothing in `openehr/`, `auth/`, `smart/`, or `transport/` imports `cadasto/`.
...
|---|---|---|
| `go.opentelemetry.io/otel`, `/trace` | `transport/` only | Distributed tracing. Pulls logr, xxhash transitively. |
| `github.com/antlr4-go/antlr/v4` | `openehr/aql/parse` only | AQL parser runtime: pure Go, no transitive deps (ADR [0007](adr/0007-aql-antlr-grammar-profile.md)). The generator (Java) is containerised, never on the build/test path. |
| `golang.org/x/oauth2` | `auth/smart` (+ probes) | RFC 7636 PKCE `code_verifier`/`code_challenge` generation (ADR [0009](adr/0009-smart-auth-library-scope.md)). Token sources/refresh/errors are the SDK's own `auth` types. |
...
- **Provide a `ServiceCatalog`.** There is no "base URL" parameter. Use `smart/discovery` for SMART-discovering backends, or build a static catalog by hand for a fixed deployment (EHRbase, a local CDR).
...
- **`context.Context` is the first argument** on every I/O method. Cancellation, deadlines, and per-request auth all travel on it.
- **Building-block-only integration.** Import `openehr/rm` + `serialize`/`validation`/`template`/`aql` with no transport or auth at all. This suits CI gates, seeders, and FHIR-mapping prototypes.
...
- **The `cadasto/` cut line** keeps open the option of moving the Cadasto-platform extras into a sibling Go module later (an open question). The cut is held now whatever that question's answer, because reversing it after v1 ships is expensive.
- **The building-block boundary** means `openehr/rm`, `serialize`, `validation`, `template`, and `aql` (with its `parse`, `lint`, and `contain` blocks) must work *without* `transport/` or `auth/`. CI validators, FHIR-mapping prototypes, and AQL linters don't need HTTP, and the SDK must not force the dependency on them.

The first cut keeps a future module split possible. The second makes the SDK easier to consume today.
...
| RM structural lookup | [`openehr/rm/rminfo/`](../openehr/rm/rminfo/) | Two BMM-derived tables emitted by one run: `lookup_gen.go` (the class universe) and `absence_gen.go` (the declared names outside it); [ADR 0005](adr/0005-compiled-template-foundation.md) |
...
The SDK uses standard Go module versioning. The module path is locked at `github.com/cadasto/openehr-sdk-go`, with `v2`+ under `…/v2/` per Go's semantic-import-versioning convention. Policy, bump rules, and the `v1.0.0` gate: [`releases.md`](releases.md) and [`module-layout.md` § Versioning](../docs/specifications/module-layout.md#versioning).

## Open decisions

Open decisions are tracked in [`research-strands.md`](../docs/specifications/research-strands.md), and resolutions become ADRs under [`adr/`](adr/). Don't settle a strand in code. Raise it or draft an ADR first.
