...
- **Modes:** Sandbox.
- **Status:** Implemented (Sandbox) — [`testkit/probes/transport/probe_101_decode_failure_surfaced.go`](../../testkit/probes/transport/probe_101_decode_failure_surfaced.go); harness in [`probes_test.go`](../../testkit/probes/transport/probes_test.go). Every arm is additionally held to exactly one request, so a leaf that refused the call before it reached the wire cannot credit the probe with a failure the server never provoked. Four planted backends, each breaking exactly one arm and each **matched against the failure detail it must produce**, pin that the arms still bite — including the silent-zero case a real leaf can still be driven into: a 200 body that decodes into a zero-valued resource, which is what a swallowed decode error looks like to a caller. Plan: [`docs/plans/2026-08-30-read-path-decode-taxonomy.md`](../plans/2026-08-30-read-path-decode-taxonomy.md).
- **Satisfies:** REQ-151.
