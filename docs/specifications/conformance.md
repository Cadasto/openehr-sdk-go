...
- **Wire assertion:** In-repo property — for each catalogue shape, `ParseQuery` MUST return a structurally faithful AST (pinned per shape) and `Emit` MUST be a fixed point; the only remaining `ErrIncompleteAST` condition is the unrepresentable-integer guard (a `LIMIT`/`OFFSET` beyond Go `int`, or an INTEGER primitive beyond `int64` in a value position), which MUST still fire.
- **Modes:** In-repo (unit-level property; no backend).
