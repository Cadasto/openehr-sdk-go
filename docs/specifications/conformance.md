...
Discovery for PROBE-030 / PROBE-033 walks `compositions/` and `rm/` via [`fixtures.ListCompositionJSON`](../../testkit/fixtures/discover.go) and [`fixtures.ListRMXML`](../../testkit/fixtures/discover.go). Composition XML on disk that the canxml round trip does not exercise **MAY** be listed in `compositionXMLExcluded`, and `rm/` JSON samples that are an alternate wire shape (`rmJSONExcluded`) or a deliberately invalid API-validation payload (`rmJSONExcludedPrefixes`) in that package, so probes stay green while the files remain available for template and validation work.
