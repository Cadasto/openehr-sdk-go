...
- **Preconditions:** A commit-pinned EHRbase `openEHR_SDK` `constrain_test` OPT + reference WebTemplate JSON, vendored under [`testkit/cassettes/`](../../testkit/cassettes/) with provenance + Apache-2.0 attribution. (`constrain_test` is used rather than `corona_anamnese` because the latter's WebTemplate cannot be built yet: several of its sibling archetype roots reuse one archetype id, so this builder derives the same web `id` for each and `webtemplate.Build` refuses to emit ambiguous duplicates — the open REQ-106 gap recorded in [`deviations.md`](../../openehr/template/webtemplate/deviations.md) § Sibling `id` disambiguation and [ADR 0014](../adr/0014-webtemplate-reference-implementation-lock.md). The reference resolves it by deriving `id` from the OPT's template-level node name and adding a name predicate to `aqlPath`, neither of which this SDK parses yet; `constrain_test` needs neither, which is why parity holds at 104/104. `corona_anamnese` *does* now compile under `templatecompile`; the earlier archetype-reuse-under-a-slot reading of that failure was incorrect — the cause was shared-path subtrees, fixed for both `corona_anamnese` and the PROBE-086 conformance OPT.)
