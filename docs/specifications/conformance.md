...
- **Modes:** Sandbox, Cassette.
- **Status:** Implemented (Sandbox) — [`testkit/probes/versioned/probe_065_minimal_return_round_trip.go`](../../testkit/probes/versioned/probe_065_minimal_return_round_trip.go); harness in [`probes_test.go`](../../testkit/probes/versioned/probes_test.go). The default `Save` (Prefer `return=minimal`) recovers the `Location` into the `VersionUID` and returns a nil Composition; a follow-up `Get` returns the full body. Can-fail plants strip the `Location` and 404 the read-back; the Cassette arm follows a recording.
- **Satisfies:** REQ-094.
...
- **Status:** Implemented (Sandbox) — [`testkit/probes/query/probe_078_verb_aware_scoping.go`](../../testkit/probes/query/probe_078_verb_aware_scoping.go); harness in [`probes_test.go`](../../testkit/probes/query/probes_test.go). Drives both endpoints (`/query/aql` and `/query/{qualified_query_name}`) over both verbs, asserting each request scopes on the verb-appropriate channel and carries no other; four can-fail plants move the scope onto the wrong channel per verb. REQ-055 verb-aware scoping is also unit-covered ([`openehr/client/query/query_test.go`](../../openehr/client/query/query_test.go)). Live is not scoped: EHRbase does not restrict a top-level `FROM EHR e` by the `openehr-ehr-id` header, so the header/query-parameter distinction is witnessed by request capture (Sandbox), not by a Live result delta.
- **Satisfies:** REQ-055.
