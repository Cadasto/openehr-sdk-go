...
- **Status:** Draft — unblocked, adapter not yet written. The corpus, its pin, and its sync/verify tooling are landed; the corpus OPT compiles (`templatecompile` admits shared-path subtrees) and, as of [REQ-116](clinical-modeling.md#req-116--template-level-node-naming-and-name-predicated-paths) Phase 4, `webtemplate.Build` succeeds for it — its two ACTION ELEMENTs that both sanitise to `dv_text` now take the reference's ordinal fallback (`dv_text`, `dv_text2`), the spelling the upstream FLAT bodies themselves use. Precise mechanism and the four layers involved: [`deviations.md`](../../openehr/template/webtemplate/deviations.md) § Sibling `id` disambiguation. What remains is this probe's own adapter and runner — [the FLAT conformance plan](../plans/2026-07-16-web-template-tests-conformance.md) Phase 1, no longer blocked. It stays Draft until that test lands. (The status force stays unbolded so `spec-check`'s Draft-probe guard can see it.)
