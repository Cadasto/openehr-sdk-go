...
- **Title:** For a vendored upstream (OPT + canonical COMPOSITION) pair, the simplified codecs round-trip the composition without losing the data the formats carry: FLAT is idempotent (`MarshalFlat → UnmarshalFlat → MarshalFlat`), STRUCTURED re-encodes to the same FLAT (`MarshalStructured → UnmarshalStructured → MarshalFlat`), and OPT-free interconversion preserves it (`FlatToStructured → StructuredToFlat` decodes+re-encodes to the same FLAT).
...
- **Wire assertion:** Not backend-facing — an in-repo round-trip **idempotence** property across the datatype corpus (DV_TEXT/CODED_TEXT/DATE_TIME/QUANTITY/COUNT/BOOLEAN/ORDINAL/PROPORTION/DURATION/IDENTIFIER/URI + the `|raw`-carried MULTIMEDIA/PARSABLE/INTERVAL). **Scope limit:** it asserts the codec is self-consistent and lossless across its own pipeline; it does **not** compare emitted FLAT/STRUCTURED to vendored simplified output, nor the decoded composition to the vendored canonical (a symmetric omission would pass). A true upstream-conformance probe (comparing to vendored simplified fixtures / canonical with an explicit exclusion list) is a documented follow-up — see [`deviations.md`](../../openehr/serialize/simplified/deviations.md) § Conformance. A template the WebTemplate builder cannot yet model is a documented skip, never a fail.
