...
- **Title:** A path parameter containing `/`, `\`, `.`, `..`, or a control character is rejected by the transport and produces no HTTP request, on every path-interpolating leaf.
...
- **Wire assertion:** For each leaf that interpolates a path parameter, at least one traversal input (`a/../../definition/query/evil`, `..`, a backslash, a control character) returns an error satisfying `errors.Is(..., transport.ErrInvalidPathSegment)` and `errors.Is(..., transport.ErrInvalidConfig)`, and the captured request count is zero. A well-formed id that only needs percent-encoding (a space in a template id) still issues exactly one request whose path is encoded once (REQ-095).
- **Modes:** Sandbox.
...
- **Wire assertion:** The captured request method and path match the ITS-REST template; a 200 decodes to the contribution type with metadata populated; empty ids issue no request (`ErrInvalidConfig`); a 404 maps to `ErrNotFound`.
- **Modes:** Sandbox.
- **Status:** Draft — defined ahead of implementation; see [2026-08-18-contribution-get-and-template-list-filters](../plans/2026-08-18-contribution-get-and-template-list-filters.md).
- **Satisfies:** REQ-142.
...
- **Preconditions:** A sandbox Definition server on `GET /definition/template/{adl1.4|adl2}`.
