...
- **Wire assertion:** In-repo property — for every value in the corpus, `ParseQuery(emit(v))` MUST succeed and MUST recover a value equal to `v` under `aql.EqualValues`, in the SELECT position as well as the WHERE one; a value the guards refuse MUST NOT round-trip intact. For a literal read from source text the property MUST hold TWICE (parse → emit → parse recovers the same value), which is what an arbitrary byte and a surrogate pair break and mere parseability does not catch. The two grammar-derived guards MUST agree with the grammar: for every token name in the lexer — plus the keyword spellings carried only in lexer `fragment`s, where the first reserved list was wrong — the builder emits a function of that name if and only if the parser accepts it; for every URI in the corpus, the builder emits the operand if and only if it re-parses as one `MATCHES` on the same URI. The `{uri}` case is asserted through round-trip IDENTITY, not parseability, because an injected operand yields text that parses cleanly as a DIFFERENT query. The three validating write paths (`aql.FormatWhere`, `aql.Builder.Build`, `(*parse.Query).Emit`) MUST agree on the same input, and a tree carrying a POINTER-shaped node MUST emit byte-identically to the value-shaped one — a dropped precedence parenthesis re-parses and re-emits to itself, so only that identity separates it from the tree the caller built. Every guard MUST be mutation-detectable: removing it MUST fail a named test, and a guard that could be TIGHTENED into refusing valid AQL MUST have a positive control that fails when it is.
- **Modes:** In-repo (unit-level property; no backend).
- **Status:** Implemented (inline) — in [`openehr/aql/parse/literal_roundtrip_test.go`](../../openehr/aql/parse/literal_roundtrip_test.go): `TestStringLiteralEscapesRoundTrip`, `TestStringLiteralEmitsGrammarEscapes`, `TestParsedStringEscapesDecode`, `TestStringRoundTripIsAFixedPoint`, `TestInvalidUTF8StringSurvivesEmission`, `TestRealLiteralStaysReal`, `TestNonFiniteRealRefused`, `TestNegativeZeroRealKeepsItsSign`, `TestScientificNotationExtraction`, `TestIntLiteralRangeBoundaries`, `TestDoubleUnaryMinusIsAccepted`, `TestFuncCallRoundTripsToAnEqualValue`, `TestEveryValueKindRoundTripsToAnEqualValue`, `TestReservedFuncNamesTrackTheGrammar`; in [`openehr/aql/parse/emit_parity_test.go`](../../openehr/aql/parse/emit_parity_test.go): `TestMatchesURIGuardTracksTheGrammar`, `TestEmitRefusesMisplacedJunction`, `TestEmitRefusesIncompleteContainment`, `TestEmitValidatesSelectValuePositions`, `TestEmitAcceptsAnAliaslessContainment`, `TestFormatValueDoesNotPanicOnAValueWithNoWireForm`, `TestFormatWhereDoesNotPanicOnItsOwnRefusalPath`, `TestFormatWhereDoesNotPanicOnATypedNilPredicate`, `TestJunctionParenthesisationBindsThePointerTwin`, `TestBuildDoesNotPanicOnATypedNilPredicate`, `TestEmitValidatesPagingAndOrderPositions`, `TestEmitTreatsAVersionClassAsAClassNode`, `TestEmitSelectBindsThePointerTwin`, `TestEmitRefusesAnIncompleteNodeInsideARootJunction`, `TestContainmentRefusalsReportTheRuleThatFired`; the write-side refusals, the pointer shapes, the narrow operand slots, the parameter spelling, and `EqualValues` in [`openehr/aql/value_test.go`](../../openehr/aql/value_test.go) (`TestParameterNamesMustSpellThePARAMETERToken`, `TestNarrowOperandSlotsRefuseAWiderValue`, `TestTerminologyGuardTrimsTheName`, `TestJunctionValidatesItsOperatorAndArity`, `TestFormatValueDoesNotPanicOnATypedNilArgument`).
- **Satisfies:** REQ-119.
