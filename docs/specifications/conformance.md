...
- **Preconditions:** A commit-pinned EHRbase `openEHR_SDK` `corona_anamnese` OPT + reference WebTemplate JSON, vendored under [`testkit/cassettes/`](../../testkit/cassettes/) with provenance + Apache-2.0 attribution.
...
- **Status:** Planned — REQ-106 not yet implemented; if the reference-fixture fetch is blocked, PROBE-075 is deferred and the export is anchored to SDK-generated goldens until the fixture is supplied (per [ADR 0014](../adr/0014-webtemplate-reference-implementation-lock.md)).
- **Satisfies:** REQ-106.
