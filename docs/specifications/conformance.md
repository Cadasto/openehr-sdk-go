...
**AQL conformance corpus** (`testkit/cassettes/aql/conformance/`). A vendored upstream corpus rather than a curated fixture set: the FROM-family combination data of the EHRbase Robot integration tests (`ehrbase/integration-tests`, `tests/robot/_resources/test_data_sets/aql/fields_and_results/from/combinations/`), each row a FROM/CONTAINS shape the engine accepted and answered, and the input [PROBE-100](#probe-100--upstream-aql-admissibility-corpus-ratchet) asserts against. It **MUST** carry its own provenance pin at `testkit/cassettes/aql/conformance/AQL_SOURCE.txt`, in the field format `testkit/cassettes/ROBOT_SOURCE.txt` already uses (source repo, source path, commit, commit date, recorded date, source tree, licence) — a separate pin because the corpus is vendored by its own ingest at its own cadence, not by the composition-fixture ingest that writes `ROBOT_SOURCE.txt`. The licence is the upstream's existing entry in `testkit/cassettes/THIRD_PARTY_LICENSES.md` — the same repository the composition fixtures come from — so the corpus adds no new licence obligation. The exclusion list `testkit/cassettes/aql/conformance/EXCLUDED.txt` **MUST** be generated by that ingest and committed beside the corpus, naming every upstream file the corpus does not carry and why, so what "covered" means is inspectable rather than asserted.
