...
- **Wire assertion:** In-repo property — for every value in the corpus, `ParseQuery(emit(v))` MUST succeed and MUST recover a value equal to `v`; a value the guards refuse MUST NOT round-trip intact. The two grammar-derived guards MUST agree with the grammar: for every token name in the lexer, the builder emits a function of that name if and only if the parser accepts it; for every URI in the corpus, the builder emits the operand if and only if it re-parses as one `MATCHES` on the same URI. The `{uri}` case is asserted through round-trip IDENTITY, not parseability, because an injected operand yields text that parses cleanly as a DIFFERENT query.
- **Modes:** In-repo (unit-level property; no backend).
- **Status:** Implemented (inline) — `TestStringLiteralEscapesRoundTrip`, `TestParsedStringEscapesDecode`, `TestRealLiteralStaysReal`, `TestIntLiteralRangeBoundaries`, `TestReservedFuncNamesTrackTheGrammar` in [`openehr/aql/parse/literal_roundtrip_test.go`](../../openehr/aql/parse/literal_roundtrip_test.go); `TestMatchesURIGuardTracksTheGrammar` and `TestEmitRefusesMisplacedJunction` in [`openehr/aql/parse/emit_parity_test.go`](../../openehr/aql/parse/emit_parity_test.go); the write-side refusals and `EqualValues` in [`openehr/aql/value_test.go`](../../openehr/aql/value_test.go).
- **Satisfies:** REQ-119.
