...
- **Modes:** Sandbox.
- **Status:** Implemented (Sandbox) — see [`testkit/probes/instance/probe_027_generated_validates.go`](../../testkit/probes/instance/probe_027_generated_validates.go). Probe runs against both `vital_signs.opt` and `clinical_note.opt` for `Minimal` and `Example` policies. Slot fills draw a conforming archetype id from the parsed REQ-104 include grammar when a safe example can be synthesized, falling back to `openEHR-EHR-<RMType>.example.v1` (the validator's RM-type-prefix `slotFitsArchetypeID` rule) only when the OPT carried no parseable includes.
- **Satisfies:** REQ-107.
