...
- **Title:** Every bracketed path predicate the vendored grammar admits at a path-segment position arrives as a typed, trivia-free, delimiter-free structured value beside its untouched verbatim text, and the same form yields the same kind whether it sits at the top of the bracket or as a junction operand.
- **Preconditions:** The `pathPredicate` alternatives — `standardPredicate`, `archetypePredicate`, `nodePredicate` — read out of the vendored `resources/aql/grammar/active/AqlParser.g4`, from which the corpus is **generated**: a hand-written case list is the "list a maintainer must remember to update" [clinical-modeling.md § REQ-113 § Acceptance](clinical-modeling.md#req-113--execution-oriented-parsed-aql-ast) forbids, and the failure it forbids is a grammar alternative silently uncovered.
- **Wire assertion:** In-repo property. For every generated predicate spelling: the structured carrier MUST be populated with the kind the grammar alternative names, its components MUST equal the values with delimiters removed, escapes resolved and trivia absent, and the verbatim text MUST be byte-identical to the source. Each form MUST be presented in BOTH positions it can occupy — at the top of a bracket and nested as an `AND` / `OR` junction operand — and MUST yield the SAME kind in both, which is the only mechanical check on the rule that the kind is a property of the form rather than of the parse-tree node that carried it. Trivia and escape independence MUST be asserted as a property over the whole corpus rather than as named cases: a spelling padded with whitespace and a `--` comment MUST produce components equal to the bare spelling's WHILE the verbatim text differs, and a corpus in which those two assertions cannot both fire is not exercising the REQ. The name slot MUST be exercised in all five spellings the grammar admits (`STRING`, `PARAMETER`, `TERM_CODE`, `AT_CODE`, `ID_CODE`) on BOTH alternatives that carry it, with the parameter spelling distinguishable from a resolvable name — a name deferred to bind time is not a name. The sweep MUST descend into the `pathPredicateOperand` production the comparison alternatives reach — its members decide structured-versus-enumerated-unstructured, and a sweep stopping at the top-level alternatives is how the operand forms went unenumerated in the first draft — and MUST fail when the grammar admits a form neither a kind nor the enumerated unstructured set covers. Each ENUMERATED unstructured form (REQ-113 § Structured node predicates names them) MUST have a corpus row asserting nil-beside-populated-verbatim at BOTH positions, since a junction containing a hole must not ship a partial structure. Negative space: a form the SDK does not structure MUST report itself unstructured with its verbatim text intact, and MUST NOT carry a partial structure; the class position MUST be unchanged, keeping its landed REQ-113 carriers and gaining no parallel one. Emission MUST be unaffected — the REQ-119 round-trip suites stay green and emitted text byte-identical, asserted rather than assumed, since a read-side model that changed a written byte would be a REQ-119 regression this probe is positioned to catch first.
- **Modes:** In-repo (unit-level property; no backend).
- **Status:** Implemented (inline) — [`openehr/aql/parse/predicate_structure_test.go`](../../openehr/aql/parse/predicate_structure_test.go). `TestEveryGrammarAlternativeIsCovered` reads every alternative of the three predicate productions AND `pathPredicateOperand` out of the vendored `AqlParser.g4` and fails when one has no corpus row, with `TestTheGrammarSweepCanFail` as its able-to-fail control (the operand descent was added by the 2026-08-19 branch review, which found the top-level sweep passing while three operand forms went unenumerated); `TestTheSameFormYieldsTheSameKindInBothPositions` is the form-not-parse-node rule; `TestTriviaAndEscapesDoNotReachTheComponents` asserts trivia independence as a property with the differing verbatim text as the second, must-also-fire arm; `TestEveryNameSpellingIsDiscriminated` covers all five name spellings on both carrying alternatives, decomposing the term code; `TestBothExtractorsAgreeOnTheStructure` keeps the two path extraction sites in step; and `TestEmissionIsUnaffected` asserts the REQ-119 fixed point rather than assuming it.
- **Satisfies:** REQ-113.
...
- **Preconditions:** A corpus reaching every **reachable** drop class — as of [clinical-modeling.md § REQ-119 § Amends REQ-117](clinical-modeling.md#req-119--re-parseable-canonical-aql-emission) that is a numeric literal outside the value vocabulary, in each clause that admits one — plus a representative lint-issue set spanning the layers `lint` runs, and, for each corpus query, the **value spans** of its input identified independently of the diagnostic under test.
- **Wire assertion:** In-repo property, three arms. **(a) Record per drop:** every extractor site that sets the incomplete flag MUST record a kind; this MUST be held by a sweep derived from the SOURCE — so a site added later is covered the day it lands — and NOT by an enumerated list, per [clinical-modeling.md § REQ-113 § The kind set enumerates CLASSES](clinical-modeling.md#req-113--execution-oriented-parsed-aql-ast). The sweep MUST be shown able to fail: a site carrying no kind MUST be reported. The accessor MUST trigger extraction itself, so a document on which nothing called `Query()` MUST NOT read as "nothing dropped"; it MUST return a copy, asserted by mutating the result and re-reading; and its zero kind MUST be the unclassified member rather than the first real one. The defensive arms — unreachable while the catalogue and the grammar profile agree — MUST have their unreachability by a legal query asserted at the unit level rather than corpus-covered, since no corpus can reach them and a probe demanding one would be weakened instead of satisfied. **(b) Value-free fields:** for every corpus query, no value-free field of either surface may contain any substring of that input's value spans — asserted against the INPUT, never against expected strings, because a test comparing to an expectation passes a record that echoed a value the author happened to expect. The classification itself is [clinical-modeling.md § REQ-113 § The two field classes](clinical-modeling.md#req-113--execution-oriented-parsed-aql-ast)'s and is not restated here; what this probe carries is that the value-bearing fields are NOT asserted clean — an arm that scrubbed them too would pass a build that had quietly made `Detail` structural and would hide the contract's whole point. **(c) Span attribution:** each span MUST resolve to the offending construct rather than to its enclosing clause, and MUST be expressed in the landed position vocabulary — one span type across both packages, so this arm is a comparison and not a conversion. An unattributable span MUST be zero rather than approximated. Additivity MUST be asserted: existing message texts, codes, severities, sentinels and `Result.OK()` outcomes MUST be unchanged over the corpus.
- **Modes:** In-repo (unit-level property; no backend).
- **Status:** Implemented (inline) — [`openehr/aql/parse/dropped_test.go`](../../openehr/aql/parse/dropped_test.go) (arm (a): the reachable-drop corpus, the accessor's four properties, the source-derived kind sweep and its able-to-fail control, the defensive-arm unreachability claim; arm (c): the span sliced back out of the source) and [`openehr/aql/lint/span_test.go`](../../openehr/aql/lint/span_test.go) (arm (b) on the lint surface, the alias span, the zero-width syntax span agreeing with `Detail`'s line:col, and the one-span-type rule held by a direct cross-package comparison). Arm (b)'s value-free assertion runs against the input's value spans on both surfaces, with the value-bearing fields deliberately left unasserted.
