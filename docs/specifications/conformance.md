...
- Replay matching **MUST** be on a normalised request key — method, the path with the deployment's REST base stripped, and the headers and body fields the probe's assertion depends on — **not** byte-exact equality: a capture necessarily carries timestamps, generated UUIDs, and `ETag` values that differ on every run, and byte-exact matching would make every recording single-use. Browser-oriented HAR fields (`timings`, `cache`, `pageref`) are outside that key, and a replayer **MUST** ignore them. While no recording's probe assertion depends on a header or a body field, the key **MUST** be method and stripped path alone; two exchanges that differ only in headers or body then match the same entry, in recorded order.
...
- The runner **MUST** refuse to execute mutating probes in Live mode without an explicit per-invocation opt-in. Writing to a deployment **MUST** be something the operator asked for, never a default that a mode flag turned on.
  The repository's Live tests take their target and the mutating opt-in from environment variables; CI sets neither.
...
Discovery for PROBE-030 / PROBE-033 walks `compositions/` and `rm/` via [`fixtures.ListCompositionJSON`](../../testkit/fixtures/discover.go) and [`fixtures.ListRMXML`](../../testkit/fixtures/discover.go). Composition XML on disk that the canxml round trip does not exercise **MAY** be listed in `compositionXMLExcluded`, and an `rm/` JSON sample that is an alternate wire shape or a deliberately invalid API-validation payload **MAY** be listed in `rmJSONExcluded` or `rmJSONExcludedPrefixes` respectively, all in that package, so probes stay green while the files remain available for template and validation work.

**AQL conformance corpus** ([`testkit/cassettes/aql/conformance/`](../../testkit/cassettes/aql/conformance/)). A vendored upstream corpus rather than a curated fixture set: the FROM-family combination data of the EHRbase Robot integration tests (`ehrbase/integration-tests`, `tests/robot/_resources/test_data_sets/aql/fields_and_results/from/combinations/`), each row a FROM/CONTAINS shape the engine accepted and answered, and the input [PROBE-100](#probe-100--upstream-aql-admissibility-corpus-ratchet) asserts against. It **MUST** carry its own provenance pin at [`testkit/cassettes/aql/conformance/AQL_SOURCE.txt`](../../testkit/cassettes/aql/conformance/AQL_SOURCE.txt), in the field format [`testkit/cassettes/ROBOT_SOURCE.txt`](../../testkit/cassettes/ROBOT_SOURCE.txt) already uses (source repo, source path, commit, commit date, recorded date, source tree, licence) — a separate pin because the corpus is vendored by its own ingest ([`scripts/ingest-robot-aql.sh`](../../scripts/ingest-robot-aql.sh)) at its own cadence, not by the composition-fixture ingest that writes `ROBOT_SOURCE.txt`. The licence is the upstream's existing entry in [`testkit/cassettes/THIRD_PARTY_LICENSES.md`](../../testkit/cassettes/THIRD_PARTY_LICENSES.md) — the same repository the composition fixtures come from — so the corpus adds no new licence obligation. The exclusion list [`testkit/cassettes/aql/conformance/EXCLUDED.txt`](../../testkit/cassettes/aql/conformance/EXCLUDED.txt) **MUST** be generated by that ingest and committed beside the corpus, naming every upstream file the corpus does not carry and why, so what "covered" means is inspectable rather than asserted.
...
- **Wire assertion:** Sandbox-only — `lint.LintString(q, &lint.Options{Compiled: c})` over each cassette MUST produce exactly the expected `lint.Issue.Code` multiset: `valid.aql` → `aql_select_no_alias`; `missing_archetype.aql` → `aql_archetype_not_in_template` and `aql_select_no_alias`; `bad_syntax.aql` → `aql_syntax`. The two `aql_select_no_alias` entries are REQ-164's deliberate re-baseline of this corpus — both queries project a column carrying no `AS` alias — whose additivity proof is recorded in [PROBE-099](#probe-099--aql-path-shape-lint-corpus)'s entry as REQ-164 § Additivity requires; the baseline table is `probe028Cases`, shared with that probe and with [PROBE-097](#probe-097--aql-semantic-and-portability-lint-corpus) arm (b) so the corpus is re-baselined once. The cross-implementation claim is scoped to **REQ-109's own codes**: any implementation of REQ-109 over the same grammar profile + template MUST report the same `aql_archetype_not_in_template` and `aql_syntax`. Codes contributed by this SDK's later additive Layer-2 groups are not required of a REQ-109 implementation — each is held by the probe of the requirement that added it — so this multiset grows whenever such a group legitimately flags a cassette, and that growth is a re-baseline recorded there, never a stability breach. Detail text and path strings are not asserted (observable-behaviour level).
- **Modes:** Sandbox.
...
- **Title:** The builder's negated containment (`NOT CONTAINS`), sibling containment junctions (`AND`/`OR` with precedence-driven parenthesisation), opt-in in-text `LIMIT`/`OFFSET`, the deprecated `SELECT TOP n [FORWARD|BACKWARD]` clause (REQ-118), and the REQ-163 write-side carriers — the `VERSION` predicate bracket, the class-position standing comparison, and the typed projection (`DISTINCT`, `AS`, aggregates, function calls, literals, star) — emit byte-stable canonical AQL, and a builder program using none of the new API produces byte-identical output to the pre-REQ-117 golden.
- **Preconditions:** Golden fixtures under [`openehr/aql/testdata/wire/`](../../openehr/aql/testdata/wire/) covering the new constructs — one per `versionPredicate` alternative, one per standing-predicate operand kind the write side can EXPRESS — `pathPredicateOperand`'s `primitive`, `objectPath` and `PARAMETER` alternatives; its `ID_CODE` and `AT_CODE` alternatives have no `aql.Value` shape and so are unreachable from the builder rather than omitted from the fixtures — plus REQ-161's suppression shape, and one per `columnExpr` alternative plus the clause-level flags — beside the untouched PROBE-020 golden.
- **Wire assertion:** Sandbox golden comparison (no backend) — built query strings MUST match the committed goldens byte-for-byte; requesting both in-text and envelope paging MUST be a build-time error, never a silently combined emission, and so MUST a `TOP` combined with either row-limit channel (QUERY Release-1.1.0 § 4.4.3 forbids `TOP` with `LIMIT`) or a negative `TOP` count. Since REQ-163 the same holds for the projection and the class brackets, and the assertion is over the refusal CLASS rather than over a fixed count of rows: a build-time error, never an emission, for (i) a `SELECT` that does not read back as the projection the builder recorded — split into more items, spilled into another clause, carrying a clause-level flag or a `TOP` direction the builder never set, or leaving a lexical region open where the clause ends — and (ii) any typed construction whose shape the grammar does not admit at the position it renders, which today is a standing predicate on a `VERSION`-spelled node (whose bracket is the other production, and which has its own carrier), a `FuncCall`-shaped projected literal, an aggregate-shaped function argument, a function-shaped comparison operand in either bracket, and a version-predicate shape outside the three the package seals. A shape added to either arm extends this assertion without amending it.
- **Modes:** Sandbox.
- **Status:** Implemented (Sandbox) — see [`testkit/probes/aql/probe_088_builder_containment_paging.go`](../../testkit/probes/aql/probe_088_builder_containment_paging.go), run by `TestProbe088BuilderContainmentAndPaging`; goldens in [`openehr/aql/testdata/wire/`](../../openehr/aql/testdata/wire/). Every golden is additionally re-parsed and re-emitted (`TestProbe088GoldensRoundTripThroughParse`), which ties the write-side canonicaliser to PROBE-087; the paging channel-exclusivity refusals — including the REQ-118 `TOP` rules — are pinned in [`openehr/aql/paging_test.go`](../../openehr/aql/paging_test.go), and the REQ-163 carriers' own accept and refusal rows in [`openehr/aql/version_test.go`](../../openehr/aql/version_test.go), [`openehr/aql/standing_test.go`](../../openehr/aql/standing_test.go) and [`openehr/aql/projection_test.go`](../../openehr/aql/projection_test.go), with the read-side identity round trip for each new construct in [`openehr/aql/containment_roundtrip_test.go`](../../openehr/aql/containment_roundtrip_test.go).
- **Satisfies:** REQ-117, REQ-118, REQ-163.
...
- **Preconditions:** The vendored grammar profile (parse) and the pinned BMM via `rminfo` — no CDR, no OPT. The [REQ-160 acceptance table](clinical-modeling.md#req-160--aql-containment-admissibility-relation) is the relation oracle; the EHRbase admissibility matrix is a neutral observed-behaviour cross-check, including the EHRbase-compatibility guard (an RM-valid pair a conformant engine admits never verdicts Never).
- **Wire assertion:** In-repo property, three arms. **(a) Per-code corpus:** for each REQ-161 code, at least one corpus query yields it with the specified severity at the span of the offending construct, and at least one near-miss query stays silent — the unknown-class suppression rule (an unknown operand suppresses the pair checks), the archetype-mismatch suppression (a literal archetype predicate whose declared class or HRID type segment is unknown yields `aql_unknown_rm_class` only, never `aql_archetype_class_mismatch`) and the `aql_fanout_row_grain` conservative firing rule each pinned by their own negative case. **(b) Additivity guard:** every query of the pre-REQ-161 corpus that carried no REQ-161 defect yields an unchanged issue-code multiset; corpus queries that genuinely carry a newly-checked defect are re-baselined deliberately, recorded here. **(c) Read/write parity (REQ-162):** for corpus queries expressible through the builder, the verification entry point's code multiset equals the containment-check subset of `lint.LintString` over the emitted text. What "expressible through the builder" covers widened at REQ-163 — the class-position `VERSION` predicate, the class-position standing comparison and the typed projection are builder constructs now, so the corpus MUST carry them through the same comparison; what stays unspellable is the FROM ROOT's own bracket (archetype, standing or version), the write-side FROM clause carrying an RM type and an alias and nothing else.
- **Modes:** In-repo (unit-level; no backend).
- **Status:** Implemented (inline) — see [`testkit/probes/aql/probe_097_semantic_lint.go`](../../testkit/probes/aql/probe_097_semantic_lint.go), run by `TestProbe097SemanticLint` in [`testkit/probes/aql/probes_test.go`](../../testkit/probes/aql/probes_test.go). Arm (a) pins all eight REQ-161 codes with severity and span, plus the three mandatory negative cases (unknown-class suppression, archetype-mismatch suppression, the `aql_fanout_row_grain` conservative firing rule). The unknown-class suppression negative is additionally backed by a Never-verdict sibling row that observably arms the operand-level suppression rule; the pure unknown-operand case is redundant-by-construction under REQ-160's pair-question totality and is pinned at engine level instead by `semcheck_test.go`'s `TestPairSuppressedByOperandVerdict`. Arm (b) re-ran PROBE-028's three-cassette corpus under the completed linter: zero REQ-161 codes gained on any of them — **no re-baseline required**, confirming the controller's precomputed prediction. Arm (c) holds read/write parity over the corpus's builder-expressible rows (the five containment codes) and asserts the corpus is not vacuous (every containment code is exercised by at least one row, and at least one row is clean); `VerifyContainment` never emits the three portability codes, and REQ-162 § Contract scopes parity to the containment-check subset, so those three are out of scope by contract rather than by omission. REQ-163 narrowed the other exclusion: the version-predicate, standing-predicate and typed-projection shapes the landed corpus recorded as builder-inexpressible are now four arm-(c) rows — REQ-161's own suppression shape (both class-position carriers at once), an explicit version tier in `CONTAINS` position, a standing predicate on an impossible pair, and a typed projection over an unknown class — so the new brackets and the rewritten `SELECT` are held to the same read/write agreement. What remains unreachable is the FROM root's bracket in every spelling (archetype, standing, version), the write-side FROM clause carrying an RM type and an alias and nothing else.
- **Satisfies:** REQ-160, REQ-161, REQ-162, REQ-163.

#### PROBE-099 — AQL path-shape lint corpus

- **Title:** Every REQ-164 issue code fires on a corpus query built to carry exactly that defect, with Warning severity and a span on the offending segment, projection item, path or class expression; every firing rule's negative near-miss stays silent; and no query free of REQ-164 defects changes its issue-code multiset.
- **Preconditions:** The vendored grammar profile (parse) and the pinned BMM via `rminfo` — no CDR, no OPT. The [REQ-164 catalogue](clinical-modeling.md#req-164--aql-path-shape-and-paging-lint) is the oracle for each code's firing and silence rules; the [REQ-160 relation](clinical-modeling.md#req-160--aql-containment-admissibility-relation) is the oracle for the redundant-step rows, which the group carries — implementation review kept `aql_contains_redundant_step` (REQ-164 § The redundant-step ruling).
- **Wire assertion:** In-repo property, two arms — PROBE-097's structure minus the read/write-parity arm, these codes having no builder analogue.
  (a) **Per-code corpus:** for each REQ-164 code, at least one corpus query yields it with Warning severity at the span of the offending construct — including the audit's two verified-silent queries (the unpredicated repeating-segment projection and the `LIMIT`-without-`ORDER BY` query), which MUST now warn — and each firing rule's negative near-miss stays silent: a predicated repeating segment, `ORDER BY` beside a `LIMIT`, a nil `Options.Query`, a `TOP`-only row bound, an aliased projection item, a `*` item, two projected paths sharing every repeating segment in their common prefix, two projected paths on different aliases, an *avoidable* unreferenced intermediate, an unreferenced leaf, and a `VERSIONED_OBJECT`-conforming operand (which carries REQ-161's code instead). The conservative walk's silent stops are pinned by name — unknown class, undeclared attribute, generic-parameter type, `$param` archetype scope — and a `Result` carrying only this group's findings reports `OK()` true.
  (b) **Additivity guard and PROBE-028 re-baseline:** every query of the pre-REQ-164 corpus that carries no REQ-164 defect yields an unchanged issue-code multiset; [PROBE-028](#probe-028--aql-lint-stability)'s three cassette queries are re-run and any code they gain is recorded here as a deliberate re-baseline, per [REQ-161 § Additivity](clinical-modeling.md#additivity).
- **Modes:** In-repo (unit-level property; no backend).
- **Status:** Implemented (inline) — see [`testkit/probes/aql/probe_099_path_shape_lint.go`](../../testkit/probes/aql/probe_099_path_shape_lint.go), run by `TestProbe099PathShapeLint` in [`testkit/probes/aql/probes_test.go`](../../testkit/probes/aql/probes_test.go). Arm (a) pins all five REQ-164 codes with Warning severity and a span on the offending construct — a path segment name, a whole identified path, a projection item or a class token, and the deliberate ZERO span of `aql_paging_no_order_by`, whose two channels have no position in the query text. The three firing rows the wire assertion names (the two audit queries, and the WHERE-only clause-scope witness) and the fifteen negatives it names are each claimed by name in the corpus, so the corpus cannot shrink below them while still reporting pass; `TestProbe099GuardsCanFail` is the able-to-fail control for every one of those guards, and mutates the shipping corpus rather than a stand-in. This widening adds three further firing rows beyond that named floor: the redundant-step row judged under a *supplied* relation (REQ-164 § Always on, never gated — a supplied relation governs that check, it does not gate it), a repeating path reached only as a function-call argument joining the fan-out like any other projection of its scope, and the counter-assertion that an in-text `LIMIT` and an envelope `Fetch` together raise `aql_paging_no_order_by` exactly once. The Silent half gains one further row beyond its named fifteen: a *discriminating* supplied relation (`EHR_STATUS`→`OBSERVATION`) that SILENCES the same default-relation-firing witness the Fire arm's supplied-relation row above uses — proving Options.Relation is actually read at probe level, which that Fire row's inert `EHR`→`VERSIONED_PARTY` overlay cannot (mirroring `TestRedundantStepReadsTheSuppliedRelation` in [`openehr/aql/lint/pathshape_redundant_test.go`](../../openehr/aql/lint/pathshape_redundant_test.go), same witness query and overlay). `aql_contains_redundant_step` **shipped**: the conditional REQ-164 § The redundant-step ruling records is resolved in favour of keeping the check, so the group carries five codes rather than four. The rule-by-rule firing and silence rows sit at unit level in [`openehr/aql/lint/pathshape_test.go`](../../openehr/aql/lint/pathshape_test.go), [`pathshape_parseonly_test.go`](../../openehr/aql/lint/pathshape_parseonly_test.go), [`pathshape_fanout_test.go`](../../openehr/aql/lint/pathshape_fanout_test.go), [`pathshape_redundant_test.go`](../../openehr/aql/lint/pathshape_redundant_test.go) and [`pathshape_internal_test.go`](../../openehr/aql/lint/pathshape_internal_test.go) (the conservative walk's four silent stops, pinned by name). Arm (b) re-ran PROBE-028's three-cassette corpus under the completed linter and **re-baselined it deliberately**: `valid.aql` and `missing_archetype.aql` each gained exactly `aql_select_no_alias` and nothing else — both project a column with no `AS` alias, a defect those queries genuinely carry — while `bad_syntax.aql` gained nothing at all, failing at Layer 1 before Layer 2 runs. Neither cassette gained `aql_path_repeating_unpredicated` (`valid.aql` predicates every repeating segment), a row bound, a second projected path under one alias or a redundant step. That is the deliberate, recorded re-baseline [REQ-161 § Additivity](clinical-modeling.md#additivity) defines, applied as designed and recorded here as REQ-164 § Additivity requires; the baseline table itself is `probe028Cases`, shared with PROBE-028 so the re-baseline was made once.
- **Satisfies:** REQ-164.

#### PROBE-100 — Upstream AQL admissibility corpus ratchet

- **Title:** Every FROM/CONTAINS variant the vendored upstream corpus records as engine-accepted parses under the SDK grammar profile and raises no Error-severity containment code, so the REQ-160 relation is held to the looser-of-the-two position against the corpus itself rather than against a maintainer-kept row list.
- **Preconditions:** The vendored corpus under `testkit/cassettes/aql/conformance/`, with its provenance pin `AQL_SOURCE.txt` and the ingest-generated exclusion list `EXCLUDED.txt` beside it ([§ Vendored fixtures](#vendored-fixtures-testkitcassettes)) — the EHRbase Robot integration-test data, whose combination rows each record a FROM/CONTAINS shape the engine accepted and answered. A corpus row is not a query: upstream keeps the varying part of a query in the CSV and the invariant part in the consuming `.robot` suite, so the **reconstruction templates are the other half of this probe's input**. They live in the reader's `conformanceSuites` table in [`testkit/probes/aql/probe_100_conformance_corpus.go`](../../testkit/probes/aql/probe_100_conformance_corpus.go), transcribed verbatim from the `.robot` sources at the pin `AQL_SOURCE.txt` records, each entry naming the suite path it came from; the `${ehr_id}` run-time Robot variable two of them carry is substituted with a fixed literal, this probe asserting admissibility rather than results. The vendored grammar profile (parse) and the pinned BMM via `rminfo` — no CDR, no OPT.
- **Wire assertion:** In-repo property — for EVERY engine-accepted FROM/CONTAINS variant in the vendored corpus: (a) the query reconstructed with the consuming suite's own template **MUST** parse under the SDK grammar profile (`parse.ParseQuery` succeeds), and (b) the [REQ-161](clinical-modeling.md#req-161--aql-semantic-and-portability-lint) containment checks **MUST** raise no Error-severity code on it — the [REQ-160](clinical-modeling.md#req-160--aql-containment-admissibility-relation) relation never verdicts **Never** on a pair a conformant engine demonstrably admits, which is that requirement's compatibility guard generalised from hand-picked rows to the corpus. Warnings are permitted: `aql_unknown_rm_class`, `aql_containment_by_reference` and the portability advisories are observations about a pair, never admissibility refusals. The reverse direction — engine-rejected shapes — is **NOT** asserted: the SDK's documented position is to stay the looser of the two, and the corpus encodes acceptance, not a rejection catalogue. Coverage is inspectable rather than claimed: the exclusion list is generated by the ingest and committed beside the corpus, so every upstream file the corpus does not carry is named with its reason.
- **Modes:** In-repo (ratchet property over a vendored corpus; no backend).
- **Status:** Implemented (inline) — see [`testkit/probes/aql/probe_100_conformance_corpus.go`](../../testkit/probes/aql/probe_100_conformance_corpus.go), run by `TestProbe100ConformanceCorpus` in [`testkit/probes/aql/probes_test.go`](../../testkit/probes/aql/probes_test.go). At the pin `AQL_SOURCE.txt` records, the reader reconstructs **67 rows across 12 CSVs in five families** — AND_OR 5, CONTAINS_A_D 16, EHR_STATUS 12, PREDICATE_A_D 5, USABLE_RM_TYPES_A_D 29 — and every one of them parses and draws no Error-severity containment code, so the relation needed no widening to meet the corpus. No row is excluded at this pin; the reader's exclusion rules (a blank template-variable cell, and a reconstruction still holding a `${…}` placeholder) are named rules carrying a reason, so a refresh that stops reconstructing a row records it in the tally rather than dropping it on the quiet. The ratchet has a refresh arm as well as a row arm: an unlearned CSV, an unlearned family directory, an unlearned file at the corpus root, a missing ingest file (`AQL_SOURCE.txt`, `EXCLUDED.txt`), a header that does not match the table field for field, or a table entry whose file went missing is a read error rather than a partial corpus, so a corpus refresh fails until the reader learns it. The ingest itself refuses a new upstream `from/combinations` input CSV that is neither vendored nor a named exclusion, so new input data cannot be mis-filed in `EXCLUDED.txt` under a fallthrough tag. A family directory contributing no asserted row fails the probe (the empty-corpus tripwire, per family rather than corpus-wide), and three suites are pinned by name because the family tripwire would not notice their loss: `CONTAINS_A_D/from_contains_plus_contain_chaining.csv` — the suite REQ-160's hand-picked compatibility guard was drawn from, so generalising that guard to the corpus cannot quietly lose it — and `EHR_STATUS/from_single_ehr.csv` / `EHR_STATUS/via_part.csv`, the only two templates carrying the `${ehr_id}` substitution, whose regression sends their rows to the exclusion list while `EHR_STATUS/contains.csv` keeps the family alive. The shipping tally itself (67 asserted, 0 excluded) is asserted by the running test, not only recorded here, so an intra-family shrink — invisible to a tripwire that fires only at zero — re-baselines this snapshot and the pin together, deliberately. Warnings are read but not asserted, and Error severity is taken from `lint.Issue.Severity` alone rather than from a copy of which code is an Error kept here. `TestProbe100DetectsInadmissibleRows`, `TestProbe100PermitsWarningSeverityContainmentCodes` (the Warnings-are-permitted arm: a crafted row drawing `aql_unknown_rm_class` or `aql_containment_by_reference` at Warning severity still yields pass — no shipping row warns, so only this control sees a severity-gate regression), `TestProbe100DetectsCorpusShrinkage`, `TestProbe100CoversThePinnedSuites`, `TestProbe100RequiresANonEmptyCorpus`, `TestProbe100ReaderRefusesAnUnlearnedCorpus`, `TestProbe100ReaderRequiresTheIngestFiles` and `TestProbe100ReaderExcludesUnreconstructableRows` are the able-to-fail controls for each of those guards; `TestConformanceSuitesVarsPrefixHeader` pins the reconstruction table's own Vars-are-leading-Header-columns contract, so a drifted entry fails by name instead of substituting the wrong cell silently.
- **Satisfies:** REQ-160.
...
- **Status:** Implemented (inline) — [`testkit/probes/serialize/probe_076_simplified_round_trip.go`](../../testkit/probes/serialize/probe_076_simplified_round_trip.go), run by `TestProbe076` over the vendored constraint-template corpus (24 pass, 1 skip).
- **Satisfies:** REQ-053.
...
- **Preconditions:** Canonical-JSON EHR_STATUS bodies — one omitting the `subject` key; one supplying `subject` as a bare `{"_type":"PARTY_SELF"}` (no external_ref); one omitting the interface-typed `name`. Planned ARCHETYPED bodies: one with an `archetype_details` missing `archetype_id`; one with `archetype_details.rm_version` missing or `null`.
...
- **Title:** `GET|POST /query/{qualified_query_name}` returns a typed `ResultSet`.
...
- **Status:** Implemented (Live snapshot) — the stored execution returns a typed `ResultSet` with `Columns` and `Rows` populated against a live CDR ([`testkit/probe/livestoredquery_test.go`](../../testkit/probe/livestoredquery_test.go)); a bound parameter scopes the result to the run's own EHR, so the row read back is this run's, not merely a decodable shape. The Sandbox and Cassette arms remain planned. The Live witness executes via POST, which REQ-057 permits; a GET arm is planned.
- **Satisfies:** REQ-057.
...
- **Wire assertion:** `POST` with an OPT body and `Content-Type: application/xml` returns `201` with a `Location` header; a subsequent `GET` returns the same OPT bytes (modulo backend-side reformatting documented per deployment). The SDK's `Accept` negotiation and 201-response-body handling are the client contract in [wire.md § REQ-144](../specifications/wire.md#req-144--definition-metadata-decoding) — *Template upload negotiation and response* — not asserted here.
- **Modes:** Sandbox, Cassette, Live.
...
- **Modes:** Sandbox; Cassette, Live not yet scoped.
- **Status:** Implemented (Sandbox) — [`testkit/probes/query/probe_078_verb_aware_scoping.go`](../../testkit/probes/query/probe_078_verb_aware_scoping.go); harness in [`probes_test.go`](../../testkit/probes/query/probes_test.go). Drives all three endpoints (`/query/aql`, `/query/{qualified_query_name}` and `/query/{qualified_query_name}/{version}`) over both verbs, six calls, asserting each request scopes on the verb-appropriate channel and carries no other: a POST's query parameter and request body are both checked, a GET's header. Seven can-fail plants move the scope onto a wrong channel per verb (one into the POST body); two of them fire only on the stored and stored-versioned arms, so dropping an arm fails a test. REQ-055 verb-aware scoping is also unit-covered ([`openehr/client/query/query_test.go`](../../openehr/client/query/query_test.go)). Live is not scoped: EHRbase does not restrict a top-level `FROM EHR e` by the `openehr-ehr-id` header, so the header/query-parameter distinction is witnessed by request capture (Sandbox), not by a Live result delta.
- **Satisfies:** REQ-055.
...
- **Wire assertion:** Three arms, kept at observable-behaviour level. (a) A read served a 200 with an undecodable body fails with a non-nil error, and the returned resource is never a zero value paired with a nil error — a silent zero-valued success is the exact defect this probe exists to catch — and nothing is handed back beside that error: the returned resource is nil, since a failed decode has nothing to return. (b) The same read served a 404 fails with a non-nil error, likewise with a nil resource beside it. (c) A Definition list leaf served a JSON object where an array is expected fails with a non-nil error, and the slice beside that error carries zero entries — reporting an empty catalog for a body that did not decode would read a failure as success, which [REQ-151](transport.md#req-151--typed-2xx-decode-failure)'s keyed exclusion admits only for an empty *body*. Each arm additionally issues exactly one request: without that count, a leaf refusing the call before it reached the wire would credit the probe with a failure the server never provoked, and the probe would pass against an SDK that decodes nothing at all. **That count rule is scoped to `Modes: Sandbox`**, where no retry or redirect policy is in play; a later Cassette or Live promotion restates it against that mode's transport policy rather than inheriting it, so the probe keeps failing on decode behaviour and never on transport policy. Error-type identity (`*transport.DecodeError` versus `*transport.WireError`), raw-`Body` recovery, and the value-free `Error()` string are pinned by `transport/` and `openehr/client/definition/` unit tests, not by this probe (REQ-080: no error-type assertions in probes).
- **Effect:** read-only — every arm issues a read and asserts on how the call fails.
- **Modes:** Sandbox.
- **Status:** Implemented (Sandbox) — [`testkit/probes/transport/probe_101_decode_failure_surfaced.go`](../../testkit/probes/transport/probe_101_decode_failure_surfaced.go); harness in [`probes_test.go`](../../testkit/probes/transport/probes_test.go). Plan: [`docs/plans/archive/2026-08-30-read-path-decode-taxonomy.md`](../plans/archive/2026-08-30-read-path-decode-taxonomy.md).
- **Satisfies:** REQ-151.

#### PROBE-102 — System capabilities via `OPTIONS /`
...
| REST binding | PROBE-060 … 068, PROBE-071, PROBE-072, PROBE-073, PROBE-078, PROBE-079, PROBE-084, PROBE-091, PROBE-092, PROBE-093, PROBE-101, PROBE-102, PROBE-103, PROBE-104 | partial — PROBE-061/071 (`Prefer: return=representation`, REQ-094) implemented (Sandbox) at [`testkit/probes/versioned/probe_071_composition_write_response_shape.go`](../../testkit/probes/versioned/probe_071_composition_write_response_shape.go) + leaf unit tests; PROBE-072 (REQ-050/095 contribution submission shape) implemented (Sandbox) at [`testkit/probes/versioned/probe_072_contribution_submission_shape.go`](../../testkit/probes/versioned/probe_072_contribution_submission_shape.go); PROBE-073 (Demographic PARTY polymorphic round-trip) implemented (Sandbox) at [`testkit/probes/demographic/probe_073_demographic_round_trip.go`](../../testkit/probes/demographic/probe_073_demographic_round_trip.go); REQ-094 `identifier` / empty-body follow-ups landed (leaf unit tests, archived [`2026-05-25-req094-prefer-followups.md`](../plans/archive/2026-05-25-req094-prefer-followups.md)); PROBE-065 (REQ-094 `minimal`→GET round-trip) is implemented (Sandbox) at [`testkit/probes/versioned/probe_065_minimal_return_round_trip.go`](../../testkit/probes/versioned/probe_065_minimal_return_round_trip.go); PROBE-078 (REQ-055 verb-aware query scoping) is implemented (Sandbox) at [`testkit/probes/query/probe_078_verb_aware_scoping.go`](../../testkit/probes/query/probe_078_verb_aware_scoping.go); PROBE-079 (REQ-057 `PutStoredQuery` `Location` recovery) is witnessed Live (snapshot) by [`testkit/probe/livestoredquery_test.go`](../../testkit/probe/livestoredquery_test.go), with its dedicated Sandbox probe file planned. PROBE-060 (REQ-095 EHR creation round-trip) and PROBE-062 (REQ-059 `openehr-audit-details` header) are implemented (Sandbox), as are the three STRAND-09 REST probes PROBE-102 (System `OPTIONS`), PROBE-103 (Admin bulk-delete) and PROBE-104 (Definition `/example`) — all under [`testkit/probes/rest/`](../../testkit/probes/rest/). PROBE-092 (REQ-142 contribution read) is implemented (Sandbox) at [`testkit/probes/versioned/probe_092_contribution_get.go`](../../testkit/probes/versioned/probe_092_contribution_get.go); PROBE-091 (REQ-150 path-segment validation) is implemented (Sandbox) at [`testkit/probes/transport/probe_091_path_segment_validation.go`](../../testkit/probes/transport/probe_091_path_segment_validation.go); PROBE-101 (REQ-151 typed 2xx decode failure — an undecodable 200, a 404 and a list body of the wrong JSON shape, each held to exactly one request) is implemented (Sandbox) beside it at [`testkit/probes/transport/probe_101_decode_failure_surfaced.go`](../../testkit/probes/transport/probe_101_decode_failure_surfaced.go); PROBE-093 (REQ-143 template list filters) is implemented (Sandbox) at [`testkit/probes/definition/probe_093_template_list_filters.go`](../../testkit/probes/definition/probe_093_template_list_filters.go); PROBE-084 (REQ-130 built contribution body) is implemented (Sandbox) at [`testkit/probes/versioned/probe_084_built_contribution_body.go`](../../testkit/probes/versioned/probe_084_built_contribution_body.go), with a byte golden beside it and the vendored submission corpus as its shape witness. |
