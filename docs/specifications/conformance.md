...
- **Status:** **Draft — blocked.** The corpus, its pin, and its sync/verify tooling are landed, and the corpus OPT now compiles (`templatecompile` admits shared-path subtrees). `webtemplate.Build` still returns `ErrIDCollision` for it: sibling archetype roots reusing one archetype id need the reference's name-derived web `id` (and name-predicated `aqlPath`), which `openehr/template` does not yet parse. Precise mechanism and the four layers involved: [`deviations.md`](../../openehr/template/webtemplate/deviations.md) § Sibling `id` disambiguation. No adapter code lands until that clears.
