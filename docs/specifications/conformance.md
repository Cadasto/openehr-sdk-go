...
- **Preconditions:** The vendored grammar profile (parse) and the pinned BMM via `rminfo` — no CDR, no OPT. The [REQ-164 catalogue](clinical-modeling.md#req-164--aql-path-shape-and-paging-lint) is the oracle for each code's firing and silence rules; the [REQ-160 relation](clinical-modeling.md#req-160--aql-containment-admissibility-relation) is the oracle for the redundant-step rows if that check ships (REQ-164 § The redundant-step ruling).
...
- **Modes:** In-repo (unit-level property; no backend).
- **Status:** Draft — planned in [`docs/plans/2026-08-26-aql-path-shape-lint.md`](../plans/2026-08-26-aql-path-shape-lint.md).
- **Satisfies:** REQ-164.
