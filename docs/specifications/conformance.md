...
- **Preconditions:** A commit-pinned EHRbase `openEHR_SDK` `constrain_test` OPT + reference WebTemplate JSON, vendored under [`testkit/cassettes/`](../../testkit/cassettes/) with provenance + Apache-2.0 attribution. (`constrain_test` is used rather than `corona_anamnese` because the latter's WebTemplate cannot be built yet: several of its sibling archetype roots reuse one archetype id, so this builder derives the same web `id` for each and `webtemplate.Build` refuses to emit ambiguous duplicates — the open [REQ-116](clinical-modeling.md#req-116--template-level-node-naming-and-name-predicated-paths) gap that blocks this surface of REQ-106, recorded in [`deviations.md`](../../openehr/template/webtemplate/deviations.md) § Sibling `id` disambiguation and [ADR 0014](../adr/0014-webtemplate-reference-implementation-lock.md). The reference resolves it by deriving `id` from the OPT's template-level node name and adding a name predicate to `aqlPath`; as of REQ-116 Phase 3 the predicate *is* emitted on both compiled and WebTemplate paths, but `id` still derives from the archetype concept term, so the collision stands until Phase 4. `constrain_test` pins **no** names at all, so its golden carries **zero** name predicates across all 104 nodes — that, not an absence of sibling collision, is why parity holds at 104/104. The earlier reading of the corona failure as "does not compile — archetype reuse under a slot" was incorrect: the cause was shared-path subtrees, now admitted. `Corona_Anamnese` and `GECCO_Diagnose` are vendored with their reference WebTemplates as the REQ-116 oracles ([plan](../plans/2026-07-29-template-node-naming.md) Phase 0, done): both compile under in-tree tests, `Build(Corona_Anamnese)` is pinned to `ErrIDCollision`, and `GECCO_Diagnose` builds but silently diverges from its name-predicated golden — parity extension to both is the plan's **Phase 4** task 4.)
...
- **Title:** For each body in the pinned upstream EHRbase FLAT conformance corpus, decoding it and re-encoding it through the SDK's REQ-053 codec reproduces the upstream FLAT — same path set, same leaf values — against a documented-deviations list.
- **Preconditions:** The commit-pinned corpus at [`testkit/cassettes/flat-conformance/`](../../testkit/cassettes/flat-conformance/) — the `conformance_ehrbase.de.v0` OPT plus 34 `ehrbase_conformance_*.json` FLAT bodies from `composition/flat/simSDT/conformance/`, integrity-checked against `MANIFEST.txt` by `make flat-conformance-check` (Apache-2.0; provenance in [`THIRD_PARTY_LICENSES.md`](../../testkit/cassettes/THIRD_PARTY_LICENSES.md)). Resolved via `fixtures.FlatConformanceOpt` / `ListFlatConformance`.
- **Wire assertion:** Not backend-facing — an in-repo parity property. `template.ParseFile` + `templatecompile.Compile` + `webtemplate.Build`, then per fixture `simplified.UnmarshalFlat(upstream, wt, WithTemplate(c))` + `simplified.MarshalFlat` MUST yield the upstream FLAT path set with equal leaf values; every accepted difference MUST appear on the deviations list, and any unlisted difference is a failure. This is strictly stronger than [PROBE-076](#probe-076--flat--structured-composition-round-trip), whose input is the SDK's *own* output and so cannot catch a path this SDK never emits, a suffix it names differently, or a leaf it silently drops — the upstream-conformance follow-up PROBE-076's scope limit names. A fixture whose construct the codec does not model yet is a documented skip, never a silent pass.
...
- **Status:** Draft — blocked. The corpus, its pin, and its sync/verify tooling are landed, and the corpus OPT now compiles (`templatecompile` admits shared-path subtrees). `webtemplate.Build` still returns `ErrIDCollision` for it: sibling archetype roots reusing one archetype id need the reference's name-derived web `id` (and name-predicated `aqlPath`) — [REQ-116](clinical-modeling.md#req-116--template-level-node-naming-and-name-predicated-paths), whose Phase 1 now parses the name. Precise mechanism and the four layers involved: [`deviations.md`](../../openehr/template/webtemplate/deviations.md) § Sibling `id` disambiguation. No adapter code lands until that clears. (The status force stays unbolded so `spec-check`'s Draft-probe guard can see it.)
