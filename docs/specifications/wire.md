...
| PARTY_IDENTIFIED / PARTY_RELATED (wherever the grammar reaches one) | — | `\|id`, `\|id_scheme`, `\|id_namespace`, `\|name`; nested `_identifier:N` (DV_IDENTIFIER — `\|id`, `\|issuer`, `\|assigner`, `\|type`); PARTY_RELATED adds `/relationship` (DV_CODED_TEXT). PARTY_SELF is spelled by the **absence** of every party key. `\|id` + `\|id_scheme` decompose the `external_ref`'s OBJECT_ID under the same discriminator rule as the OBJECT_REF families above (scheme present ⇒ GENERIC_ID), `\|id_namespace` its namespace; the reference writes no `\|type` and hardcodes `PARTY`, so absent **MUST** decode as `PARTY` and `\|type` **MUST** be emitted only where the reference's own value would be lost |
