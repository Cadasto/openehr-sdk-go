...
- **A polymorphic-slot failure**, which arrives as `typereg.DecodeError` (re-exported as `canjson.DecodeError`) carrying the path — including when a nested one travels out through an enclosing type's `canjson: <RM_TYPE>:` prefix, where it **MUST** keep only its own classification. `ErrInvalidShape` means "JSON-level shape", not "any decode failure".
