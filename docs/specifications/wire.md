...
- **Field order.** The encoder **MUST** write every RM value in this deterministic profile, so that re-encoding a decoded value is a fixed point (PROBE-030) and two encodes of the same value are byte-identical — the SDK's own output contract, which a consumer may rely on for byte comparison and hashing (see [`docs/plans/archive/2026-05-15-canonical-json-serialization.md`](../plans/archive/2026-05-15-canonical-json-serialization.md)):
