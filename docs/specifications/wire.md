...
- `TERM_MAPPING.match` **MUST** encode as a one-character JSON string and **MUST** decode from one; a string that is empty or longer than one character **MUST** be a decode error; a bare number **MAY** decode (back-compat) but **MUST NOT** be the encoded form. As with a preserved `Extras` value (§ Unknown response keys), `encoding/json` spells `<` and `>` as `\u003c` / `\u003e`; the escaped form decodes to the identical single character, so the obligation is on the decoded character, not the literal bytes. A value that is not valid UTF-8, or whose single character is the replacement character U+FFFD, **MUST** be refused on both decode and encode: `encoding/json` substitutes U+FFFD, reporting no error, for a lone UTF-16 surrogate escape and for an invalid byte, so a decoded U+FFFD is indistinguishable from corrupted input and a rune count alone would launder it into an apparently valid character; a bare number naming U+FFFD or a code point outside Unicode is refused the same way. Decode refusals on this path wrap `canjson.ErrInvalidShape` (§ Decode-side shape sentinel below); encode refusals surface through `canjson.ErrInvalidValue` like any other encode failure. The one-character rule is a property of the `Character` primitive, not of the JSON codec: `rm.Character` also implements `encoding.TextMarshaler` / `encoding.TextUnmarshaler`, so canonical XML (§ REQ-056) refuses the same values as element content, carrying no JSON sentinel.
...
**Decode-side shape sentinel.** A decode failure raised **inside** a generated RM type's `UnmarshalJSON` — the `canjson: <RM_TYPE>:` family, where the bytes are valid JSON but the wrong shape for the target type — **MUST** wrap the decode-only sentinel `canjson.ErrInvalidShape`, the counterpart of the encode-side clause above. The same sentinel also classifies a **hand-written primitive codec** (e.g. `rm.Real`) refusing a decimal literal past the precision budget § Floating-point precision defines — valid JSON, valid shape, but more significant digits than the budget admits — attached directly rather than through the generated funnel; see § Floating-point precision below. The failure's message **MUST** be unchanged by the classification, and the underlying `encoding/json` error (for example `*json.UnmarshalTypeError`) **MUST** stay reachable through unwrapping, so the sentinel costs no diagnostic. Two decode failures stay **outside** the sentinel by design, and **MUST NOT** acquire it:
