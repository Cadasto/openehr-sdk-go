...
|---|---|
| DV_PARSABLE | bare value + `\|formalism`, both RM-mandatory — so a DV_PARSABLE at a leaf is always fully captured and never rides `\|raw`. Its `_charset` / `_language` members are § REQ-140 rows |
