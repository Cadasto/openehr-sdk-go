...
- **A `typereg.DecodeError`** (re-exported as `canjson.DecodeError`) carrying the path, whether it was raised at a polymorphic slot or by a whole-value `_type` mismatch — a `_type` naming a different class than the target, refused on `/_type`. This includes a nested one travelling out through an enclosing type's `canjson: <RM_TYPE>:` prefix, where it **MUST** keep only its own classification. `ErrInvalidShape` means "JSON-level shape", not "any decode failure".

The sentinel value lives in `openehr/rm/typereg` so generated code under `openehr/rm` can attach it without an `openehr/rm → openehr/serialize` import cycle; `canjson.ErrInvalidShape` is the caller-facing name for that same value, and the three-distinct-sentinel rule above binds it unchanged. `bmmgen` emits one funnel for every `UnmarshalJSON` it generates, so the AOM 1.4 companions (`openehr/aom/aom14`, REQ-042) classify on the same terms; this clause's obligation is stated for the canonical-JSON RM surface this § governs.
