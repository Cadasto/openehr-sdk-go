...
- `TERM_MAPPING.match` **MUST** encode as a one-character JSON string and **MUST** decode from one; a string that is empty or longer than one character **MUST** be a decode error; a bare number **MAY** decode (back-compat) but **MUST NOT** be the encoded form. As with a preserved `Extras` value (§ Unknown response keys), `encoding/json` spells `<` and `>` as `\u003c` / `\u003e`; the escaped form decodes to the identical single character, so the obligation is on the decoded character, not the literal bytes.
- **`DV_TEXT.mappings` decode collapse (known limitation).** The canonical decoder cannot distinguish `"mappings": []`, `"mappings": null`, and an absent `mappings` key: all three decode to an empty `[]TermMapping`, and `omitempty` re-drops the key on encode, so all three collapse to the same wire form (pinned by `TestDVTextMappingsCollapse`). A caller needing the distinction reads JSON-key presence at the decode boundary — the shape REQ-112 uses for `EHR_STATUS.subject` (`ValidateRMEHRStatusBytes`). `omitempty` is intentionally unchanged: emitting `[]` would be RM-invalid (`Mappings_valid`), and emitting `null` is a spelling nothing needs.
- **`DV_MULTIMEDIA.data` / `integrity_check` (`[]byte`) encode as base64** — the correct canonical spelling, matching the FLAT conformance fixture (pinned by `TestDVMultimediaBytesRoundTrip`) — but carry the same `[]` / `null` / absent collapse as `mappings` above. This is not fixed: `size` already carries the unencoded byte count, so a present-but-empty inline `data` is not a distinction the SDK needs to preserve.
...
**Decode-side shape sentinel.** A decode failure raised **inside** a generated RM type's `UnmarshalJSON` — the `canjson: <RM_TYPE>:` family, where the bytes are valid JSON but the wrong shape for the target type — **MUST** wrap the decode-only sentinel `canjson.ErrInvalidShape`, the counterpart of the encode-side clause above. The same sentinel also classifies a **hand-written primitive codec** (e.g. `rm.Real`) refusing a value its Go type cannot represent without loss — valid JSON, valid shape, but more precision than the target can hold — attached directly rather than through the generated funnel; see § Floating-point precision below. The failure's message **MUST** be unchanged by the classification, and the underlying `encoding/json` error (for example `*json.UnmarshalTypeError`) **MUST** stay reachable through unwrapping, so the sentinel costs no diagnostic. Two decode failures stay **outside** the sentinel by design, and **MUST NOT** acquire it:
