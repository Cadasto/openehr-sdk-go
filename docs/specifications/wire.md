...
**System descriptor.** `system.Capabilities` decodes the `OPTIONS /` response into `ServiceCapabilities`, the descriptor that advertises the pinned `restapi_specs_version`. It is the third descriptor [§ REQ-144 *Unknown response keys*](#req-144--definition-metadata-decoding) governs — that rule, not a copy of it here, is what preserves its unknown response keys on decode and re-emits them on encode — so a deployment-specific capability the pin does not name survives a round trip through the SDK.
...
- **Member order.** The encoder **SHOULD** emit `_type` as the first member of every encoded concrete RM value, so that a consumer decoding as a stream can select the concrete type before reading the rest of the object. Beyond that, the order in which the encoder writes members is **unspecified**, and a consumer **MUST NOT** rely on it. JSON object member order carries no meaning (RFC 8259 § 4), the openEHR specifications prescribe none, and CDR implementations differ in the order they emit. The decoder **MUST** accept members in any order, `_type` included. The SDK asserts no order for another implementation's output, and it does not defer to a future openEHR member-order rule. [REQ-056](#req-056)'s element-order profile for canonical XML is deliberately **not** relaxed: element order is part of an XML document's identity in a way member order is not part of a JSON object's, so PROBE-033 keeps its byte assertion.
  ...
  Canonical-JSON member names are matched exactly. The case-insensitive matching [§ REQ-144 *Unknown response keys*](#req-144--definition-metadata-decoding) requires for preserved `Extras` values binds the Definition, System and AQL result surfaces, which are not on this codec path.
...
- `TERM_MAPPING.match` **MUST** encode as a one-character JSON string and **MUST** decode from one; a string that is empty or longer than one character **MUST** be a decode error; a bare number **MAY** decode (back-compat) but **MUST NOT** be the encoded form. As with a preserved `Extras` value (§ Unknown response keys), the codec leaves `<` and `>` unescaped; the obligation stays on the decoded character, not the literal bytes. U+FFFD (the replacement character) is an ordinary `Character` value (the openEHR BASE `Character` primitive excludes nothing), so a genuine U+FFFD, written as the literal character or as the JSON escape `\ufffd`, **MUST** decode. The codec refuses invalid UTF-8 and a lone surrogate escape outright, and a genuine U+FFFD still decodes. The bare-number arm refuses `0` and any non-scalar value (negative, a surrogate half, or past U+10FFFF); `65533` (U+FFFD) decodes like any other legal code point. Encode refuses a value that is not valid UTF-8 or is not exactly one character. Two kinds of decode refusal fall differently. Invalid UTF-8 and a lone surrogate escape are refused by the tokenizer, which the codec reports before it dispatches to any `UnmarshalJSONFrom` method (the **Malformed JSON** exclusion below), so they carry no SDK sentinel, exactly as that exclusion states. A refusal `rm.Character` itself raises, applying the one-character rule to a value the tokenizer accepted (an empty or multi-character string, or a control character), wraps `canjson.ErrInvalidShape` (§ Decode-side shape sentinel below), leaving the error's message unchanged by the classification. Encode refusals surface through `canjson.ErrInvalidValue` like any other encode failure. The one-character rule is a property of the `Character` primitive, not of the JSON codec: `rm.Character` also implements `encoding.TextMarshaler` / `encoding.TextUnmarshaler`, so canonical XML (§ REQ-056) holds the same value rule (valid UTF-8, exactly one character) on element content, carrying no JSON sentinel. A substituted U+FFFD is refused only on the JSON path, where the tokenizer rejects a lone surrogate escape and invalid UTF-8 outright before `rm.Character` sees the value: `encoding/xml` refuses invalid UTF-8 and a character reference past U+10FFFF itself, but converts a surrogate-half character reference such as `&#xD800;` to U+FFFD reporting no error, and that channel is accepted as a genuine U+FFFD rather than closed by refusing a legal character.
...
- **Typed-nil in an optional polymorphic slot.** An interface-typed optional attribute (`ELEMENT.value`, `ELEMENT.null_reason`, `LOCATABLE.uid`) holding a typed-nil pointer encodes as **absent** — the reading `rm.IsTypedNil`, `ValidateRM` and the simplified encoders already give it — and a `null` on decode yields a nil interface. This is a `canjson` contract: a v1 `encoding/json` caller marshalling the same value spells it `null`, under v1's legacy `omitempty`, so the two entry points are not byte-identical here (as they are not under HTML escaping either).
...
- **Malformed JSON**, which the codec reports before it dispatches to any `UnmarshalJSONFrom` method, as an underlying syntax or truncated-input error reachable through unwrapping.
...
**Causes may name the literal.** A cause beneath the sentinel **MAY** name the offending literal (`*strconv.NumError` and an underlying codec type-mismatch error both do) because the value-free discipline binds the boundary strings (`WireError.Error()`, `DecodeError.Error()`; [§ REQ-093](transport.md#req-093--openehr-error-envelope-mapping), [§ REQ-151](transport.md#req-151--typed-2xx-decode-failure)), not codec causes. A codec's own prefix **MUST NOT** repeat a value the wrapped cause already carries: `rm.Real` and `rm.Integer` report a quoted literal that fails to parse as `parse quoted literal: <cause>`, so the literal appears exactly once, in the cause.
...
The codecs **MUST** use the canonical media types `application/openehr.wt.flat+json` (FLAT) and `application/openehr.wt.structured+json` (STRUCTURED); they **SHOULD** accept the deprecated `.schema`-suffixed variants — retired from the specification but still served by EHRbase — on input for interoperability, but **MUST NOT** emit them. Classification is per media-type token — one `Content-Type` value or one `Accept` media range, matched case-insensitively with well-formed parameters ignored, `q` included; a comma-separated `Accept` list is the caller's to split. A value whose type part or parameters do not parse — a comma-separated list among them — or that names neither format — including the WebTemplate resource type `application/openehr.wt+json`, a template projection rather than a composition format — **MUST** be refused with a typed error (`simplified.ErrUnknownMediaType`), never defaulted to a format. (`simplified.ParseMediaType` and `Format.MediaType` implement this; the codecs themselves take bytes, so negotiation happens one call before them. Recorded in the package [deviations register](../../openehr/serialize/simplified/deviations.md).)
...
**Unknown response keys.** The two Definition descriptors — and, by [§ REQ-050](#req-050), the System descriptor `ServiceCapabilities` — carry an `Extras` map, and decode **MUST** route every response key outside the documented field set into it rather than dropping it; encode **MUST** re-emit those keys with their values unchanged, up to the insignificant whitespace `encoding/json` compacts and the `<`, `>`, `&` it escapes as `\u003c`, `\u003e`, `\u0026` inside a preserved value. The escaped spelling decodes to the identical value, so the carve-out is about the bytes, not the content. An `Extras` key whose name collides with a documented field name **MUST** be ignored on encode — the documented field is authoritative — and **MUST** be ignored even where that field's own JSON contract emits no key for it, since otherwise a caller-set collision would survive into output the descriptor's own decode rejects. Collision **MUST** be decided by **exact**, case-sensitive comparison against the documented JSON names. A wire key differing from a documented name only by case is therefore not a collision: `encoding/json` matches field names case-insensitively on decode, so such a key populates the documented field **and** is preserved in `Extras`, and encode emits both. Decode cannot create an exact-name collision — a documented name is routed to its field and never into `Extras` — so that case is caller-constructed. Round-trip identity of the key set is **not** promised in either direction: documented fields are emitted per their own JSON contract — a field whose contract omits it when empty or zero emits no key at all — and key order is not part of the contract.

**Empty list bodies.** `ListTemplates` and `ListStoredQueries` **MUST** return a **non-nil** zero-length slice and a nil error when a 2xx response body is empty — empty as [§ REQ-094](transport.md#req-094--prefer-response-shape-negotiation) defines it, decided by `transport.IsNoRepresentationBody` ahead of decode, which is what keeps a `null` list body from unmarshalling to a nil slice. This is the empty-*body* arm: a JSON `[]` already decodes to a non-nil empty slice through `encoding/json`. A nil slice boxed in a non-nil interface marshals as JSON `null` rather than `[]`, so a caller who re-serialises the result would publish `null` for "no templates" — the read-side twin of the typed-nil trap [REQ-094](transport.md#req-094--prefer-response-shape-negotiation) documents on the write path.

**Template upload negotiation and response.** `UploadTemplate` **MUST** send `Accept: application/json, application/xml`. The ITS-REST `201_Template_adl1_4_upload` response offers three representations — a JSON `TemplateIdentifier`, an XML `OperationalTemplate`, and `application/openehr.wt+json` — and its body is empty when the deployment applies the `return=minimal` default (this call sends no `Prefer` header). Requesting both is a client-contract interoperability choice, not a pin departure: the `Accept_Template` OpenAPI value is a single-token media range, and a comma-separated `Accept` list is ordinary HTTP the pin does not constrain (REQ-095). The decoded `*TemplateMetadata` **MUST** be taken from a JSON **object** body when one is present, and otherwise — an empty, `null`, or XML body — from the trailing segment of the `Location` header; a leading UTF-8 BOM is stripped before that classification, and a non-object JSON body is a typed decode error. A response that carries neither a JSON `template_id` nor a `Location` header cannot name the template and **MUST** surface as a typed error (`transport.ErrInvalidShape`), never a metadata record with an empty id. A deployment that serves the ADL 1.4 template surface only as XML answers `406 Not Acceptable` to an `application/json`-only `Accept`; that is a deployment characteristic (EHRbase 2.35.1 is one), not an ITS-REST violation, and requesting both representations is what keeps the SDK interoperable with it.
