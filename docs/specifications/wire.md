...
**Decode-side shape sentinel.** A decode failure raised **inside** a generated RM type's `UnmarshalJSON` — the `canjson: <RM_TYPE>:` family, where the bytes are valid JSON but the wrong shape for the target type — **MUST** wrap the decode-only sentinel `canjson.ErrInvalidShape`, the counterpart of the encode-side clause above. The failure's message **MUST** be unchanged by the classification, and the underlying `encoding/json` error (for example `*json.UnmarshalTypeError`) **MUST** stay reachable through unwrapping, so the sentinel costs no diagnostic. Two decode failures stay **outside** the sentinel by design, and **MUST NOT** acquire it:
...
Numeric magnitudes are serialised as IEEE 754 double-precision JSON numbers. The SDK **MUST NOT** silently coerce a magnitude through `float32` or a similarly lossy intermediate. If a wire value exceeds JSON's number precision (rare in clinical data), the SDK **MUST** report this on decode as a typed error rather than silently rounding. This clause is met on both its arms: an out-of-range magnitude (e.g. `1e400`) fails with a typed `*json.UnmarshalTypeError`, reachable by `errors.As` through the generated `UnmarshalJSON` wrapper; and a decimal literal carrying more than 17 significant decimal digits — `float64`'s shortest-round-trip guarantee, counted on the mantissa alone (sign, decimal point and exponent excluded, leading zeros before the first nonzero digit and trailing zeros of the digit run ignored) — fails decode wrapping `canjson.ErrInvalidShape`, in both the bare-number and quoted-decimal-string forms `Real` accepts. The SDK does **not** over-report: a value that is merely inexact but round-trips through `float64`, such as `0.1`, decodes cleanly. `rm.Real` is the sole primitive this clause governs; `DV_QUANTITY.magnitude`, `DV_PROPORTION.numerator`/`denominator` and every other `Real`-typed field inherit the check through the ordinary struct-field decode path, with no change to the generated code.
