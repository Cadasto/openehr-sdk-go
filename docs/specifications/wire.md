...
- Composition-level metadata is carried under the `ctx/` prefix (mandatory `language`, `territory`; optional `composer`, `time`, `setting`, participations, …). The reference implementation instead spells several of these as real paths under the template root (`<root>/language|code`, `<root>/composer|name`, `<root>/context/start_time`, …), carrying the same information. Decode **MUST** accept either spelling for a field that has an exact `ctx/` equivalent; encode **MUST** emit only the `ctx/` short form. Where both spellings appear for one field and disagree, the codec **MUST** fail rather than choose — see [ADR 0015](../adr/0015-flat-metadata-spelling.md), which also records the two composition-level families (`context/setting`, the composer's `external_ref`) that are **not** respellings and stay refused.
