...
| REQ-112 | Template-less Reference Model validation floor | [clinical-modeling.md § REQ-112](clinical-modeling.md#req-112--template-less-reference-model-validation-floor) | partial |
...
| REQ-130 | Contribution builder | [wire.md § REQ-130](wire.md#req-130--contribution-builder) | landed |
...
| REQ-144 | Definition metadata decoding | [wire.md § REQ-144](wire.md#req-144--definition-metadata-decoding) | landed |
...
| REQ-164 | AQL path-shape and paging lint | [clinical-modeling.md § REQ-164](clinical-modeling.md#req-164--aql-path-shape-and-paging-lint) | landed |
...
| Wire format — extensions | 140, 142–144 | 145–149 (141 retired — see note) |
| Transport — extensions | 150–151 | 152–159 |
| AQL semantics | 160–164 | 165–169 |

Identifiers **MUST** be stable once published. Renumbering is prohibited.

The wire band (050–059) is exhausted; **140–149** continues it for wire-format requirements. **REQ-141 is retired, not free**: it was briefly assigned to the path-validation requirement during PR #107's review and re-homed to the transport band as REQ-150; re-using 141 for an unrelated requirement would make that review history ambiguous, so the wire band continues at 142. The clinical-modeling band (100–119) is **exhausted** as of REQ-119: 110–113 and 116–119 are registered, and 114 / 115 are reserved by the [OPT author-validator](../plans/2026-07-16-opt-author-validator.md) and [FLAT author-linter](../plans/2026-07-16-flat-author-linter.md) plans. The next clinical-modeling requirement needs a new band, allocated here first. **130–139** is the SDK authoring & client tooling band, opened by [REQ-130](wire.md#req-130--contribution-builder) (the contribution builder) — an authoring surface over a landed wire shape rather than a wire requirement of its own, which is why it takes a band instead of continuing 140–149; its prose is homed in [wire.md](wire.md) beside the submission shape it assembles. The transport / REST-extension range (096–099) is exhausted; **150–159** continues it for transport requirements (first allocation: REQ-150). **160–169** is the AQL semantics band, opened by [REQ-160](clinical-modeling.md#req-160--aql-containment-admissibility-relation) / [REQ-161](clinical-modeling.md#req-161--aql-semantic-and-portability-lint) / [REQ-162](clinical-modeling.md#req-162--builder-containment-verification) (the containment admissibility relation and its lint / builder consumers, [ADR 0017](../adr/0017-aql-semantic-layer.md)); the prose is homed in [clinical-modeling.md](clinical-modeling.md) beside the REQ-109/113 AQL sections because the clinical-modeling band (100–119) is exhausted. The band had stayed free deliberately until this allocation: the earlier AQL structured-predicate and value-free-diagnostics work extended the landed [REQ-113](clinical-modeling.md#req-113--execution-oriented-parsed-aql-ast) and [REQ-109](clinical-modeling.md#req-109--aql-static-lint) sections rather than taking new identifiers. **163** was reserved by the [AQL write-side parity plan](../plans/archive/2026-08-26-aql-write-side-parity.md) and taken 2026-08-27 by [REQ-163](clinical-modeling.md#req-163--aql-write-side-expressivity-parity) (AQL write-side expressivity parity — the builder carriers that mirror the read-side version-predicate, standing-predicate and projection vocabularies), homed in [clinical-modeling.md](clinical-modeling.md) beside REQ-160/161/162 for the same reason they are; **164** was reserved by the [AQL path-shape lint](../plans/archive/2026-08-26-aql-path-shape-lint.md) plan following the 114/115 precedent and taken 2026-08-27 by [REQ-164](clinical-modeling.md#req-164--aql-path-shape-and-paging-lint) (AQL path-shape and paging lint — the repeating-segment, paging, projection-alias, path-fan-out and redundant-step checks the linter did not cover outside Layer 3), homed beside REQ-161 whose Layer-2 catalogue it extends, leaving **165–169 free** — the band is not exhausted. **REQ-048** — the RM meta-model introspection surface — was allocated in the BMM headroom rather than in the RM behavioural-functions headroom (124–129): [rm-functions.md](rm-functions.md) scopes itself to runtime behaviour on RM *values*, while this requirement is BMM-derived generated *structure* beside REQ-041/042/045. Numbering it 124 while homing the prose in [bmm-conformance.md](bmm-conformance.md) would have straddled one band across two documents; 124–129 therefore stays free for behavioural functions, and the BMM band’s remaining headroom was 049 — taken 2026-08-26 by [REQ-049](bmm-conformance.md#req-049--rm-class-universe-absence-reasons) (the class-universe absence-reason surface), which exhausts the band. **144** was reserved by the [Definition metadata decoding](../plans/archive/2026-08-30-definition-metadata-decoding.md) plan following the 114/115 precedent and taken 2026-08-30 by [REQ-144](wire.md#req-144--definition-metadata-decoding) (Definition metadata decoding — the closed tolerant layout set for the two catalog timestamps, decided by [ADR 0019](../adr/0019-definition-timestamp-tolerance.md), and the non-nil empty-list shape), homed in [wire.md](wire.md) beside REQ-143 whose Definition-leaf read path it extends, leaving 145–149 free; **151** was reserved by the [read-path decode taxonomy](../plans/archive/2026-08-30-read-path-decode-taxonomy.md) plan following the 114/115 precedent and taken 2026-08-30 by [REQ-151](transport.md#req-151--typed-2xx-decode-failure) (the typed 2xx decode failure — the `*transport.DecodeError` that carries the undecodable body instead of dropping it, [ADR 0018](../adr/0018-raw-bytes-on-decode-error.md)), homed in [transport.md](transport.md) after REQ-150 because it binds the shared `transport.Client` decode, leaving **152–159 free**. **034** was taken 2026-09-07 by [REQ-034](rm-modeling.md#openehr-terminology-vocabulary-req-034) (the vendored openEHR terminology vocabulary — the `openehr` groups and code sets, pinned and generated the way the BMM is), homed in [rm-modeling.md](rm-modeling.md) rather than the exhausted BMM band because the groups are what the RM's own invariants reference; **035–039 stay free**.
