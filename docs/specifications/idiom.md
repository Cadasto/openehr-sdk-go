...
The premise: **the SDK is idiomatic Go.** Wire-level correctness is defined against the openEHR spec (REQ-080), not by a particular source shape. Java-style "every type is an object", Python-style keyword arguments, repository-graph ports from other languages — none of these constrain the Go API.
...
// MAY — the pre-1.26 out-parameter shape
...
Sentinel matching with `errors.Is` is unchanged. Neither form is nil-proof: a failed match leaves a nil typed pointer that a caller can box into a non-nil `error`, which is why the no-panic rule below covers the nil receiver.
...
- A nil receiver on an exported method of an SDK error type (`Error`, `Unwrap` and any accessor) — the typed nil a failed `errors.As` / `errors.AsType` match leaves behind — answers instead of panicking, and so do the accessors of `transport.Client` (`Do`, `Catalog`, `HTTPClient`). An error type answers with the text its zero value produces unless its godoc documents a different nil-receiver text.
