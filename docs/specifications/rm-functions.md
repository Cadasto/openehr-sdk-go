...
**Attribute coverage for synthesized in-context leaves.** Path navigation is the resolution step the FLAT encoder (REQ-053) runs before leaf mapping, so an attribute this package cannot navigate is data the encoder cannot emit. Therefore:

- Every RM attribute leaf the Web Template export synthesizes from the RM rather than from the template ([REQ-106](clinical-modeling.md#req-106--webtemplate-json-export) *in-context* leaves) **MUST** either resolve through these read operations on a populated instance of its container RM type, or be a **documented exemption** recording why it stays unresolved and what that costs on encode. A guard test beside the WebTemplate builder is the enforcement point: a newly synthesized in-context leaf with no navigation support **MUST** fail that test rather than reach the encoder, and an exemption that no longer matches an emitted leaf **MUST** be reported as stale.
- The FLAT encoder **MUST NOT** silently drop a leaf that resolves and whose datatype it can serialise. Resolution failure **MAY** be treated as an absent optional (that is what an unpopulated optional attribute looks like); a leaf whose datatype the encoder cannot write **MAY** stay unemitted. Neither licence extends to a resolvable, serialisable leaf — an omission there is a defect, not an absent optional.

This enforcement is hand-kept rather than derived because `item_at_path` returns the same `ErrPathNotFound` whether the addressed attribute is *unknown to the navigator* or *known but unpopulated* — the conflation that kept the encode-side loss PROBE-086 found silent. Whether to split the sentinel (e.g. `ErrUnknownAttribute`) and retire the exemption list is open as [STRAND-10](research-strands.md#strand-10--rmpath-one-sentinel-for-two-not-found-conditions); it **MUST NOT** be resolved in code ahead of that strand.

**Acceptance:** against a representative composition (the blood-pressure / vital-signs shape, with real archetype-node-id and name predicates), a known unique leaf path resolves via `item_at_path` to the expected value; a non-unique path returns the expected item count via `items_at_path`; an absent path reports `path_exists = false`; a multi-match path reports `path_unique = false` and `item_at_path` errors. A typed-nil node anywhere in the tree (including the root) yields no match rather than a panic. For the in-context coverage rule: every leaf the WebTemplate synthesizes resolves on a populated instance of its container RM type, or appears in the exemption list with its reason and encode consequence — and a stale exemption fails the guard.
