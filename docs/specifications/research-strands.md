...
1. **Dedicated REST conformance probes (resolved 2026-09-09).** The plan called for four `testkit/probes/rest/*` probes (audit-details header grammar, System `OPTIONS`, Admin bulk-delete, Definition `/example`). They are dedicated Sandbox probes in [`testkit/probes/rest/`](../../testkit/probes/rest/) — PROBE-062 (`openehr-audit-details` grammar + Contribution round-trip, REQ-059), PROBE-102 (System `OPTIONS`, REQ-095), PROBE-103 (Admin bulk-delete, REQ-099), PROBE-104 (Definition `/example`, REQ-095) — promoted from the package `httptest` request-capture assertions, each with can-fail controls. They follow the request-capture idiom (driven by their package tests against a `sandbox.Scripted` route with a captured-request accessor); wiring them into the REQ-082 runner's catalog-wide entries is Phase 4 work of the [probe-runnability plan](../plans/2026-08-18-probe-runnability.md), so the gate this item originally waited for is the dedicated catalog probe, not the runner. Affects REQ-059, REQ-095, REQ-099.

2. **Stored-query `fetch` omission vs OpenAPI `required` (open; ADR pending).** The Definition/Query `Query` schema marks `offset`, `fetch`, and `query_parameters` `required`, but `fetch` has no spec default ("depends on the implementation") and `fetch: 0` means *zero rows*. Today the SDK emits `offset` (documented default 0) and `query_parameters` and omits `fetch` unless the caller sets it (`storedBody` in `openehr/client/query/execute.go`), a documented deviation from the literal `required` list. **Observed (2026-09-09):** EHRbase 2.35.1 returns `422 Unprocessable Entity` ("Query parameter for offset provided, but no fetch parameter") for a stored-query POST carrying `offset` without `fetch`; the Live snapshot sets `WithFetch` explicitly. That meets the revisit condition this item set (a conformant backend rejects a body without `fetch`). **Resolution form:** an ADR choosing between always emitting `fetch`, emitting a sentinel, or sending the server's advertised default; not yet written.
...
**Status:** Resolved. See [ADR 0020](../adr/0020-cassette-recording-har.md).
...
**Evidence (2026-09-05 census):** `internal/bmmgen/primitive_ancestor_census_test.go` walks every `class_definitions` class in each of the six pinned schema roots (`base 1.3.0`, `rm 1.2.0`, `am 1.4.0`, `am 2.4.0`, `lang 1.1.0`, `term 3.1.0`) and lists each property reachable only through a primitive-mapped ancestor — a property the class or a non-primitive ancestor declares itself is planned and shipped, so it does not count. Result: **exactly one** — `Iso8601_timezone.value` via `Iso8601_type`, present in the five roots that include `base` (`term 3.1.0` includes no base schema and has no ISO 8601 classes). The four DV temporal types (`DV_DATE`, `DV_TIME`, `DV_DATE_TIME`, `DV_DURATION`) also descend from `Iso8601_type` but redeclare `value` on the class, which is why PROBE-094's reduction never flagged them and why the generated structs carry it. The set is pinned by the test, so growth is loud, and the evidence is guarded three ways: the root list is checked against the schemas actually vendored under `resources/bmm`, so a newly vendored schema cannot pass unwalked; the census logic runs on hand-built schemas covering transitive, shadowed, secondary and cyclic ancestry, so a traversal or filter regression cannot silently empty or inflate the result; and the four DV temporal types are pinned as excluded *because* they redeclare `value`, not because they are absent. What this settles: `Iso8601_timezone` is a one-off, not the visible member of a family — the fold-into-both option's blast radius is one generated struct and one `rminfo` row. What it does not settle: the fold itself, which remains an ADR amending § Mapping rules plus a regenerated tree; the strand stays open.
...
**Why it's open:** today the two layers compose but do not chain. Template validity covers RM-mandatory presence, so a template-driven pass already reports the floor's (a) arm; the floor's (b) arm — `DV_INTERVAL` lower > upper, `TERM_MAPPING.match` outside its value set, `Mappings_valid`, `DV_QUANTITY.precision` and `DV_PROPORTION.precision` below -1 (-1 means no limit) — fires only through `ValidateRM`. A caller who runs only `ValidateComposition` can therefore commit an RM-invalid composition that is template-valid. Whether that is a gap or a deliberate separation of concerns is the fork.
...
| [STRAND-09](#strand-09--its-rest-conformance-follow-ups) | ITS-REST conformance follow-ups (REST probes **done**; stored-query `fetch` ADR pending) | Open (item 2) | REQ-059, REQ-095, REQ-099 |
...
| [STRAND-11](#strand-11--probe-recording-format-har-or-a-purpose-built-yaml) | Probe recording format (HAR vs YAML) | **Resolved** ([ADR 0020](../adr/0020-cassette-recording-har.md)) | REQ-082 |
