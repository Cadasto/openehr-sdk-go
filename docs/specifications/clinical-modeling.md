...
**Components are values, not spellings.** Every component the model exposes **MUST** be the value a reader compares or displays, not the source spelling of it: delimiters removed (an at-code without its brackets, a name without its quotes), escapes resolved, and trivia — whitespace and `--` comments — absent. This is the whole point: a component that still carries trivia leaves the consumer exactly where [§ REQ-119](#req-119--re-parseable-canonical-aql-emission) left it. `Raw` continues to carry the spelling, verbatim, for anyone who needs it.

**An unstructured predicate is a statement, not a gap.** A predicate the SDK does not structure **MUST** report itself as unstructured — a nil structured carrier beside a populated `Raw` — and the set of unstructured forms **MUST** be enumerated here. A reader **MUST** be able to fail closed on exactly the forms the SDK leaves raw, which it cannot do against a carrier that is sometimes nil for reasons the spec does not name. A **best-effort partial** structure is specifically forbidden: a partially-structured predicate is indistinguishable from a fully-structured one at the type level, so a consumer comparing components would silently compare an incomplete predicate as though it were complete. The enumerated unstructured set is **empty for the path-segment position** — every alternative the grammar admits there is structured.
...
- **The kind sets are held against the vendored artefacts mechanically.** A hand-written corpus is the list a maintainer must remember to update, which [§ REQ-119 § Acceptance](#req-119--re-parseable-canonical-aql-emission) already forbids for the guards it governs; the same standard binds here. The predicate corpus **MUST** be derived from the `pathPredicate` alternatives in `resources/aql/grammar/active/AqlParser.g4` and **MUST** fail when the grammar admits a form no kind covers; the drop-record completeness sweep **MUST** be derived from the extractor source and **MUST** be shown able to fail.
