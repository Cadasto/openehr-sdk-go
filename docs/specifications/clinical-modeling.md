...
Because the accept set is a lexical condition over bracket text rather than a token, it has no token NAMES to walk, so § Acceptance's mechanical-honesty rule binds in its generated form: the corpus MUST be generated and confronted with the parser in BOTH directions — accepted text must never re-parse as a different query, and refused text must never be text the parser reads back unchanged.
