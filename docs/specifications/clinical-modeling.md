...
- **Every shape, including its pointer twin.** `aql.Value` is satisfied by both the value and the pointer form of each shape — `aql.MatchesExpr.Terminology` is itself a `*aql.FuncCall` — so validation MUST normalise a pointer to the shape it addresses rather than admit it unchecked, and MUST refuse a nil one instead of panicking.
