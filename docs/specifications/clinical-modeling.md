...
- **Every shape, including its pointer twin.** The SDK's sealed interfaces are each satisfied by both the value and the pointer form of every shape, because their methods have value receivers — `aql.Value`, `aql.WhereExpr`, `parse.SelectExpr` and `parse.LimitExpr` all behave this way, and `aql.MatchesExpr.Terminology` is itself a `*aql.FuncCall`, so a pointer is reachable without a caller writing `&`. Any code that decides behaviour from a concrete shape MUST normalise first, and MUST refuse a nil one instead of panicking. The SDK MUST expose that normalisation for each vocabulary a consumer type-switches over ([`aql.DerefValue`](../../openehr/aql/value.go), [`aql.DerefWhere`](../../openehr/aql/where.go)), so a write path outside `openehr/aql` — and a consumer type-switching over an additive sealed set — binds the same rule to both carriers rather than to whichever one its `switch` happens to list.

  The rule binds **every dispatch site, not every validator**: enumerating the validators is what let it leak four times. Four corollaries, each of which is where it leaked in practice:

  - **The refusal path is bound too.** A diagnostic that names the offending operand MUST render it through the same normalisation, because an error message is built *before* the operand it quotes has been checked. A validator that crashes while reporting an invalid query is the worst form of this defect — the caller loses both the query and the reason.
  - **The emission path is bound too, and more sharply than validation.** Where emission chooses a spelling from a concrete shape — the precedence parentheses around a nested `aql.Junction` are the case in force — normalising in `validate` alone is not enough: the tree validates, then renders without its parentheses, and the result is valid AQL denoting a *different* predicate. That is the silent class, so this is the corollary with the worst failure mode.
  - **Every validating write path is bound, not the one that was noticed.** `aql.FormatWhere`, `aql.Builder.Build` and `(*parse.Query).Emit` are named together by the closure clause above and MUST behave alike on the same input; a fix applied to one of them leaves the others crashing on a value the requirement says is permitted.
...
- **A value-position function name** MUST lex as the grammar's `IDENTIFIER` or one of its `*_FUNCTION_ID` tokens. A name whose spelling matches a keyword token declared BEFORE `IDENTIFIER` in `AqlLexer.g4` MUST be refused — notably the aggregates (`COUNT`, `MIN`, `MAX`, `SUM`, `AVG`), which are real AQL functions but belong to `aggregateFunctionCall`, admitted in `SELECT` alone.
  ...
  - A **projected** function name in `SELECT` MUST be held to the same rule minus the aggregates, since `aggregateFunctionCall` is reachable from `columnExpr` and from no value position.
...
- a containment junction may only END a containment chain; and
- a class node MUST carry an RM type (or be a `VERSION` class expression), and MUST NOT carry a child-join of its own.
...
- **Lives in:** [`openehr/aql/value.go`](../../openehr/aql/value.go) (literal spellings, function names, `ValidateValue`, `EqualValues`), [`openehr/aql/where.go`](../../openehr/aql/where.go) (`FormatValue`, URI operand guard), [`openehr/aql/parse/extract_query.go`](../../openehr/aql/parse/extract_query.go) (unescaping, signed and scientific numeric extraction), [`openehr/aql/parse/query.go`](../../openehr/aql/parse/query.go) (SELECT-position validation, Emit-side structural parity).
