...
- **The kind sets are held against the vendored artefacts mechanically.** A hand-written corpus is the list a maintainer must remember to update, which [§ REQ-119 § Acceptance](#req-119--re-parseable-canonical-aql-emission) already forbids for the guards it governs; the same standard binds here. The predicate corpus **MUST** be derived from the `pathPredicate` alternatives in `resources/aql/grammar/active/AqlParser.g4` — **including the `pathPredicateOperand` production those alternatives reach**, whose members decide structured-versus-enumerated-unstructured; a sweep that stops at the top-level alternatives is how the operand forms went unenumerated in this section's first draft — and **MUST** fail when the grammar admits a form neither a kind nor the enumerated unstructured set covers; the drop-record completeness sweep **MUST** be derived from the extractor source and **MUST** be shown able to fail.
