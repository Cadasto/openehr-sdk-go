...
- The walk **MUST** exclude the `LOCATABLE` / `PATHABLE` housekeeping attributes — `name`, `uid`, `links`, `feeder_audit`, `archetype_details`, `archetype_node_id`, and the `parent` back-reference — because AQL `CONTAINS` navigates archetyped content, not audit or reference metadata. Concretely, an attribute whose declaring class (`rminfo` `DeclaredOn`) is `LOCATABLE` or `PATHABLE` is not a containment edge. Without this, an `ELEMENT` would reach `CLUSTER` through `LOCATABLE.feeder_audit → FEEDER_AUDIT_DETAILS.other_details` (an `ITEM_STRUCTURE`) — a by-value path the RM permits but AQL containment does not, which is why `ELEMENT CONTAINS CLUSTER` is **Never** (§ Acceptance).
