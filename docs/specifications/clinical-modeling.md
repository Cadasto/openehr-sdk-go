...
The operands are a different matter and stay LOUD, because `objectPath` recurses where the position does not: `pathPart : IDENTIFIER pathPredicate?` reaches `nodePredicate`, which reaches `objectPath` again. Deciding that `NOT a/b` is not a legal left operand therefore costs exactly the validator the exemption buys off, so it is left to the parser. **The position's SHAPE is decided; its OPERANDS are not** — and that line, rather than the severity of the text, is what this rule turns on.
...
- The class predicate positions are confronted the same way, and in BOTH directions, because the CLASS guard is a necessary condition rather than a sub-grammar parser and each direction fails on its own (the VERSION guard is held to its whole production instead, and gets the stricter oracle below): `TestPredicateGuardAgreesWithTheParser` generates the corpus from the fragments that drive the scanner through each of its states and asserts that accepted text never re-parses as a different query and that refused text is never text the parser reads back unchanged — the tightening claim stated as a property rather than argued. The corpus is generated because this position has no token NAMES to walk at all: its subject is bracket text, not a token. `TestPredicateSourceTextRoundTripsAsIdentity` holds every predicate form in the round-trip corpus to IDENTITY, which is what catches a mangled predicate that still parses. The VERSION position needs a corpus of its OWN, because the shared one is built from scanner-state fragments and so carries junk operands, over which the only assertable arm is "never a DIFFERENT query" — under which a hole in the SHAPE rule this § does decide passes unseen. `TestVersionPredicateShapeAgreesWithTheParser` therefore generates that position's candidates by crossing legal `objectPath` spellings with every `COMPARISON_OPERATOR` spelling and legal `pathPredicateOperand`s, beside the shapes the production does not have, so every accepted case MUST re-parse: with the operands legal by construction, an unparseable emission can only be a shape the production lacks.
