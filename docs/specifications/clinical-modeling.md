...
The skeleton MUST therefore be taken over what `Emit` renders, with the trees FLATTENED into ordered sequences — the class expressions of the `FROM` tree in source order (a junction node contributing none, since it renders no class of its own), and the leaves of the `WHERE` tree in order — because flattening is precisely the nesting freedom factored out. A substitution cannot preserve that skeleton: to change the query's meaning it must add or remove a class expression, a projection, or a predicate leaf.
...
- The after-emission verification § Emission verified after emission requires is held by three tests, one per direction it can fail in: `TestEmitRefusesACrossBracketRegexSubstitution` (issue #103's own two-predicate query, asserting the STRUCTURAL arm — a refusal on the re-parse arm would mean the row had stopped exercising the residual — and that the diagnostic leaks no predicate content), `TestEmitVerificationAcceptsEveryReNestedEncoding` (the anti-tightening arm a field-by-field AST comparison fails: junction re-nesting, a FROM-root junction, a `VERSION` class's ignored `RMType`, and a predicate carried without the read-side flag), and `TestEmitNeverReturnsUnparseableText` (the property the other two rest on, over a corpus that MUST exercise both outcomes or report itself one-sided).
