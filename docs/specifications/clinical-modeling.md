...
The four path, paging and projection codes are unconditional. `aql_contains_redundant_step` is not: it is the only check here that needs a **new query on the REQ-160 relation** — reachability from an ancestor class to a descendant class with one intermediate class excluded (working name `Unavoidable`) — and it is specified above so that the decision is reviewable against a written rule rather than against code. **If implementation review judges that API not worth its weight, the check MUST be recorded as out of scope with that reasoning (§ Out of scope) and this requirement ships four codes.** In that event the code string `aql_contains_redundant_step` **MUST NOT** be reused for another rule: a published code is stable in the same way a `REQ-NNN` is, and re-pointing it would make this requirement's history ambiguous.
...
- No REQ-109 or REQ-161 **code, severity, `Detail` text, `Span` rule or `Result.OK()` semantic** changes, and no entry point changes: that is the whole of the claim. REQ-161's catalogue gains one **scope-boundary sentence** on its `aql_fanout_row_grain` row and no rule — its existing rule and `Detail` text stay byte-stable, per [§ REQ-161 § Additivity](#additivity). What this group **does** widen is the direct `lint → rminfo` **consultation surface** [§ REQ-161 § Relation supply](#relation-supply) describes for its own checks, from one class question to two; that widening is recorded in § The conservative segment walk, adds no import edge, and changes nothing about what a supplied relation governs.
...
- `aql_contains_redundant_step`, **conditionally** — if implementation review cuts it under § The redundant-step ruling, that decision and its reasoning are recorded here and the code is not allocated.

- **Lives in:** [`openehr/aql/lint/`](../../openehr/aql/lint/) (extension)
- **Probes:** [PROBE-099](conformance.md#probe-099--aql-path-shape-lint-corpus)
- **Plan:** [`docs/plans/2026-08-26-aql-path-shape-lint.md`](../plans/2026-08-26-aql-path-shape-lint.md)
