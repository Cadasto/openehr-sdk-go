...
Escape is decided over the three states in which a bracket is CONTENT rather than a delimiter, each of which MUST be tracked:
...
- **A contained regex.** `SLASH_REGEX_CHAR : ~[/\n\r] | ESCAPE_SEQ | '\\/'` admits both brackets and the closing brace, so `{/[0-9]+/}` and `{/a{2}/}` MUST be accepted, and the region MUST be stepped over on the token's own terms rather than by searching for the first `}`.
- **A nested path predicate.** `pathPart : IDENTIFIER pathPredicate?` makes `a[at0001]/b='c'` legal, so a BALANCED pair MUST be accepted.

Balance MUST hold in both directions. A trailing unclosed `[` is not merely a loud error: the emitted `]` closes the INNER bracket, leaving the outer one to close on whatever `]` appears later in the query. `TERM_CODE`'s trailing `|…|` section needs no rule of its own — the grammar spells its content `~[|[\]]+`, which already excludes both brackets.
