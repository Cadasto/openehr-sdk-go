...
| Unaliased projection | `aql_select_no_alias` | Warning | A `SELECT` item carrying no `AS` alias **MUST** raise this code, once per item, with `Span` on the item: a stored-query contract and every column-addressed result reader depend on stable column names, and an unaliased column's name is engine-defined. A `*` item **MUST NOT** raise it — there is nothing to alias, and that shape is REQ-109's `aql_select_star`. A **bare alias** item (`SELECT o`) is **not** exempt: it names no column either, and the engine picks that column's name as freely as it picks a path's. An aliased item **MUST** stay silent. |
