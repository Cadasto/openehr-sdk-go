...
- No REQ-109 or REQ-161 code, severity, `Detail` text, `Span` rule or `Result.OK()` semantic changes. REQ-161's catalogue gains one **scope-boundary sentence** on its `aql_fanout_row_grain` row and no rule — its existing rule and `Detail` text stay byte-stable, per [§ REQ-161 § Additivity](#additivity).
