...
- **Lint owns the diagnosis** — codes `aql_deprecated_top` (Warning, whenever `TOP` is present), `aql_top_with_limit` (Error) and, for the envelope channel, `aql_top_with_fetch` (Error, when the linted `aql.Query` carries a `Fetch`). All three are Layer-2 checks whose canonical catalogue home is [§ REQ-109 — Layer 2](#layer-2--shape-ast-walk-no-cdr); this section adds no second copy of them. All three MUST key on the clause's **presence in the source**, not on a successfully decoded bound: an out-of-range count is refused by the extractor and leaves no bound to read, and a query pairing that count with a row limit is precisely one that must not lint clean.
