...
The skeleton MUST therefore be taken over what `Emit` renders, with the trees FLATTENED into ordered sequences — the class expressions of the `FROM` tree in source order (a junction node contributing none, since it renders no class of its own), and the leaves of the `WHERE` tree in order, with a same-operator chain spliced into its parent and a one-term junction reduced to its term, because those are exactly the encodings the emitted text erases. Where one value has TWO carriers the carrier MUST be normalised away, not compared: a comparison's path LHS reaches `Path` from the parser and `Left` from `aql.Compare`, and both render the same text. Any operand the skeleton reduces MUST be reduced through the SAME renderer emission uses, so the two cannot disagree about the text just written.
...
- **Carried:** the class sequence with each class's RM type, alias and bracket text; the containment CONNECTIVE sequence read IN ORDER between adjacent classes of the flattened tree — `CONTAINS`, the junction keywords, and negation riding on its edge — because the in-order reading is stable under exactly the re-nestings one text admits (`AND(AND(a,b),c)` and `AND(a,b,c)` both read `[AND, AND]`) where a per-node join slot was not, and because the RE-PARSED side's keywords come from the emitted text, which a path-position splice can counterfeit over an unchanged class sequence; the `WHERE` junction operators and leaves; `WHERE` PRESENCE as its own coordinate (absence and unreadability render the same nothing, so without it a dropped clause aliases an absent one); the projection COUNT and each item's `AS` alias, with the bare star and a SOLE unaliased star ITEM reduced to one encoding (`SELECT *` re-parses as the flag); `ORDER BY` arity, directions and path text; `LIMIT` / `OFFSET`. Path text enters a slot with its EDGE whitespace normalised: the emitters render a path verbatim while the parser's span strips the padding, so an untrimmed carrier from a primary constructor is one encoding of the trimmed text, not a different query.
- **Deliberately not carried,** each for a stated reason: a projection item's EXPRESSION (the SELECT vocabulary re-encodes, and an item whose path text changes its kind is the path-splice class § Out of scope defers); and the containment GROUPING itself (the tree's node count differs between encodings of one text). The junction KEYWORDS were once in this list on the ground that they are written from `ChildJoin`, never spliced — a one-sided argument, since the re-parsed side's keywords come from the emitted text; they are carried as the connective sequence above.
...
So the claim this closure supports is bounded and MUST be stated as such: a substitution that adds or removes a class expression, a projection, a predicate leaf, or a whole clause — or that flips a containment connective or a negation — cannot preserve the skeleton. It does not claim to catch a substitution confined WITHIN a path position's own text, which REQ-055 rule 3 exempts.
