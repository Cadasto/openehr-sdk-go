...
- **Every shape, including its pointer twin.** `aql.Value` is satisfied by both the value and the pointer form of each shape — `aql.MatchesExpr.Terminology` is itself a `*aql.FuncCall` — so validation MUST normalise a pointer to the shape it addresses rather than admit it unchecked, and MUST refuse a nil one instead of panicking. This binds the **refusal path too**: a diagnostic that names the offending operand MUST render it through the same normalisation, because an error message is built *before* the operand it quotes has been checked. A validator that crashes while reporting an invalid query is the worst form of this defect — the caller loses both the query and the reason.
