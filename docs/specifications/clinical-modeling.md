...
- **A contained regex whose body can continue PAST its own closing brace.** `{/a\/}` completes a token inside the predicate AND leaves the body reachable beyond it, because `'\\/'` carries a `/` inside the body and `}` is an ordinary body character. Which token the lexer forms therefore depends on text the predicate does not contain: with an ordinary suffix it is the short one and the round trip is identity, while a suffix carrying a later `/` … `}` forms a longer token that swallows the emitter's `]`. Both readings are the lexer's, so the escape condition here is a property of the WHOLE QUERY and no per-predicate validator can decide it — refusing it would refuse `{/a\/}`, which `ParseQuery` reads back unchanged in every ordinary query, i.e. the tightening failure. The completing-across-the-bracket case that IS decidable — a body still open at the end of the text — is refused (§ The class predicate positions); this residual is the remainder. Closing it means verifying AFTER emission rather than per position: `Emit` re-parsing its own output and comparing the AST would catch this and any successor, at the cost of a second parse per call and a new error mode. That is a scope change to the emission contract, so it is recorded here rather than taken — tracked as [issue #103](https://github.com/Cadasto/openehr-sdk-go/issues/103), which carries an end-to-end demonstration through `Emit`.
