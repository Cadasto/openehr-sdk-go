...
**The comparison runs over the REDUCED structure, and the reduction is § REQ-119 § Emission verified after emission's own** — this REQ adds no second one. Two consequences are named here because an implementer reading "item count" as a raw carrier count would get each one backwards:
