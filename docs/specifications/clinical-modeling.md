...
| `rm_version_valid` | a present-but-empty `ARCHETYPED.rm_version` (REQ-112 floor) |
...
- **ARCHETYPED** — wherever an `archetype_details` node is present, on any LOCATABLE, the floor **MUST** report its two RM-mandatory value-typed attributes. `archetype_id` (`ARCHETYPE_ID`) and `rm_version` (`String`) are value-typed, so the value-based walk cannot tell absent from `null` from empty (they decode to the same zero value). The value-based entries (`ValidateRM` and the typed sugars) therefore report an empty `archetype_id.value` as `required` at `…/archetype_details/archetype_id/value`, and an empty `rm_version` as `rm_version_valid` (`Rm_version_valid: not rm_version.is_empty`) at `…/archetype_details/rm_version`. Reporting an empty `archetype_id.value` as `required` is SDK policy: `OBJECT_ID.value` is RM-mandatory but carries no non-empty invariant, so this follows the policy the floor already applies to the empty `action_archetype_id` in `clinical_notes.v0`. The byte-based entry decides key presence from the source JSON and reports an absent key as `required` at the attribute path; its rules and scope live in § *Value-typed mandatory presence* below. Validating the `ARCHETYPE_ID` grammar is out of scope for this catalogue.
