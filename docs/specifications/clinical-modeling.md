...
- **Guarding the IDENTIFIER-token positions of a hand-built `Emit` AST** — `SelectItem.Alias`, `ClassExpr.Alias`, `ClassExpr.RMType` — where splice text (`Alias: "x, c/y"`) emits and re-parses as a different query. Unlike the path positions above, these admit exactly one grammar token, so an exact guard IS writable; it is DEFERRED, not refused, because the alphabet check alone still passes reserved words and the honest predicate ("lexes as a single `IDENTIFIER`") belongs with the grammar confrontations. Tracked as [#96](https://github.com/Cadasto/openehr-sdk-go/issues/96); until it lands these three positions carry the same developer-authored trust as paths.
