...
- **Carried:** the class sequence with each class's RM type, alias, bracket text and negation; the `WHERE` junction operators and leaves; `WHERE` PRESENCE as its own coordinate (absence and unreadability render the same nothing, so without it a dropped clause aliases an absent one); the projection COUNT and each item's `AS` alias; `ORDER BY` arity, directions and path text; `LIMIT` / `OFFSET`.
- **Deliberately not carried,** each for a stated reason: a projection item's EXPRESSION (the SELECT vocabulary re-encodes, and an item whose path text changes its kind is the path-splice class § Out of scope defers); a containment junction's `ChildJoin` (the containment tree's node count differs between encodings of one text, so a join slot has no stable position and refused legal re-nestings — and a join keyword is written by the emitter from the field itself, never spliced from caller text); and the containment GROUPING itself, for the same reason.
...
So the claim this closure supports is bounded and MUST be stated as such: a substitution that adds or removes a class expression, a projection, a predicate leaf, or a whole clause cannot preserve the skeleton. It does not claim to catch a substitution confined to a path position, which REQ-055 rule 3 exempts.
