...
- a class expression carrying a BLANK standing predicate. Emission brackets the field whenever it is non-empty, so a whitespace-only value emits `[   ]`, which no `pathPredicate` reading admits and the parser rejects. This is the emptiness EDGE of that position, not the sub-grammar guard deferred to [issue #99](https://github.com/Cadasto/openehr-sdk-go/issues/99): "is there any content at all" is decidable with a token-level check, which is what separates it. Nothing the extractor produces is affected, since a parsed bracket always carries a form.
