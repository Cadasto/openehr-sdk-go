...
- An attribute whose declaring class (`rminfo` `DeclaredOn`) is `LOCATABLE` or `PATHABLE` **MUST NOT** be a containment edge: AQL `CONTAINS` navigates archetyped content, not audit or reference metadata. On the current pin that excludes exactly the six `LOCATABLE` housekeeping attributes — `name`, `uid`, `links`, `feeder_audit`, `archetype_details`, `archetype_node_id`; the pin declares no `PATHABLE` properties (its `parent` back-reference is a BMM *function*, not an attribute), so the `PATHABLE` half is forward-looking pin insurance — the rule is declaration-site-derived, never a hand-kept name list. Without the exclusion an `ELEMENT` would reach `CLUSTER` through `LOCATABLE.feeder_audit → FEEDER_AUDIT_DETAILS.other_details` (an `ITEM_STRUCTURE`) — a by-value path the RM permits but AQL containment does not, which is why `ELEMENT CONTAINS CLUSTER` is **Never** (§ Acceptance).
...
- **Lives in:** [`openehr/aql/contain/`](../../openehr/aql/contain/) (Phase 1 — landed)
