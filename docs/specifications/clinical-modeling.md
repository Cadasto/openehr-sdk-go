...
>
> [§ REQ-119](#req-119--re-parseable-canonical-aql-emission) then narrowed residual 1 itself: `int64`'s range is asymmetric, so `math.MinInt64` is exactly representable and **MUST be accepted**. Residual 1 binds only a literal genuinely outside the type.
...
- **Every shape, including its pointer twin.** `aql.Value` and `aql.WhereExpr` are each satisfied by both the value and the pointer form of every shape — their methods have value receivers, and `aql.MatchesExpr.Terminology` is itself a `*aql.FuncCall`, so a pointer is reachable without a caller writing `&`. Any code that decides behaviour from a concrete shape MUST normalise first, and MUST refuse a nil one instead of panicking. The SDK MUST expose that normalisation ([`aql.DerefValue`](../../openehr/aql/value.go)) so a write path outside `openehr/aql` — and a consumer type-switching over an additive sealed set — binds the same rule to both carriers rather than to whichever one its `switch` happens to list.

  Two corollaries, each of which is where this rule leaked in practice:
  ...
  - **Absence is positional.** A predicate that denotes nothing — an untyped nil or a nil pointer — is *no clause* at the top of a WHERE, which is permitted, but MUST be refused as an operand of a `NOT` or a term of a junction, where an arm that quietly vanished changes the result set.
...
- Its containment structural refusals were required of `Build()` alone. They are now required of `(*parse.Query).Emit` too (§ Emit-side structural parity above), so the read and write sides refuse the same trees.
