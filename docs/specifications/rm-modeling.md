...
- **Generated, drift-detected accessor.** A stdlib-only building-block package, `openehr/terminology`, **MUST** expose every group and every code set of the pin as typed, closed, source-ordered values — groups with code → rubric, rubric → code and membership lookups, code sets with membership lookups — plus the pinned release version. Its tables **MUST** be generated from the pin and the build gate **MUST** fail when they drift from it (the REQ-042 rule). The package **MUST NOT** import `openehr/rm`, `openehr/serialize/*`, `openehr/client/*`, `transport/` or `auth/` — it sits *below* `openehr/rm` so RM-level consumers can adopt it — and it joins the [REQ-013](module-layout.md#req-013--building-block-independence) set.
- **One home per code.** SDK code that emits, defaults, validates or decodes an `openehr`-terminology coded value **MUST** take the code set, the rubric and the membership verdict from the accessor; a hand-typed openEHR code table or rubric outside it is a defect. A surface **MAY** keep named constants for the codes it promotes (`LifecycleStateComplete`, `ChangeTypeCreation`, …), but a code's validity at that surface **MUST** be its group membership, and a `DV_CODED_TEXT` the SDK builds from a code **MUST** carry that code's pinned rubric as its `value` — never a rubric typed beside the code.
