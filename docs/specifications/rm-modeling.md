...
## openEHR terminology vocabulary (REQ-034)

The openEHR Terminology — the `openehr` terminology's **groups** (*audit change type*, *version lifecycle state*, *setting*, *participation mode*, *composition category*, …) and **code sets** (*normal statuses*, *compression algorithms*, *integrity check algorithms*) that the RM's own invariants reference (`EVENT_CONTEXT.Setting_valid`, `AUDIT_DETAILS.Change_type_valid`, `PARTICIPATION.Mode_valid`, `DV_ORDERED.Normal_status_validity`, …) — is part of the specifications the SDK targets, published as a computable artefact by the openEHR Foundation. It is small, closed and versioned with the specifications; it is **not** a runtime terminology-service lookup. The SDK **MUST** carry it the way it carries the BMM ([bmm-conformance.md § REQ-041](bmm-conformance.md#req-041--pinned-bmm-sources) / [§ REQ-042](bmm-conformance.md#req-042--generated-code-drift-detected)):

- **Pinned source.** The Foundation's `openehr_terminology.xml` for one named TERM release **MUST** be vendored byte-identical under `resources/terminology/`, with the release, the upstream commit and the file's sha256 recorded beside it. A version bump is an explicit, reviewable commit; the pin **MUST NOT** be hand-edited, and the build gate **MUST** fail when the vendored file no longer matches its recorded sha256.
- **Generated, drift-detected accessor.** A stdlib-only building-block package, `openehr/terminology`, **MUST** expose every group and every code set of the pin as typed, closed, source-ordered values — groups with code → rubric, rubric → code and membership lookups, code sets with membership lookups — plus the pinned release version. Its tables **MUST** be generated from the pin and the build gate **MUST** fail when they drift from it (the REQ-042 rule). The package **MUST** import nothing outside the Go standard library — it sits *below* `openehr/rm` so RM-level consumers can adopt it without a cycle — and it joins the [REQ-013](module-layout.md#req-013--building-block-independence) building-block set, where its independence is enforced (`TestTerminologyForbiddenImports`).
- **One home per code.** SDK code that mints, defaults, validates, or reconstructs an `openehr`-terminology coded value from a bare code or a bare rubric **MUST** take the code set, the rubric and the membership verdict from the accessor; a hand-typed openEHR code table or rubric outside it is a defect. A codec that instead *transports* a caller-supplied code-and-rubric pair whole — the FLAT `ctx/setting` `|code`+`|value`, say — transcribes it and does **not** re-validate its membership here; enforcing the RM instance invariants (`EVENT_CONTEXT.Setting_valid`, …) on decoded values is [REQ-112](clinical-modeling.md#req-112--template-less-reference-model-validation-floor)'s, not this requirement's. A surface **MAY** keep named constants for the codes it promotes (`LifecycleStateComplete`, `ChangeTypeCreation`, …), but a code's validity at that surface **MUST** be its group membership, and a `DV_CODED_TEXT` the SDK builds from a code **MUST** carry that code's pinned rubric as its `value` — never a rubric typed beside the code.
- **Negative space.** A lookup of an unknown code or rubric **MUST** report absence (a false second return) rather than a fabricated value, and **MUST NOT** panic — on a nil group either (REQ-025). A surface that refused a code outside its group before adopting the accessor **MUST** keep refusing it afterwards; the guard tests carry that bar.

*Out of scope:* external terminologies (SNOMED CT, LOINC, …) — [REQ-105](clinical-modeling.md#req-105--terminology-bindings) covers the bindings an OPT declares, and a terminology *service* stays a consumer concern; wiring the template-less RM floor's coded invariants ([REQ-112](clinical-modeling.md#req-112--template-less-reference-model-validation-floor)) to the accessor is a follow-up, not part of this requirement.
