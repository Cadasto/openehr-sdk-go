...
- **A consumer that hashed, diffed or signed SDK output loses a guarantee it may have relied on.** Pre-1.0 this folds into an `### Added` CHANGELOG entry (the repository's pre-1.0 policy is `### Added` only — AGENTS.md) and a minor bump, not a deprecation cycle. A consumer needing a stable digest computes it over a canonicalisation of its own choosing. The SDK does not ship one, because an SDK-blessed canonical form would be the same promise wearing a different name.
