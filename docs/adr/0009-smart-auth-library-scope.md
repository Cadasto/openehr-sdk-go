...
| Library | Used for |
|---|---|
| `golang.org/x/oauth2` | PKCE parity (RFC 7636 challenge / verifier generation), token-endpoint helpers |
...
Considered and partially applied. `x/oauth2`'s PKCE helpers (`oauth2.GenerateVerifier`, `oauth2.S256ChallengeOption`) are used for RFC 7636 parity. However, the SDK needs its own `TokenSource` abstraction (with `ExpiresAt`, `Issuer`, `Scope` fields and `context.Context`-first semantics) so `x/oauth2` is used as a library, not as the top-level auth engine. This keeps the SDK's API shape independent of `x/oauth2`'s API surface.
