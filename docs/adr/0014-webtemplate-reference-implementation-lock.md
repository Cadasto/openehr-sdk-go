...
4. **Fixture provenance.** The reference OPT + WebTemplate JSON are vendored from a commit-pinned EHRbase `openEHR_SDK` revision (Apache-2.0) under `testkit/cassettes/` with a provenance + license note. The oracle is **`constrain_test`** (not the originally-scoped `corona_anamnese`): it exercises the core datatype set and needs no sibling disambiguation, so it exports cleanly. (`corona_anamnese` now compiles; its reused sibling archetype roots hit `ErrIDCollision` until REQ-116 landed the name-derived `id` — since then it exports and serves as a PROBE-075 parity oracle.) If the environment blocks the fetch, PROBE-075 is deferred and the export is anchored to SDK-generated goldens until the fixture is supplied — the reason recorded in the probe catalogue.
...
- **Archetype-reuse-under-slot gap (partially closed):** several EHRbase reference templates — including the canonical `corona_anamnese` — reuse one archetype multiple times under a multi-valued slot (name-distinguished instances), which yields **duplicate compiled AQL paths**. `templatecompile.Compile` rejected these as `ErrInvalidInput` when this ADR was written; the first-wins path index anticipated here has since **landed** (shared-path subtrees are admitted, the tree still retains every instance), so they compile. The oracle stays `constrain_test` because export is still blocked one layer further on: each reused sibling derives the same web `id`, so `Build` fails with `ErrIDCollision`. Unlocking export needs the template-level node name and name-predicated paths — specified as [REQ-116](../specifications/clinical-modeling.md#req-116--template-level-node-naming-and-name-predicated-paths), which converges on this ADR's reference lock rather than departing from it.
