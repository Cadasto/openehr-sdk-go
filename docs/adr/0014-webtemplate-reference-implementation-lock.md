...
4. **Fixture provenance.** The reference OPT + WebTemplate JSON are vendored from a commit-pinned EHRbase `openEHR_SDK` revision (Apache-2.0) under `testkit/cassettes/` with a provenance + license note. The oracle is **`constrain_test`** (not the originally-scoped `corona_anamnese`): it compiles under the SDK's `templatecompile` and exercises the core datatype set. If the environment blocks the fetch, PROBE-075 is deferred and the export is anchored to SDK-generated goldens until the fixture is supplied — the reason recorded in the probe catalogue.
...
- **Archetype-reuse-under-slot gap (deferred):** several EHRbase reference templates — including the canonical `corona_anamnese` — reuse one archetype multiple times under a multi-valued slot (name-distinguished instances), which yields **duplicate compiled AQL paths** that `templatecompile.Compile` currently rejects as `ErrInvalidInput`. Such templates cannot be exported by this slice; the oracle was switched to `constrain_test` (unique paths). Relaxing the compiler to a first-wins path index (the tree already retains every instance) would unlock them and is recorded as a possible follow-up against REQ-100/REQ-111 — out of scope for REQ-106.
