...
| `.schema`-suffixed media types on input | **Landed 2026-09-05** (REQ-053 SHOULD): [`ParseMediaType`](mediatype.go) classifies one media-type token under the § REQ-053 contract (the two canonical strings and EHRbase's `.schema`-suffixed variants accepted; anything else refused with `ErrUnknownMediaType`); `Format.MediaType` emits the canonical strings only, never `.schema`. The codecs still take bytes — negotiation is the one call before them. | Landed |
