...
| Underscore-prefixed RM attributes — **rest deferred**: `_feeder_audit`, `_health_care_facility`, `_participation:N`, `_other_participation:N`, `_identifier:N`, `_charset`, `_thumbnail`, and DV_TEXT's own `_language` / `_encoding`; the DV_TEMPORAL `_accuracy` (a DV_DURATION object, so it has no scalar suffix); plus the two the plan defers indefinitely, `_instruction_details` (ACTION) and `_wf_definition` (INSTRUCTION), which the spec names but the pinned corpus does not exercise. | Refused on decode as `ErrUnknownPath` through the router's unknown-family arm, so they stay visible in the PROBE-086 census rather than decoding into nothing; a value carrying one of the still-deferred *value* decorations (`language`, `encoding`, `accuracy` on a temporal) rides `\|raw` whole on encode, losslessly. The party families land in REQ-140 Phase C2, `_feeder_audit` and the DV_MULTIMEDIA members in C3. | Deferred |
