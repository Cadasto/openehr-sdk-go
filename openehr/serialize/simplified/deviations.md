...
| **Vendored vocabulary** — the openEHR Terminology `participation mode` group (codes 193–224, code ↔ rubric), read from `openehr/terminology` (`terminology.ParticipationMode`). | The only terminology this codec reads, and it reads it because the reference gives `\|mode` **no code channel**: it writes the bare rubric (`"face-to-face communication"`) where PARTICIPATION.mode is a DV_CODED_TEXT whose `defining_code` is RM-mandatory, so rebuilding the code is the only alternative to fabricating an empty CODE_PHRASE. **No hand-typed copy remains in the codec** (since 2026-09-07, [§ REQ-034](../../../docs/specifications/rm-modeling.md#openehr-terminology-vocabulary-req-034)): the table is generated from the pinned `resources/terminology/openehr_terminology.xml` (TERM Release-3.0.0), so it is versioned with the openEHR specifications rather than transcribed here — and it is still a compiled-in table, not a runtime terminology lookup. The same package supplies the `openehr` terminology **identifier** every implied-terminology site in this codec uses (`\|normal_status`, `ctx/setting`) and the rubrics of the three `ctx/` completion defaults (`238 other care`, `433 event`, `640 actual`), so no rubric is typed beside a code anywhere in the codec. A rubric outside the group is refused loudly; encode is the exact inverse and refuses a mode whose code, terminology (`openehr` is implied) or value that rebuild would not reproduce, plus any DV_CODED_TEXT decoration the bare key cannot hold. `TestParticipationModeRoundTripsEveryGroupMember` pins the round trip over all 32 members, and `TestCtxDefaultsAreGroupMembersWithPinnedRubrics` pins that each `ctx/` default is a group member carrying the pin's own rubric. | landed (REQ-140 Phase C2, 2026-08-05; reads the pin since REQ-034, 2026-09-07) |
...
| `.schema`-suffixed media types on input | **Landed 2026-09-05** (REQ-053 SHOULD): [`ParseMediaType`](mediatype.go) classifies one media-type token under the § REQ-053 contract (the two canonical strings and the deprecated `.schema`-suffixed variants — retired from the specification, still served by EHRbase — accepted; anything else refused with `ErrUnknownMediaType`); `Format.MediaType` emits the canonical strings only, never `.schema`. The codecs still take bytes — negotiation is the one call before them. | Landed |
...
- **Empty repeat instances are not representable in FLAT** — an instance of a repeatable
  ...
  **minus that instance**, because the format simply has no spelling for "an instance
  with no values". The third of the three narrowings listed at the top of this file;
  the other two are the composer projection and the folded structural wrappers.
...
`allowedSuffixes` entry: it rode `|raw` on encode and its suffix form was *refused* on decode,
an unregistered gap rather than a decision. It now takes `|code` + `|value` + `|ordinal`,
`|ordinal` carrying the Real. **Not corpus-pinned:** no vendored body and no reference sample
spells a DV_SCALE, so — like the `"|"` member above — this is this codec's choice within the
format's conventions; reusing the sibling's spelling for the sibling's attribute is the same
move the party `|type` suffix makes. Should a reference sample later spell it otherwise,
[ADR 0014](../../../docs/adr/0014-webtemplate-reference-implementation-lock.md) means the
reference wins. No census movement: DV_SCALE appears in no fixture.

**Interval bound vs `|*_unbounded`.** wire.md § REQ-140 now states the rule for both
directions: a bound standing beside its own `|*_unbounded: true` is a typed error on decode
and on encode. Encode can only report it where the instantiation distinguishes the bound from
absence — see `boundContradictsUnbounded`; a concrete `Interval[DVCount]` cannot, and that
...
output unencodable. The same trade as the ADR 0015 composer projection (§ deferred features, the `ctx/`
context row), for the same reason.
