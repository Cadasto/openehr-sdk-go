...
| `.schema`-suffixed media types on input | **Landed 2026-09-05** (REQ-053 SHOULD): [`ParseMediaType`](mediatype.go) classifies a Content-Type / Accept value as FLAT or STRUCTURED, accepting the two canonical strings and EHRbase's `.schema`-suffixed variants (case-insensitive, parameters ignored); `Format.MediaType` emits the canonical strings only, never `.schema`. The codecs still take bytes — negotiation is the one call before them. | Landed |
