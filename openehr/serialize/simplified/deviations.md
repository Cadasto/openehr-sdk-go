...
- **Encode** — an unmapped **clinical** datatype (`DV_*`) returns
  [`ErrUnsupportedDatatype`](simplified.go) rather than dropping the value. A container
  node that does not resolve to a `Locatable` is an error, not a skip.
