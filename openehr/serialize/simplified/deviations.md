...
semantics-preserving), with exactly two registered exceptions on the **encode**
leg — the ADR 0015 composer projection (§ `ctx/` context row below) and the
folded structural wrappers (§ below). Both are named in
`wire.md` § REQ-140 as deliberately out of scope. Concretely:
...
- **Empty repeat instances are not representable in FLAT** — an instance of a repeatable
  ...
  **minus that instance**, because the format simply has no spelling for "an instance
  with no values". One of three places encode narrows rather than erroring; the other
  two are the composer projection and the folded structural wrappers, both below.
