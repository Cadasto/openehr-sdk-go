...
// AbsenceReason says why an RM type name is not in the class universe. A
// name can be undeclared by the pinned schemas, declared but in a package the
// generator skips wholesale, declared but named in its class-exclusion set, a
// primitive value rather than a modelled class, or a BMM enumeration. A plain
// membership boolean cannot tell these five cases apart.
//
// The zero value, [AbsenceNone], means the name is in the universe, so a
// zero value cannot be misread as "absent".
//
// The set is closed and suitable for an exhaustive switch. A member may be
// added in a minor release, so a consumer switching on it should handle one
// it does not recognise rather than assume the set it compiled against. Compare reasons with == or a switch; apart from
...
const (
	// AbsenceNone means the name is in the class universe and is not absent
	// at all. It is the zero value, and the answer for exactly the names
	...
	// is one the generator skips wholesale (the ehr_extract package, and the
	// functional/builtins foundation packages). Which package it is, is not
	// reported.
	...
	// package models. It is reported even where the SDK does have a Go type
	// for the name, because being outside the class universe does not mean
	// no Go type exists.
	AbsencePrimitive
...
// the counterpart of the membership question [Lookup.KnownRMTypes] answers.
//
// It is an optional capability interface beside [Lookup] rather than an
// extension of it, because widening Lookup or [Hierarchy] would break every
// external implementer; [AttributeLister] is kept separate for the same
// reason. [Default] implements it, and so does every [Lookup] returned by [New] and
...
type AbsenceReporter interface {
	...
	// [Lookup.KnownRMTypes]. An out-of-universe name that no absence entry
	// backs, including the empty string, reports [AbsenceUndeclared].
...
// pinned RM. Intended for unit tests, like [New]; production callers
...
// defines always reports [AbsenceNone], whatever absence says about it, so
// membership and absence never disagree.
...
// [Lookup.KnownRMTypes].
//
// The returned Lookup retains both maps rather than copying them, as [New]
// retains data. Callers must not mutate either afterwards; see [New] for
// what that costs.
