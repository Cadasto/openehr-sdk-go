...
// covers the whole grammar profile. A reader must be able to switch
// exhaustively and fail closed; adding a member is a consumer-visible
// change.
...
const (
	...
	// class a legal query reaches: degrading it to a float would silently
	// change the query, so it is dropped instead.
	KindNumericOutOfRange

	// KindUnmodelledConstruct is a construct the extraction catalogue does
	// not model. No legal query reaches it while the catalogue covers the
	// whole SDK grammar profile: every site that records it is a defensive
	// arm, present so that a widened grammar fails loudly here instead of
	// dropping a clause silently. It is a single member instead of one per
	// arm because no corpus can reach it, so a per-arm enumeration could
	// never be shown complete.
...
// represent, in source order. Empty when [Document.QueryErr] is nil.
//
...
// and re-readable.
//
// The [Document.QueryErr] text and its [aql.ErrIncompleteAST] sentinel are
// independent of these records, which add detail alongside them.
