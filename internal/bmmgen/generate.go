...
// # Targets
//
// A [Target] identifies one generation destination: the RM target
// ([TargetRM] → openehr/rm/) or the AOM 1.4 target
// ([TargetAOM14] → openehr/aom/aom14/). The two targets share
// openehr_base_1.3.0 by include, but AOM does not duplicate base
// classes; it references them as `rm.<Name>` via a one-way
// `aom14 → rm` import.
//
// # Registry location
//
// Registrations are emitted as `<package>/typereg_gen.go`, inside the
// target package, so the constructors
// `func() any { return &DVQuantity{} }` can refer to the target types
// directly. Placing them in the typereg package would create an import
// cycle (typereg → rm for the constructors, rm → typereg for the
// [typereg.Registry] type); this way typereg stays free of reverse
// dependencies. `typereg.Default.Register` is called once per concrete
// class at init time.
//
...
// edited by hand. Hand-written companions named `*_ext.go` are
// preserved untouched.
//
...
// from the regenerated output. This is an internal package and not
...
type Options struct {
	...
	// only. Retained for backwards compatibility with the older
	// single-target CLI. New callers should populate
	// [Targets] directly.
	RootID string
...
type Result struct {
	...
	Notes []string
	// MethodStubsEmitted is the total number of BMM function
