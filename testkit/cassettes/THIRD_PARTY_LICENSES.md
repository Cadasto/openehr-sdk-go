...
**REQ-116 sibling-naming oracles (`webtemplate/`):** `Corona_Anamnese.opt` + `Corona_Anamnese.webtemplate.json` (upstream `corona_anamnese.opt` / `webtemplate/corona_anamnese.json`) and `GECCO_Diagnose.opt` + `GECCO_Diagnose.webtemplate.json` (upstream `GECCO_Diagnose.opt` / `webtemplate/GECCO_Diagnose.json`), pinned at the same commit `22b01e0c99b53669394e56da29c2410838b5cf7e`. Vendored unmodified as the REQ-116 oracles ([plan](../../docs/plans/2026-07-29-template-node-naming.md)) covering the gap's two failure modes: `Corona_Anamnese` fails loudly (four `SECTION.adhoc.v1` siblings reuse one archetype, and eight reused screening OBSERVATIONs under Symptome do the same — `webtemplate.Build` returns `ErrIDCollision`; its golden carries 350 name-predicate segments over 213 `aqlPath`s), `GECCO_Diagnose` diverges silently (builds today, but 24 reference `aqlPath`s carry 30 name-predicate segments this builder does not yet emit). Not part of the SDK runtime.
