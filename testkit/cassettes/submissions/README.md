...
Each file is a full `CONTRIBUTION` body with `versions[]` holding inline `ORIGINAL_VERSION<T>` payloads (the ITS-REST submission shape). They are **not** persisted `CONTRIBUTION` resources, whose `versions` are `OBJECT_REF` only.
