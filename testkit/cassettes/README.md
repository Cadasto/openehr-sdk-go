...
| `GECCO_Diagnose` | REQ-116 oracle — silent mode: builds today; its golden carries 30 name-predicate segments over 24 `aqlPath`s (its three `/content` children have **distinct** archetype ids and are all predicated), all 24 now reproduced since Phase 3. Residual divergence: four spurious `…/name` data leaves (Phase 4) | 210 KB + 73 KB |
