...
func Probe005ScopeRoundTrip(ctx context.Context) (Result, error) { // PROBE-005 (REQ-061)
	...
	cap, err := runPKCEFlow(ctx, []string{want, "openid"}, "")
	if err != nil {
		return r, fmt.Errorf("PROBE-005: %w", err)
	}

	authScopes := strings.Fields(cap.authQuery.Get("scope"))
	...
	respScopes := strings.Fields(cap.tokenResp.Scope)
	if !slices.Contains(respScopes, want) {
		r.Status = "fail"
		r.Detail = fmt.Sprintf("token response scope = %q; want it to contain %q", cap.tokenResp.Scope, want)
		return r, nil
	}
